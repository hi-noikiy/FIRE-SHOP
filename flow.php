...
if ($_REQUEST['step'] == 'add_cart_combo_goodsAttr') {
	...
	if ((!isset($_CFG['use_bonus']) || ($_CFG['use_bonus'] == '1')) && ($flow_type != CART_GROUP_BUY_GOODS) && ($flow_type != CART_EXCHANGE_GOODS)) {
		$user_bonus = user_bonus($_SESSION['user_id'], $total['goods_price'], $cart_value, $total['seller_amount']);
		...
		if (defined('THEME_EXTENSION')) {
			$bonus = get_user_bouns_new_list($_SESSION['user_id'], 1, 0, 'bouns_available_gotoPage', 0, 20);
	...
	if (($_CFG['use_coupons'] == 1) && ($flow_type == CART_GENERAL_GOODS)) {
		...
		if (defined('THEME_EXTENSION')) {
			$coupons_list = get_user_coupons_list($_SESSION['user_id'], true);
			...
			if (!empty($coupons_list)) {
				foreach ($coupons_list as $k => $v) {
					...
					$coupons_list[$k]['cou_goods_name'] = $v['cou_goods'] ? $_LANG['lang_goods_coupons']['is_goods'] : $_LANG['lang_goods_coupons']['is_all'];
	...
	if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type)) {
		...
		$smarty->assign('config', $_CFG);
		$order = flow_order_info();
		$value_card = value_card_info(intval($_GET['value_card']));
		if ((!empty($value_card) && ($value_card['user_id'] == $_SESSION['user_id'])) || ($_GET['bonus'] == 0)) {
			$order['vc_id'] = intval($_GET['value_card']);
		}
		else {
			$order['vc_id'] = 0;
	...
	if ($_REQUEST['act'] == 'balance') {
		...
		if ($order_amount <= $user_money) {
			...
			if (0 < $order_info['user_id']) {
				...
				$is_update_sale = is_update_sale($order_info['order_id']);
				...
				if (($GLOBALS['_CFG']['sms_order_payed'] == '1') && ($sms_shop_mobile != '')) {
					...
					$smsParams = array('shop_name' => $shop_name, 'order_sn' => $order_info['order_sn'], 'consignee' => $order_info['consignee'], 'order_region' => $order_region, 'address' => $order_info['address'], 'order_mobile' => $order_info['mobile'], 'mobile_phone' => $sms_shop_mobile);
				...
				if ($stores_order && (0 < $stores_order['store_id'])) {
					...
					if ($user_mobile_phone) {
						...
						$store_smsParams = array('user_name' => $user_name, 'order_sn' => $order_info['order_sn'], 'code' => $pick_code, 'store_address' => $store_address, 'mobile_phone' => $user_mobile_phone);
		...
		if (file_exists(MOBILE_DRP)) {
			...
			$sql = 'INSERT INTO ' . $ecs->table('order_goods') . '( ' . 'order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, goods_price,' . 'goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, freight, tid, shipping_fee, is_distribution, drp_money, commission_rate) ' . ' SELECT \'' . $new_order_id . '\', ca.goods_id, ca.goods_name, ca.goods_sn, ca.product_id, ca.goods_number, ca.market_price, ca.goods_price, ca.goods_attr, ' . 'ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.model_attr, ca.goods_attr_id, ca.ru_id, ca.shopping_fee, ca.warehouse_id, ca.area_id, ca.freight, ca.tid, ca.shipping_fee,' . 'g.is_distribution*\'' . $is_distribution . '\' AS is_distribution, ' . 'g.dis_commission * g.is_distribution * ca.goods_price * ca.goods_number / 100 * \'' . $is_distribution . '\' AS drp_money, ca.commission_rate' . ' FROM ' . $ecs->table('cart') . ' ca' . ' LEFT JOIN  ' . $ecs->table('goods') . ' as g ON ca.goods_id = g.goods_id' . ' WHERE ca.' . $sess_id . ' AND ca.rec_type = \'' . $flow_type . '\'' . $goodsIn;
			$db->query($sql);
		}
		else {
			$goodsIn = '';
			...
			$sql = 'INSERT INTO ' . $ecs->table('order_goods') . '( ' . 'order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, commission_rate, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, freight, tid, shipping_fee) ' . ' SELECT \'' . $new_order_id . '\', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, commission_rate, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, freight, tid, shipping_fee' . ' FROM ' . $ecs->table('cart') . ' WHERE ' . $sess_id . ' AND rec_type = \'' . $flow_type . '\'' . $goodsIn;
			$db->query($sql);
		}
	...
	if (defined('THEME_EXTENSION')) {
		$region = array('province' => $order['province'], 'city' => $order['city'], 'district' => $order['district']);
	...
	if (isset($_POST['rec_id']) && isset($_POST['goods_number'])) {
		...
		if (0 < $val) {
			...
			if ($row['extension_code'] == 'package_buy') {
				...
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' SET goods_number= \'' . $val . '\', goods_price = \'' . $goods_price . '\', freight = \'' . $row['freight'] . '\', tid = \'' . $row['tid'] . '\', shipping_fee = \'' . $row['shipping_fee'] . '\' WHERE rec_id=\'' . $key . '\' AND ' . $sess_id;
			}
		}
		else {
			for ($i = 0; $i < count($offers_accessories_res); $i++) {
