...
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0)
{
	...
	if ($status == 1) {
		...
		$cache_info = cache('message_' . $_SESSION['user_id']);
	}
	...
	foreach ($res as $key => $row) {
		...
		if ($row['order_status'] == OS_UNCONFIRMED) {
			$row['handler'] = '<a class="btn-default-new br-5" href="' . url('user/order/cancel', array('order_id' => $row['order_id'])) . '" onclick="if (!confirm(\'' . L('confirm_cancel') . '\')) return false;">' . L('cancel') . '</a>';
		}
...
function get_order_detail($order_id, $user_id = 0)
{
	...
	$sql = 'SELECT cu.uc_id, c.cou_money, cu.cou_id FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' as cu' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('coupons') . ' as c' . ' ON cu.cou_id = c.cou_id' . ' WHERE cu.order_id = ' . $order['order_id'];
...
function get_user_bouns_list($user_id, $type = 0, $num = 10, $start = 0)
{
	...
	$sql = 'SELECT  u.bonus_sn, u.order_id, b.user_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ' . ' FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' AS u ,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b ' . ' WHERE  u.bonus_type_id = b.type_id ' . ' ' . $where . ' ' . ' AND u.user_id = \'' . $user_id . '\'';
	...
	foreach ($res as $row) {
		...
		if (100 <= $row['type_money']) {
			$temark = 1;
		}
		else if (50 <= $row['type_money']) {
			$temark = 2;
		}
		else {
			$temark = 3;
