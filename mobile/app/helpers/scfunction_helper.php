...
function get_insert_order_goods_single($orderInfo, $row, $order_id, $postscript = '', $ru_number)
{
	...
	$shipping_code = (!empty($row['shipping_code']) ? $row['shipping_code'] : 0);
	...
	foreach ($newInfo as $key => $info) {
		...
		$row['shipping_code'] = !empty($shipping['shipping_code']) ? $shipping['shipping_code'] : 0;
		$row['shipping_type'] = !empty($shipping['shipping_type']) ? $shipping['shipping_type'] : 0;
		...
		do {
			...
			$row['integral'] = integral_of_value($row['integral']);
...
{
	$shippingInfo = array('shipping_id' => '', 'shipping_code' => '', 'shipping_name' => '', 'shipping_type' => $GLOBALS['_CFG']['freight_model'], 'shipping_fee' => '', 'shipping_fee_formated' => '', 'free_money' => '', 'is_shipping' => 0);
	$shippingFee = 0;
	...
	$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' . $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' . $GLOBALS['ecs']->table('area_region') . ' AS r ' . 'WHERE r.region_id ' . db_create_in($region) . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.shipping_id = \'' . $sellerShippingInfo['shipping_id'] . '\' AND s.enabled = 1 AND a.ru_id = \'' . $goodsInfo['user_id'] . '\' ORDER BY s.shipping_order';
...
function get_goods_transport_tpl($goodsInfo = array(), $region = array(), $shippingInfo = array(), $goods_number = 1)
{
	...
	if (empty($shippingInfo)) {
		...
		if (!$shippingInfo) {
			$tpl_shipping = get_goods_transport_tpl_shipping($goodsInfo['tid'], 0, $region, 1, 1);
	...
	$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' . $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' . $GLOBALS['ecs']->table('area_region') . ' AS r ' . 'WHERE r.region_id ' . db_create_in($region) . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.shipping_id = \'' . $shippingInfo['shipping_id'] . '\' AND s.enabled = 1 AND a.ru_id = \'' . $goodsInfo['user_id'] . '\'';
	$val = $GLOBALS['db']->getRow($sql);
	$is_shipping = 0;
	...
	$arr = array('shippingFee' => $shippingFee, 'shipping_fee_formated' => price_format($shippingFee, false), 'is_shipping' => $is_shipping);
	return $arr;
}
