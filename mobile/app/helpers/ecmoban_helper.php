...
{
	$sql = 'select g.goods_id, c.comment_id from ' . $GLOBALS['ecs']->table('goods') . ' as g, ' . $GLOBALS['ecs']->table('comment') . ' as c' . ' where g.user_id = \'' . $ru_id . '\' and g.goods_id = c.id_value order by c.comment_id DESC limit 0, 100';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();

	foreach ($res as $key => $row) {
		$arr[$key] = $row;
		$arr[$key]['mc_all_Rank'] = ments_count_all($row['goods_id'], 'comment_rank');
		$arr[$key]['mc_one_Rank'] = ments_count_rank_num($row['goods_id'], 1, 'comment_rank');
		$arr[$key]['mc_two_Rank'] = ments_count_rank_num($row['goods_id'], 2, 'comment_rank');
		$arr[$key]['mc_three_Rank'] = ments_count_rank_num($row['goods_id'], 3, 'comment_rank');
		$arr[$key]['mc_four_Rank'] = ments_count_rank_num($row['goods_id'], 4, 'comment_rank');
		$arr[$key]['mc_five_Rank'] = ments_count_rank_num($row['goods_id'], 5, 'comment_rank');
		$arr[$key]['mc_all_Server'] = ments_count_all($row['goods_id'], 'comment_server');
		$arr[$key]['mc_one_Server'] = ments_count_rank_num($row['goods_id'], 1, 'comment_server');
		$arr[$key]['mc_two_Server'] = ments_count_rank_num($row['goods_id'], 2, 'comment_server');
		$arr[$key]['mc_three_Server'] = ments_count_rank_num($row['goods_id'], 3, 'comment_server');
		$arr[$key]['mc_four_Server'] = ments_count_rank_num($row['goods_id'], 4, 'comment_server');
		$arr[$key]['mc_five_Server'] = ments_count_rank_num($row['goods_id'], 5, 'comment_server');
		$arr[$key]['mc_all_Delivery'] = ments_count_all($row['goods_id'], 'comment_delivery');
		$arr[$key]['mc_one_Delivery'] = ments_count_rank_num($row['goods_id'], 1, 'comment_delivery');
		$arr[$key]['mc_two_Delivery'] = ments_count_rank_num($row['goods_id'], 2, 'comment_delivery');
		$arr[$key]['mc_three_Delivery'] = ments_count_rank_num($row['goods_id'], 3, 'comment_delivery');
		$arr[$key]['mc_four_Delivery'] = ments_count_rank_num($row['goods_id'], 4, 'comment_delivery');
		$arr[$key]['mc_five_Delivery'] = ments_count_rank_num($row['goods_id'], 5, 'comment_delivery');
		$sql = 'select comment_id from ' . $GLOBALS['ecs']->table('comment') . ' where id_value = \'' . $row['goods_id'] . '\' AND status = 1 AND parent_id = 0';
	...
	@$arr['cmt']['commentRank']['zconments'] = get_conments_stars($arr['commentRank']['mc_all'], $arr['commentRank']['mc_one'], $arr['commentRank']['mc_two'], $arr['commentRank']['mc_three'], $arr['commentRank']['mc_four'], $arr['commentRank']['mc_five']);
	@$arr['cmt']['commentServer']['zconments'] = get_conments_stars($arr['commentServer']['mc_all'], $arr['commentServer']['mc_one'], $arr['commentServer']['mc_two'], $arr['commentServer']['mc_three'], $arr['commentServer']['mc_four'], $arr['commentServer']['mc_five']);
	@$arr['cmt']['commentDelivery']['zconments'] = get_conments_stars($arr['commentDelivery']['mc_all'], $arr['commentDelivery']['mc_one'], $arr['commentDelivery']['mc_two'], $arr['commentDelivery']['mc_three'], $arr['commentDelivery']['mc_four'], $arr['commentDelivery']['mc_five']);
	...
	if (4 <= $arr['cmt']['commentRank']['zconments']['score']) {
		$arr['cmt']['commentRank']['zconments']['goodReview'] = '高';
	}
	else if (3 < $arr['cmt']['commentRank']['zconments']['score']) {
		$arr['cmt']['commentRank']['zconments']['goodReview'] = '中';
	}
	else {
		$arr['cmt']['commentRank']['zconments']['goodReview'] = '低';
	}

	if (4 <= $arr['cmt']['commentServer']['zconments']['score']) {
		$arr['cmt']['commentServer']['zconments']['goodReview'] = '高';
	}
	else if (3 < $arr['cmt']['commentServer']['zconments']['score']) {
		$arr['cmt']['commentServer']['zconments']['goodReview'] = '中';
	}
	else {
		$arr['cmt']['commentServer']['zconments']['goodReview'] = '低';
	}

	if (4 <= $arr['cmt']['commentDelivery']['zconments']['score']) {
		$arr['cmt']['commentDelivery']['zconments']['goodReview'] = '高';
	}
	else if (3 < $arr['cmt']['commentDelivery']['zconments']['score']) {
		$arr['cmt']['commentDelivery']['zconments']['goodReview'] = '中';
	}
	else {
		$arr['cmt']['commentDelivery']['zconments']['goodReview'] = '低';
	}
...
function get_user_default($user_id = 0)
{
	...
	$info['user_picture'] = get_image_path($wechat_user['user_picture']);
