...
namespace app\http\seckill\controllers;
...
class Index extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		...
		if (IS_AJAX) {
			...
			exit(json_encode(array('gb_list' => $list, 'totalPage' => $list['totalPage'], 'page' => $page, 'end_time' => $end_time, 'seckill_status' => $seckill_status)));
		}

		$this->assign('time_list', $time_list);
		$this->assign('page_title', '限时秒杀');
		$this->display();
	}
	...
	public function actionDetail()
	{
		...
		$sql = 'select b.is_IM,a.ru_id,a.province, a.city, a.kf_type, a.kf_ww, a.kf_qq, a.meiqia, a.shop_name, a.kf_appkey,kf_secretkey from {pre}seller_shopinfo as a left join {pre}merchants_shop_information as b on a.ru_id=b.user_id where a.ru_id=\'' . $goods_info['user_id'] . '\' ';
	...
	public function actionInfo()
	{
		...
		$sql = 'SELECT ld.goods_desc FROM {pre}link_desc_goodsid AS dg, {pre}link_goods_desc AS ld WHERE dg.goods_id = ' . $goods_id . '  AND dg.d_id = ld.id';
