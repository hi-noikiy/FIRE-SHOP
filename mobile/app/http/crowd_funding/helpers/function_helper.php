...
{
	$sql = 'SELECT c.cat_id,c.cat_name,c.parent_id,c.is_show ' . "FROM {pre}zc_category\r\n as c " . 'WHERE c.parent_id = 0 AND c.is_show = 1 ORDER BY c.sort_order ASC, c.cat_id ASC';
...
function get_child_trees($tree_id = 0)
{
	...
	if ($GLOBALS['db']->getOne($sql) || ($tree_id == 0)) {
		$child_sql = 'SELECT c.cat_id, c.cat_name, c.parent_id, c.is_show ' . "FROM {pre}zc_category\r\n as c " . ' WHERE c.parent_id = \'' . $tree_id . '\' AND c.is_show = 1 GROUP BY c.cat_id ORDER BY c.sort_order ASC, c.cat_id ASC';
...
{
	$sql = ' SELECT oi.user_id,oi.add_time,u.user_name,u.nick_name,u.user_picture,zg.price,zg.content ' . ' FROM {pre}order_info as oi ' . ' LEFT JOIN {pre}users as u on u.user_id=oi.user_id ' . ' LEFT JOIN {pre}zc_goods as zg on zg.id=oi.zc_goods_id ' . ' LEFT JOIN {pre}zc_project as zd on zd.id=zg.pid ' . ' WHERE oi.is_zc_order=1 AND oi.pay_status=2 AND zd.id = \'' . $goods_id . '\' ' . ' ORDER BY oi.order_id DESC ';
	...
	foreach ($backer_list as $key => $val) {
		...
		if (empty($val['nick_name'])) {
			$backer_list[$key]['user_name'] = $val['user_name'];
		}
		else {
			$backer_list[$key]['user_name'] = $val['nick_name'];
		}

		$backer_list[$key]['user_picture'] = get_zc_image_path($row['user_picture']);
	}
...
function get_topic_list($goods_id = 0)
{
	...
	foreach ($topic_list as $key => $val) {
		$sql = ' select user_name,nick_name,user_picture from {pre}users where user_id=' . $val['user_id'];
		$user_info = $GLOBALS['db']->getRow($sql);

		if (empty($user_info['nick_name'])) {
			$topic_list[$key]['user_name'] = encrypt_username($user_info['user_name']);
		}
		else {
			$topic_list[$key]['user_name'] = encrypt_username($user_info['nick_name']);
		}

		$topic_list[$key]['user_picture'] = get_zc_image_path($user_info['user_picture']);
		...
		if (0 < count($child_topic)) {
			foreach ($child_topic as $k => $v) {
				$sql = ' select user_name,nick_name,user_picture from {pre}users where user_id=' . $v['user_id'];
				$user_info = $GLOBALS['db']->getRow($sql);

				if (empty($user_info['nick_name'])) {
					$child_topic[$k]['user_name'] = encrypt_username($user_info['user_name']);
				}
				else {
					$child_topic[$k]['user_name'] = encrypt_username($user_info['nick_name']);
				}
...
function zc_comment_list($id, $size, $page)
{
	...
	if ($res) {
		foreach ($res as $key => $row) {
			...
			$sql = ' select user_name,nick_name,user_picture from {pre}users where user_id=' . $row['user_id'];
			$user_info = $GLOBALS['db']->getRow($sql);

			if (empty($user_info['nick_name'])) {
				$arr[$row['topic_id']]['username'] = encrypt_username($user_info['user_name']);
			}
			else {
				$arr[$row['topic_id']]['username'] = encrypt_username($user_info['nick_name']);
			}

			$arr[$row['topic_id']]['user_picture'] = get_zc_image_path($user_info['user_picture']);
