...
class Database extends \app\http\base\controllers\Backend
{
	...
	public function actionIndex()
	{
		...
		$obj = \Think\Db::getInstance(config());
		$cache_id = md5('tables' . config('DB_NAME'));
		$tables = cache($cache_id);

		if ($tables === false) {
			$tables = $obj->getTables();
			cache($cache_id, $tables);
		}
