...
class Index extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		...
		if ((C('shop.use_value_card') == '1') && ($flow_type != CART_EXCHANGE_GOODS)) {
		...
		if ((!isset($can_invoice) || ($can_invoice == '1')) && isset($GLOBALS['_CFG']['invoice_content']) && (trim($GLOBALS['_CFG']['invoice_content']) != '')) {
			...
			$invoice_list = dao('users_vat_invoices_info')->where(array('user_id' => $_SESSION[user_id]))->find();
			$invoice_list = (isset($invoice_list['id']) ? $invoice_list['id'] : 0);
	...
	public function actionDone()
	{
		...
		if (is_dir(APP_DRP_PATH)) {
			...
			$sql = 'INSERT INTO ' . $this->ecs->table('order_goods') . '( ' . 'order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, goods_price, commission_rate, ' . 'goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, is_distribution, drp_money) ' . ' SELECT \'' . $new_order_id . '\', ca.goods_id, ca.goods_name, ca.goods_sn, ca.product_id, ca.goods_number, ca.market_price, ca.goods_price, ca.commission_rate, ca.goods_attr, ' . 'ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.model_attr, ca.goods_attr_id, ca.ru_id, ca.shopping_fee, ca.warehouse_id, ca.area_id,' . 'g.is_distribution*\'' . $is_distribution . '\' as is_distribution, ' . 'g.dis_commission*g.is_distribution*ca.goods_price*ca.goods_number/100*\'' . $is_distribution . '\' as drp_money' . ' FROM ' . $this->ecs->table('cart') . ' ca' . ' LEFT JOIN  {pre}goods as g ON ca.goods_id=g.goods_id' . ' WHERE ca.' . $this->sess_id . ' AND ca.rec_type = \'' . $flow_type . '\'' . $goodsIn;
			$this->db->query($sql);
		}
		else {
			$goodsIn = '';
			...
			$sql = 'INSERT INTO ' . $this->ecs->table('order_goods') . '( ' . 'order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, commission_rate, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id) ' . ' SELECT \'' . $new_order_id . '\', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, commission_rate, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id' . ' FROM ' . $this->ecs->table('cart') . ' WHERE ' . $this->sess_id . ' AND rec_type = \'' . $flow_type . '\'' . $goodsIn;
			$this->db->query($sql);
		}
		...
		if ((0 < $order['user_id']) && (0 < $order['surplus'])) {
			log_account_change($order['user_id'], $order['surplus'] * -1, 0, 0, 0, sprintf(L('pay_order'), $order['order_sn']));
		}
	...
	public function actionChangeIntegral()
	{
		...
		$user_points = $user_info['pay_points'];
		$tmp_shipping_id_arr = I('shipping_id');
		if (($user_points < $points) && ($user_points == 0)) {
