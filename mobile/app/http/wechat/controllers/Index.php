...
class Index extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		...
		case 'event':
			...
			else if ($wedata['Event'] == 'unsubscribe') {    
				$this->unsubscribe($wedata['FromUserName']);
				exit();
			}
			else if ($wedata['Event'] == \App\Extensions\Wechatt::EVENT_SCAN) {
				$scene_id = $this->weObj->getRevSceneId();
			}
			else if ($wedata['Event'] == \App\Extensions\Wechatt::EVENT_MENU_CLICK) {
				$keywords = $wedata['EventKey'];
			}
			else if ($wedata['Event'] == \App\Extensions\Wechatt::EVENT_MENU_VIEW) {
				redirect($wedata['EventKey']);
			}
			else if ($wedata['Event'] == \App\Extensions\Wechatt::EVENT_LOCATION) {
				exit();
			}
	...
	public function check_auth()
	{
		...
		$obj = new \App\Extensions\Wechatt($config);
