...
class Profile extends \app\http\base\controllers\Frontend
{
	...
	public function actionUserEditPaypwd()
	{
		if (IS_POST) {
			...
			if ($paypwd_id) {
				$old_pay_paypwd = I('old_pay_paypwd', '', 'trim');
	...
	public function actionRealname()
	{
		if (IS_POST) {
			...
			if (!empty($_FILES['front_of_id_card']['name']) || !empty($_FILES['reverse_of_id_card']['name'])) {
				$result = $this->upload('data/attached/iduser_image', false);
			...
			$row = dao('users_real')->where(array('user_id' => $this->user_id))->find();
		...
		if (IS_POST) {
			...
			if ((0 < $bind_user_id) && !empty($_SESSION['unionid'])) {
				if ($bind_user_id != $this->user_id) {
					$res = dao('connect_user')->data(array('user_id' => $bind_user_id))->where(array('user_id' => $this->user_id))->save();
					...
					$json_result = array('status' => 'n', 'info' => '不能关联已登录账号');
					exit(json_encode($json_result));
				}
			}
			else {
		...
		if (IS_AJAX) {
			...
			if (!empty($relation_user_id)) {
				...
				if (!empty($userinfo)) {
					...
					dao('connect_user')->data($data)->where(array('user_id' => $relation_user_id))->save();
					unset($_SESSION['user_id']);
					unset($_SESSION['user_name']);
