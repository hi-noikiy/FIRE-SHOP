...
class Map extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		if (IS_POST) {
			...
			$url = 'http://apis.map.qq.com/tools/poimarker?type=0&marker=' . $store . '&key=' . config('shop.tengxun_key') . '&referer=ectouch';
	...
	public function actionTest()
	{
		...
		foreach ($seller_shopinfo as $key => $vo) {
			...
			$district = get_region_name($vo['district']);
			$address = $province['region_name'] . $city['region_name'] . $district['region_name'] . $vo['shop_address'];
			$result = \App\Extensions\Http::doGet('http://apis.map.qq.com/ws/geocoder/v1/?key=' . config('shop.tengxun_key') . '&address=' . $address);
