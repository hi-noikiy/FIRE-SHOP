...
class Index extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		...
		if (IS_AJAX) {
			...
			if (($lat == 0) && ($lng == 0)) {
				$sql = 'SELECT * FROM {pre}merchants_shop_information as a LEFT JOIN {pre}seller_shopinfo as b ON a.user_id = b.ru_id  WHERE ' . $condition . ' and a.is_street = 1 and a.merchants_audit = 1 order by a.shop_id ' . $sort . $limit;
			}
	...
	public function actionShopInfo()
	{
		...
		$share_data = array('title' => $data['shop_name'], 'desc' => $data['street_desc'], 'link' => '', 'img' => get_wechat_image_path(ltrim($data['logo_thumb'], '../')));
		$this->assign('share_data', $this->get_wechat_share_content($share_data));
		$this->assign('description', $data['street_desc']);
		$this->assign('page_title', $data['shop_name']);
		$path = ROOT_PATH . '../kefu/';
