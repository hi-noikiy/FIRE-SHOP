...
namespace app\http\presale\controllers;
...
class Index extends \app\http\base\controllers\Frontend
{
	...
	private function get_pre_goods($cat_id, $status = 0, $sort = 'cat_id', $order = 'DESC', $page = 1, $size = 10, $keyword = '')
	{
		...
		$total ? $total : 0;
		$sql = 'SELECT a.*, g.goods_thumb, g.goods_img, g.goods_name, g.shop_price, g.market_price, g.sales_volume FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS a ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON a.goods_id = g.goods_id ' . ' WHERE g.goods_id > 0 ' . $where . ' AND g.is_on_sale = 0 AND a.review_status = 4 ORDER BY ' . $sort . ' ' . $order . ' LIMIT ' . (($page - 1) * $size) . ',  ' . $size;
		$res = $GLOBALS['db']->getAll($sql);
