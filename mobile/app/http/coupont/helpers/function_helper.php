...
function get_coupons_list($num = 2, $page = 1, $status = 0)
{
	...
	if (status == 0) {
		foreach ($tab as &$v) {
			$v['begintime'] = local_date('Y-m-d H:i:s', $v['cou_start_time']);
			$v['endtime'] = local_date('Y-m-d H:i:s', $v['cou_end_time']);
		...
		foreach ($tab as &$vs) {
			$vs['begintime'] = local_date('Y-m-d H:i:s', $v['cou_start_time']);
			$vs['endtime'] = local_date('Y-m-d H:i:s', $v['cou_end_time']);
