...
class View extends \app\http\base\controllers\Frontend
{
	...
	public function actionProduct()
	{
		if (IS_POST) {
			...
			if (0 < $user_id) {
				...
				$goodsnum = dao('goods')->where(array('store_best' => 1, 'is_on_sale' => 1, 'is_alone_sale' => 1, 'is_delete' => 0, 'user_id' => $user_id))->count();
			}
	...
	public function actionStore()
	{
		if (IS_POST) {
			...
			$sql = 'SELECT ms.shop_id,ms.user_id, ms.rz_shopName, ss.logo_thumb, ss.street_thumb ' . ' FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS ms ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' AS ss ' . ' ON ms.user_id = ss.ru_id ' . ' order by ms.sort_order ASC ' . ' limit  0, ' . $number;
	...
	public function actionStoreIn()
	{
		if (IS_POST) {
			...
			foreach ($store as $key => $value) {
				$sql = 'SELECT goods_name, goods_thumb, shop_price ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE user_id = \'' . $value['user_id'] . '\' AND is_on_sale = 1 AND is_delete = 0 and is_alone_sale = 1 AND review_status >2 ';
				$goods = $GLOBALS['db']->getAll($sql);
				$new = dao('goods')->where(array('store_new' => 1, 'user_id' => $value['user_id'], 'review_status' => 3, 'is_on_sale' => 1, 'is_delete' => 0, 'is_alone_sale' => 1))->count();
				$promote = dao('goods')->where(array('is_promote' => 1, 'user_id' => $value['user_id'], 'review_status' => 3, 'is_on_sale' => 1, 'is_delete' => 0, 'is_alone_sale' => 1))->count();
				$store[$key]['total'] = count($goods);
				$store[$key]['new'] = $new;
	...
	public function actionStoreDown()
	{
		if (IS_POST) {
			...
			foreach ($shop as $key => $value) {
				...
				if ($value['kf_qq']) {
					$store[$key]['kf_qq'] = preg_replace('/^[^\\-]*\\|/is', '', $value['kf_qq']);
				}

				if ($value['kf_ww']) {
					$store[$key]['kf_ww'] = preg_replace('/^[^\\-]*\\|/is', '', $value['kf_ww']);
				}
			}
