...
namespace app\http\topic\controllers;
...
class Index extends \app\http\base\controllers\Frontend
{
	...
	{
		if (IS_AJAX) {
			$topic = dao('touch_page_view')->field('id, title , description, thumb_pic')->where(array('type' => 'topic'))->order('update_at DESC')->select();
			$num = (!empty($topic) ? 1 : 0);

			foreach ($topic as $k => $v) {
				$topic[$k]['topic_img'] = get_image_path('data/gallery_album/original_img/' . $v['thumb_pic']);
				$topic[$k]['url'] = '?topic_id=' . $v['id'];
			}

			exit(json_encode(array('list' => $topic, 'totalPage' => $num)));
		}
