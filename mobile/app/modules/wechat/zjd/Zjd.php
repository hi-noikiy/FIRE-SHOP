...
namespace app\modules\wechat\zjd;
...
class Zjd extends \app\http\wechat\controllers\Plugin
{
	...
	public function html_show()
	{
		...
		if (($nowtime < $starttime) || ($endtime < $nowtime)) {
			...
			if (0 < $config['point_status']) {
				$sql = 'SELECT u.pay_points FROM {pre}users as u LEFT JOIN {pre}wechat_user as wu ON u.user_id = wu.ect_uid WHERE wu.openid = \'' . $openid . '\' ';
				$userpay = $GLOBALS['db']->query($sql);
				$num = floor($userpay[0]['pay_points'] / $config['point_value']);
	...
	public function executeAction()
	{
		...
		if (IS_POST) {
			...
			show_message('资料提交成功，请等待发放奖品', '继续抽奖', url('wechat/index/plugin_show', array('name' => $this->plugin_name)));
			exit();
		}

		if (!empty($_GET['id']) && !IS_AJAX) {
			$id = I('get.id');
			$rs = dao('wechat_prize')->field('winner,issue_status')->where(array('openid' => $_SESSION['openid'], 'id' => $id, 'wechat_id' => $this->wechat_id, 'prize_type' => 1, 'activity_type' => $this->plugin_name))->find();
			$winner_result = array();
		...
		if (IS_GET && IS_AJAX) {
			...
			if (!empty($openid)) {
				if ($config['point_status'] == 1) {
					...
					$num = (floor($count_num[0]['pay_points'] / $config['point_value']) < 0 ? 0 : floor($count_num[0]['pay_points'] / $config['point_value']));
				}
				else {
					$sql = 'SELECT count(*) as num FROM {pre}wechat_prize WHERE  wechat_id = \'' . $this->wechat_id . '\' AND openid = \'' . $openid . '\' AND activity_type = \'' . $this->plugin_name . '\' AND dateline > \'' . ($nowtime - $config['point_interval']) . '\'';
					$count_num = $GLOBALS['db']->query($sql);
			...
			if (!empty($prize)) {
				...
				if (($level != $lastarr['prize_level']) && !empty($id)) {
					$rs['link'] = url('wechat/index/plugin_action', array('name' => $this->plugin_name, 'id' => $id));
