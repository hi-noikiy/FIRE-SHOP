...
class Wechat
{
	...
	public function send($to = 0, $title = '')
	{
		...
		if ($list) {
			foreach ($list as $k => $v) {
				...
				$sql = 'UPDATE {pre}wechat_template_log SET status = 1 WHERE code = \'' . $v['code'] . '\' AND openid = \'' . $v['openid'] . '\' AND wechat_id = \'' . $v['wechat_id'] . '\' ';
	...
	private function get_openid($to = 0)
	{
		if (isset($_COOKIE['ectouch_ru_id']) || isset($_SESSION['ectouch_ru_id'])) {
			$openid = (isset($_COOKIE['seller_openid']) && !empty($_COOKIE['seller_openid']) ? $_COOKIE['seller_openid'] : $_SESSION['seller_openid']);
		}
		else if ($to) {
