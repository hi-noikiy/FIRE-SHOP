...
class ShippingRepository implements \App\Contracts\Repository\Shipping\ShippingRepositoryInterface
{
	...
	public function total_shipping_fee($address, $products, $shipping_id)
	{
		...
		$model = \App\Models\Shipping::select('*')->leftJoin('shipping_area', 'shipping_area.shipping_id', '=', 'shipping.shipping_id')->leftJoin('area_region', 'area_region.shipping_area_id', '=', 'shipping_area.shipping_area_id')->where('shipping.enabled', 1)->where('shipping.shipping_id', $shipping_id);
