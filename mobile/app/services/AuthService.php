...
class AuthService
{
	...
	private function wxLogin()
	{
		...
		if (empty($result)) {
			...
			$res = \App\Models\User::where(array('user_name' => $username))->first();
	...
	{
		if (!\App\Models\User::where(array('user_name' => $newUser['username']))->where(array('email' => $newUser['email']))->first()) {
	...
	{
		$token = $_SERVER[strtoupper('HTTP_X_' . _config('app.name', 'ectouch') . '_Authorization')];

		if (empty($token)) {
			return array('error' => 1, 'msg' => strtolower('header parameter `x-' . _config('app.name') . '-authorization` is required'));
		}
