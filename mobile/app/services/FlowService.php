...
class FlowService
{
	...
	public function flowInfo($userId)
	{
		...
		if (empty($defaultAddress['country']) || empty($defaultAddress['province']) || empty($defaultAddress['city'])) {
			$result['default_address'] = '';
		}
		else {
			$defaultAddress['country'] = $this->regionRepository->getRegionName($defaultAddress['country']);
			$defaultAddress['province'] = $this->regionRepository->getRegionName($defaultAddress['province']);
			$defaultAddress['city'] = $this->regionRepository->getRegionName($defaultAddress['city']);
			$defaultAddress['district'] = $this->regionRepository->getRegionName($defaultAddress['district']);
			$result['default_address'] = $defaultAddress;
		}
	...
	private function arrangeCartGoods($userId)
	{
		...
		unset($cartGoodsList);
		return array('total' => $total, 'list' => $list, 'order_total' => $totalAmount);
	}
	...
	public function shippingFee($args)
	{
		...
		if (empty($cart_goods_list)) {
			$result['error'] = 1;
			$result['massage'] = '购物车没有商品';
			return $result;
		}

		foreach ($cart_goods_list as $key => $val) {
			if ((0 < $shippingId) && ($val['ru_id'] == $ruId)) {
				$cart_goods[$key]['tmp_shipping_id'] = $shippingId;
			}
		}

		$shipFee = $this->shippingRepository->total_shipping_fee($address, $cart_goods_list, $shippingId);
		...
		if ($shipFee) {
			...
			$result['error'] = 1;
			$result['massage'] = '该地区不支持配送';
		}

		$result['fee_format'] = $shipFee;
		return $result;
	}
}
