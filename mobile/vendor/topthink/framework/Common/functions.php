...
function T($template='',$layer=''){
    ...
    // 开发模块模板文件
    $tpl = PLAYGROUND_PATH . strtolower($module . $layer . 's/' . $file . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    $tpl = PLAYGROUND_PATH . strtolower($module . $layer . 's/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    // 应用模板文件
    $tpl = APP_PATH . strtolower($module . $layer . 's/' . $file . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    $tpl = APP_PATH . strtolower($module . $layer . 's/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    // 兼容模板文件
    $tpl = dirname($baseUrl) . '/base/' . str_replace($depr, '/', strtolower($file)) . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
    $tpl = dirname($baseUrl) . '/base/' . end(explode('.', strtolower($file))) . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
...
function controller($name,$path=''){
    ...
    if(!C('APP_USE_NAMESPACE')){
        ...
        import(MODULE_NAME.'/'.strtolower($layer . 's').'/'.$class);
    }else{
        $class  =   ( $path ? basename(ADDON_PATH).'\\'.$path : MODULE_NAME ).'\\'.strtolower($layer . 's');
        $array  =   explode('/',$name);
        foreach($array as $name){
            $class  .=   '\\'.parse_name($name, 1);
        }
...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        if (isset($var[C('VAR_CONTROLLER')]) && strtolower($var[C('VAR_CONTROLLER')]) == strtolower(C('DEFAULT_CONTROLLER'))) {
            unset($var[C('VAR_CONTROLLER')]);
        }
        if (isset($var[C('VAR_ACTION')]) && strtolower($var[C('VAR_ACTION')]) == strtolower(C('DEFAULT_ACTION'))) {
            unset($var[C('VAR_ACTION')]);
        }
        $url = __APP__ . '?';
        $var_query = empty($var) ? '' : '&' . http_build_query(array_reverse($var));
        if (!empty($module)) {
            $url .= C('VAR_MODULE') . "={$module}" . $var_query;
        } else {
            $url .= ltrim($var_query, '&');
        }
        if ($urlCase) {
