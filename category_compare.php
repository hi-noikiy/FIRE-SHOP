...
if (!empty($_REQUEST['goods']) && is_array($_REQUEST['goods']) && (1 < count($_REQUEST['goods']))) {
	...
	while ($row = $db->fetchRow($res)) {
		...
		if (0 < $row['user_id']) {
			$sql = ' SELECT b.brand_name ' . ' FROM ' . $GLOBALS['ecs']->table('brand') . ' AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('link_brand') . ' AS lb ON lb.brand_id = b.brand_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_brand') . ' AS msb ON msb.bid = lb.bid AND msb.audit_status = 1 AND msb.is_show = 1 ' . ' WHERE msb.bid = \'' . $row['brand_id'] . '\' ';
			$msb_brand_name = $GLOBALS['db']->getOne($sql);
			$goods_list[$goods_id]['brand_name'] = $msb_brand_name;
		}
