...
else if ($_REQUEST['act'] == 'remove') 
{
	...
	$sql = 'DELETE w,vp FROM ' . $ecs->table('wholesale') . ' AS W ' . ' LEFT JOIN ' . $ecs->table('wholesale_volume_price') . ' AS vp ' . ' ON w.goods_id = vp.goods_id ' . ' WHERE w.act_id = \'' . $id . '\'';
	$res = $db->query($sql);
...
else if ($_REQUEST['act'] == 'batch_add') 
{
	...
	$wholesale = array( 'act_id' => 0, 'goods_id' => 0, 'goods_name' => $_LANG['pls_search_goods'], 'enabled' => '1', 'shipping_fee' => '0.00', 'price_list' => array() );
...
else if ($_REQUEST['act'] == 'batch_add_insert') 
{
	...
	clear_cache_files();
	$links = array( array('href' => 'wholesale.php?act=list', 'text' => $_LANG['back_wholesale_list']), array('href' => 'wholesale.php?act=add', 'text' => $_LANG['continue_add_wholesale']) );
...
else 
{
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) 
	{
		...
		if ($is_add) 
		{
			$wholesale = array( 'act_id' => 0, 'goods_id' => 0, 'goods_name' => $_LANG['pls_search_goods'], 'enabled' => '1', 'price_list' => array(), 'shipping_fee' => '0.00', 'goods_extend' => array('is_delivery' => 0, 'is_return' => 0, 'is_free' => 0), 'start_time' => strtotime('1 day'), 'end_time' => strtotime('3 day') );
		}
		else 
		{
			if (empty($_GET['id'])) 
		...
		if (1 < count($_attribute)) 
		{
			array_pop($_attribute);
		}
		if (empty($wholesale['price_list'])) 
		{
			$wholesale['price_list'] = array( array( 'qp_list' => array( array('quantity' => 0, 'price' => 0) ) ) );
		}
	...
	else 
	{
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) 
		{
			...
			$shipping_fee = (!(empty($_REQUEST['shipping_fee'])) ? intval($_REQUEST['shipping_fee']) : 0);
			...
			$wholesale = array('act_id' => intval($_POST['act_id']), 'goods_id' => $goods_id, 'wholesale_cat_id' => $cat_id, 'goods_name' => $goods_name, 'rank_ids' => isset($_POST['rank_id']) ? join(',', $_POST['rank_id']) : '', 'review_status' => 3, 'is_recommend' => $is_recommend, 'is_promote' => $is_promote, 'freight' => $freight, 'shipping_fee' => $shipping_fee, 'tid' => $tid, 'enabled' => empty($_POST['enabled']) ? 0 : 1, 'price_model' => intval($_POST['price_model']), 'goods_type' => intval($_POST['goods_type']), 'goods_price' => floatval($_POST['goods_price']), 'moq' => intval($_POST['moq']), 'goods_number' => intval($_POST['goods_number']), 'start_time' => $promote_start_date, 'end_time' => $promote_end_date);
			if ($is_add) 
			{
				if ($have_goods) 
				{
			...
			if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) 
			{
				...
				foreach ($goods_attr_list as $attr_id => $attr_value_list ) 
				{
					foreach ($attr_value_list as $attr_value => $info ) 
					{
						if ($info['sign'] == 'insert') 
						{
							$sql = 'INSERT INTO ' . $ecs->table('wholesale_goods_attr') . ' (attr_id, goods_id, attr_value, attr_sort)' . 'VALUES (\'' . $attr_id . '\', \'' . $goods_id . '\', \'' . $attr_value . '\', \'' . $info['attr_sort'] . '\')';
						}
			...
			foreach ($product['product_sn'] as $key => $value ) 
			{
				...
				if ($product['product_id'][$key]) 
				{
					...
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table($table) . ' SET product_number = \'' . $product['product_number'][$key] . '\'' . ' WHERE product_id = \'' . $product['product_id'][$key] . '\'';
					$GLOBALS['db']->query($sql);
				}
				else 
				{
					...
					$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table($table) . ' (goods_id, goods_attr, product_sn, product_number ' . $products_extension_insert_name . ') VALUES ' . ' (\'' . $product['goods_id'] . '\', \'' . $goods_attr . '\', \'' . $value . '\', \'' . $product['product_number'][$key] . '\'' . $products_extension_insert_value . ')';
		...
		{
			if (($_REQUEST['act'] == 'set_attribute_table') || ($_REQUEST['act'] == 'wholesale_attribute_query')) 
			{
				check_authz_json('goods_manage');
				$goods_id = (empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']));
				$goods_type = (empty($_REQUEST['goods_type']) ? 0 : intval($_REQUEST['goods_type']));
					...
					foreach ($attr_arr as $key => $val ) 
					{
						...
						foreach ($val as $k => $v ) 
						{
							...
							if (!($data)) 
							{
								...
								$sql = ' INSERT INTO ' . $GLOBALS['ecs']->table('wholesale_goods_attr') . ' (goods_id, attr_id, attr_value, attr_sort, admin_id) ' . ' VALUES ' . ' (\'' . $goods_id . '\', \'' . $key . '\', \'' . $v . '\', \'' . $attr_sort . '\', \'' . $_SESSION['admin_id'] . '\') ';
								$GLOBALS['db']->query($sql);
			...
			else if ($_REQUEST['act'] == 'changedgoods') 
			{
				...
				switch ($sort_order) 
				{
					case '1': $sort .= ' ORDER BY g.add_time ASC';
					break;
					case '2': $sort .= ' ORDER BY g.add_time DESC';
					break;
					case '3': $sort .= ' ORDER BY g.sort_order ASC';
					break;
					case '4': $sort .= ' ORDER BY g.sort_order DESC';
					break;
					case '5': $sort .= ' ORDER BY g.goods_name ASC';
					break;
					case '6': $sort .= ' ORDER BY g.goods_name DESC';
					break;
				}
				if ($type == 1) 
				{
				...
				foreach ($attr_value_list as $key => $attr_value ) 
				{
					if ($attr_value) 
					{
						...
						else 
						{
							...
							$sql = 'INSERT INTO ' . $ecs->table('wholesale_goods_attr') . ' (attr_id, goods_id, attr_value, attr_sort, admin_id)' . 'VALUES (\'' . $attr_id . '\', \'' . $goods_id . '\', \'' . $attr_value . '\', \'' . $key . '\', \'' . $admin_id . '\')';
						}
						$db->query($sql);
					}
				}
