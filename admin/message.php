...
if ($_REQUEST['act'] == 'list') {
	...
	if ($rec_arr[0] == 0) {
		...
		while ($rows = $db->FetchRow($result)) {
			$sql = 'INSERT INTO ' . $ecs->table('admin_message') . ' (sender_id, receiver_id, sent_time, ' . 'read_time, readed, deleted, title, message) ' . 'VALUES (\'' . $_SESSION['admin_id'] . '\', \'' . $rows['user_id'] . '\', \'' . gmtime() . '\', ' . '0, \'0\', \'0\', \'' . $_POST['title'] . '\', \'' . $_POST['message'] . '\')';
			$db->query($sql);
		}
		...
		foreach ($rec_arr as $key => $id) {
			$sql = 'INSERT INTO ' . $ecs->table('admin_message') . ' (sender_id, receiver_id, ' . 'sent_time, read_time, readed, deleted, title, message) ' . 'VALUES (\'' . $_SESSION['admin_id'] . '\', \'' . $id . '\', \'' . gmtime() . '\', ' . '\'0\', \'0\', \'0\', \'' . $_POST['title'] . '\', \'' . $_POST['message'] . '\')';
			$db->query($sql);
		}
