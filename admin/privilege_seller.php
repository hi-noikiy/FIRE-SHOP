...
if ($_REQUEST['act'] == 'list') {
	...
	$row = $db->getRow($sql);
	$sql = 'INSERT INTO ' . $ecs->table('admin_user') . ' (user_name, email, password, add_time, nav_list,action_list,ru_id,parent_id) ' . 'VALUES (\'' . trim($_POST['user_name']) . '\', \'' . trim($_POST['email']) . '\', \'' . $password . '\', \'' . $add_time . '\', \'' . $row['nav_list'] . '\',\'' . $action_list . '\',\'' . $ru_id . '\',\'' . $_SESSION['admin_id'] . '\')';
	$db->query($sql);
	$new_id = $db->Insert_ID();
