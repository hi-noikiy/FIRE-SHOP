...
if ($_REQUEST['act'] == 'list') {
	...
	if (0 < $parent_id) {
		$cat_list = get_category_store_child_list($parent_id, $level, $adminru['ru_id']);
	}
...
if ($_REQUEST['act'] == 'update') {
	...
	if (($cat['level'] < 2) && (0 < $adminru['ru_id'])) {
		...
		if (0 < $cat['cat_id']) {
			$link[0]['href'] = 'category_store.php?act=edit&cat_id=' . $cat['cat_id'];
		}
		else {
