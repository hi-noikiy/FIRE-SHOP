...
define('WEBSITE', true);
admin_priv('website');
$smarty->assign('menu_select', array('action' => '01_system', 'current' => 'website'));
...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('website_name', $name);
	...
	case 'create':
		...
		$contents = 'new website( "' . $url . '" , "' . join('|', $t) . '" , "' . join('|', $className) . '" , ' . ($show_name ? '"' . join('|', $name) . '"' : 'false') . ' , ' . ($show_title ? '"' . $_LANG['qita'] . '"' : 'false') . ',' . ($show_help ? '"' . join('|', $help) . '"' : 'false') . ' , ' . ($is_open ? 'true' : 'false') . ' ,"{$back_act}", "' . $date . '" )';
		...
		$smarty->display('website_view.dwt');
		exit();
		break;
	...
	if (($_REQUEST['act'] == 'install') || ($_REQUEST['act'] == 'view')) {
		...
		assign_query_info();
		$smarty->display('website_install.dwt');
	}
	else {
		if (($_REQUEST['act'] == 'query_install') || ($_REQUEST['act'] == 'update_website')) {
			$type = getChar('type');
