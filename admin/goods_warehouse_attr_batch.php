...
if ($_REQUEST['act'] == 'add') {
	...
	if ($_FILES['file']['name']) {
		...
		$arr = array();
		$goods_list = array();
		$field_list = array_keys($_LANG['upload_warehouse_attr']);
	...
	if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
		...
		$sql = 'SELECT id FROM ' . $GLOBALS['ecs']->table('warehouse_attr') . ' WHERE goods_id = \'' . $result['list']['goods_id'] . '\' AND warehouse_id = \'' . $result['list']['warehouse_id'] . '\'' . ' AND goods_attr_id = \'' . $result['list']['goods_attr_id'] . '\'';
