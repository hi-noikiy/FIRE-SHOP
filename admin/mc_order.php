			...
			if (0 < count($str_arr)) {
				...
				for ($j = 0; $j < count($array_goods[$i]); $j++) {
					if (!empty($array_goods[$i][$j])) {
						...
						$goods_other = array('order_id' => $order_id, 'goods_id' => $goods_info['goods_id'], 'goods_sn' => $goods_info['goods_sn'], 'goods_name' => $goods_info['goods_name'], 'goods_number' => $goods_number, 'goods_price' => $goods_info['goods_price'], 'market_price' => $goods_info['market_price']);
