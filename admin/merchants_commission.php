...
function merchants_commission_list()
{
	...
	if ($result === false) {
		...
		$sql = "SELECT mis.*, msf.companyName, msf.company_adress, msf.company_contactTel, s.server_id, s.suppliers_desc, s.suppliers_percent  \r\n                FROM " . $GLOBALS['ecs']->table('merchants_shop_information') . ' as mis ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_server') . ' as s on mis.user_id = s.user_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_steps_fields') . ' as msf on s.user_id = msf.user_id ' . ' ' . $where . ' ' . ' group by mis.user_id ' . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
...
function merchants_order_list()
{
	...
	if ($result === false) {
		...
		$filter['record_count'] = $GLOBALS['db']->getOne($sql);
		$filter['page_count'] = 0 < $filter['record_count'] ? ceil($filter['record_count'] / $filter['page_size']) : 1;
		$sql = 'SELECT o.is_frozen,o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete, o.is_settlement,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, o.goods_amount, o.shipping_fee, ' . '(' . order_commission_field('o.') . ') AS total_fee, o.discount, o.coupons, o.integral_money, o.bonus, o.user_id, ' . '(' . order_activity_field_add('o.') . ') AS activity_fee ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $where . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ';
		$sql .= ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ', ' . $filter['page_size'] . ' ';
		set_filter($filter, $sql);
	}
	else {
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if (file_exists(MOBILE_DRP)) {
			...
			if ($commission_info['commission_model']) {
				...
				$row[$i]['effective_amount_price'] = ($total_return_amount * $percent_value) + $brokerage_amount['should_amount'];
			}
			...
			if ($commission_info['commission_model']) {
				...
				$row[$i]['effective_amount_price'] = ($total_return_amount * $percent_value) + $goods_rate['should_amount'];
			}
...
if ($_REQUEST['act'] == 'merchant_download') {
	...
	$db->query($sql);
	$sql = 'UPDATE ' . $ecs->table('seller_bill_order') . ' SET bill_id = 0 WHERE bill_id = \'' . $id . '\'';
	$db->query($sql);
