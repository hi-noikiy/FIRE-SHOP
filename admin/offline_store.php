...
if ($_REQUEST['act'] == 'list') {
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if ($_REQUEST['act'] == 'add') {
			...
			$smarty->assign('districts', get_regions(3, 3));
		}
		else {
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if ($_REQUEST['act'] == 'insert') {
				...
				$ec_salt = rand(1, 9999);
				$time = gmtime();
				$sql = 'INSERT INTO' . $ecs->table('offline_store') . '(`stores_name`,`country`,`province`,`city`,' . '`district`,`stores_address`,`stores_tel`,`stores_opening_hours`,`stores_traffic_line`,`stores_img`,`is_confirm`,`ru_id`,`add_time`)' . ' VALUES (\'' . $stores_name . '\',\'' . $country . '\',\'' . $province . '\',\'' . $city . '\',\'' . $district . '\',\'' . $stores_address . '\'' . ',\'' . $stores_tel . '\',\'' . $stores_opening_hours . '\',\'' . $stores_traffic_line . '\',\'' . $file_url . '\',\'' . $is_confirm . '\',\'' . $adminru['ru_id'] . '\',\'' . $time . '\')';

				if ($db->query($sql) == true) {
					$store_id = $db->insert_id();
					$sql = 'INSERT INTO' . $ecs->table('store_user') . '(`ru_id`,`store_id`,`stores_user`,`stores_pwd`,`ec_salt`,`store_action`,`add_time`,`tel`,`email`) VALUES (\'' . $adminru['ru_id'] . '\',\'' . $store_id . '\',\'' . $stores_user . '\',\'' . md5(md5($stores_pwd) . $ec_salt) . '\',\'' . $ec_salt . '\',\'all\',\'' . $time . '\',\'' . $stores_tel . '\',\'' . $stores_email . '\')';
					$db->query($sql);
				...
				$sql = 'UPDATE' . $ecs->table('offline_store') . ' SET stores_name=\'' . $stores_name . '\',country=\'' . $country . '\'' . ',province=\'' . $province . '\',city=\'' . $city . '\',district=\'' . $district . '\',stores_address=\'' . $stores_address . '\',stores_tel=\'' . $stores_tel . '\',' . 'stores_opening_hours=\'' . $stores_opening_hours . '\',stores_traffic_line=\'' . $stores_traffic_line . '\',stores_img=\'' . $file_url . '\',is_confirm=\'' . $is_confirm . '\' WHERE id = \'' . $id . '\'';
			...
			if (($_REQUEST['batch_handle'] == 'open_batch') || ($_REQUEST['batch_handle'] == 'off_batch')) {
				...
				$sql = 'UPDATE' . $ecs->table('offline_store') . ' SET is_confirm = \'' . $is_confirm . '\' ';
