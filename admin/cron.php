...
if ($_REQUEST['act'] == 'list') {
	...
	if (empty($_POST['step'])) {
		...
		assign_query_info();
		$page_list = array('index' => 0, 'user' => 0, 'pick_out' => 0, 'flow' => 0, 'group_buy' => 0, 'snatch' => 0, 'tag_cloud' => 0, 'category' => 0, 'goods' => 0, 'article_cat' => 0, 'article' => 0, 'brand' => 0, 'search' => 0);
		...
		$sql = 'SELECT COUNT(*) FROM ' . $ecs->table('crons') . ' WHERE  cron_code = \'' . $_POST['cron_code'] . '\'';
		...
		$sql = 'INSERT INTO ' . $ecs->table('crons') . ' (cron_code, cron_name, cron_desc, cron_config, nextime, day, week, hour, minute, run_once, allow_ip, alow_files)' . 'VALUES (\'' . $_POST['cron_code'] . '\', \'' . $_POST['cron_name'] . '\', \'' . $_POST['cron_desc'] . '\', \'' . $cron_config . '\', \'' . $next . '\', \'' . $cron_day . '\', \'' . $cron_week . '\', \'' . $cron_hour . '\', \'' . $cron_minute . '\', \'' . $_POST['cron_run_once'] . '\', \'' . $_POST['allow_ip'] . '\', \'' . $_POST['alow_files'] . '\')';
		$db->query($sql);
	...
	if (empty($_POST['step'])) {
		...
		$cron = array('day' => $cron_day, 'week' => $cron_week, 'm' => $cron_minute, 'hour' => $cron_hour);
		$next = get_next_time($cron);
		$sql = 'UPDATE ' . $ecs->table('crons') . 'SET cron_name = \'' . $_POST['cron_name'] . '\', cron_desc = \'' . $_POST['cron_desc'] . '\', cron_config = \'' . $cron_config . '\', nextime=\'' . $next . '\', day = \'' . $cron_day . '\', week = \'' . $cron_week . '\', hour = \'' . $cron_hour . '\', minute = \'' . $cron_minute . '\', run_once = \'' . $_POST['cron_run_once'] . '\', allow_ip = \'' . $_POST['allow_ip'] . '\', alow_files = \'' . $_POST['alow_files'] . '\'' . 'WHERE cron_id = \'' . $_POST['cron_id'] . '\' LIMIT 1';
		$db->query($sql);
		sys_msg($_LANG['edit_ok'], 0, $links);
	}
}
else if ($_REQUEST['act'] == 'uninstall') {
	$sql = 'DELETE FROM ' . $ecs->table('crons') . 'WHERE cron_code = \'' . $_REQUEST['code'] . '\' LIMIT 1';
	$db->query($sql);
	$links[] = array('text' => $_LANG['back_list'], 'href' => 'cron.php?act=list');
	...
	$sql = 'UPDATE ' . $ecs->table('crons') . 'SET enable = \'' . $val . '\' ' . 'WHERE cron_code = \'' . $id . '\' LIMIT 1';
	$db->query($sql);
	...
	if (file_exists(ROOT_PATH . 'includes/modules/cron/' . $_REQUEST['code'] . '.php')) {
		...
		include_once ROOT_PATH . 'includes/modules/cron/' . $_REQUEST['code'] . '.php';
		$timestamp = gmtime();
		$sql = 'UPDATE ' . $ecs->table('crons') . 'SET thistime = \'' . $timestamp . '\' ' . 'WHERE cron_code = \'' . $_REQUEST['code'] . '\' LIMIT 1';
		$db->query($sql);
	}
