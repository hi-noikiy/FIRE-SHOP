...
function zc_topic_list($conditions = '')
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT zt.*, u.user_name, zp.title ' . ' FROM ' . $GLOBALS['ecs']->table('zc_topic') . ' AS zt ' . $leftjoin . $where . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
