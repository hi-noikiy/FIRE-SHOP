...
if ($_REQUEST['act'] == 'list') {
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (!empty($tpl_id)) {
			...
			$sql = 'DELETE FROM' . $ecs->table('goods_transport_tpl') . 'WHERE id in (' . $tpl_id . ') AND tid=0';
			$db->query($sql);
		}
		...
		if ($_REQUEST['act'] == 'add') {
			...
			if (0 < $tid) {
				$sql = 'SELECT g.tid,g.shipping_id,s.shipping_name,g.region_id FROM ' . $ecs->table('goods_transport_tpl') . ' as g LEFT JOIN ' . $ecs->table('shipping') . ' AS s ON g.shipping_id=s.shipping_id WHERE user_id = \'' . $adminru['ru_id'] . '\' AND tid = \'' . $tid . '\'';
				$shipping_tpl = $db->getAll($sql);
			}
		}
		...
		if ($shipping_tpl) {
			foreach ($shipping_tpl as $k => $v) {
				if ($v['region_id']) {
					$shipping_tpl[$k]['region_list'] = get_area_list($v['region_id']);
				}
			}
		}
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			$data = array();
			$data['tid'] = empty($_REQUEST['tid']) ? NULL : intval($_REQUEST['tid']);
			...
			if (0 < $data['freight_type']) {
				...
				if (!empty($tpl_id)) {
					$tpl_id = implode(',', $tpl_id);
					$sql = 'UPDATE' . $ecs->table('goods_transport_tpl') . ' SET tid = \'' . $tid . '\' WHERE id in (' . $tpl_id . ')';
					$db->query($sql);
				}
			}
			...
			$tid = (empty($_REQUEST['tid']) ? 0 : intval($_REQUEST['tid']));
			$sql = 'SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* ' . ' FROM ' . $ecs->table('goods_transport_tpl') . ' AS b ' . ' left join ' . $ecs->table('shipping') . ' AS a on a.shipping_id=b.shipping_id ' . ' WHERE b.tid=\'' . $tid . '\' and b.shipping_id=\'' . $shipping_id . '\' and b.user_id=\'' . $adminru['ru_id'] . '\'';
			$row = $db->getRow($sql);
			...
			if (($shipping_id == 0) || empty($regionId)) {
				...
				$sql = 'SELECT count(*) FROM ' . $ecs->table('goods_transport_tpl') . ' WHERE shipping_id = \'' . $shipping_id . '\' and user_id = \'' . $adminru['ru_id'] . '\' AND tid = \'' . $tid . '\'';
				$res = $db->getOne($sql);

				if (0 < $res) {
					$db->autoExecute($ecs->table('goods_transport_tpl'), $other, 'UPDATE', ' tid= ' . $tid . ' and user_id = "' . $adminru['ru_id'] . '" and shipping_id= ' . $shipping_id);
				}
				else {
			...
			if ($tpl_id && $_SESSION['tpl_id'] && !empty($_SESSION['tpl_id'])) {
				$_SESSION['tpl_id'] = array_merge($tpl_id, $_SESSION['tpl_id']);
			}
			else {
				$_SESSION['tpl_id'] = $tpl_id;
			}
			...
			$tid = (empty($_REQUEST['tid']) ? 0 : $_REQUEST['tid']);
			$shipping_id = (empty($_REQUEST['shipping_id']) ? 0 : intval($_REQUEST['shipping_id']));
			$sql = 'DELETE FROM' . $ecs->table('goods_transport_tpl') . 'WHERE tid = \'' . $tid . '\' AND shipping_id=\'' . $shipping_id . '\'';
			$db->query($sql);
			exit($json->encode($result));
		}
	}
