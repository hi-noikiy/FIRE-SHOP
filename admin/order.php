...
function operable_list($order)
{
	...
	if (OS_UNCONFIRMED == $os) {
		...
		if ((OS_RETURNED_PART == $os) || (0 < $chargeoff_status)) {
			if ($priv_list['ps'] < 2) {
				$list['pay'] = true;
			}
...
function order_list()
{
	...
	if ($result === false) {
		...
		if (($filter['source'] == 'start') || (0 < $adminru['ru_id']) || ($filter['order_type'] == 2) || $filter['keywords']) {
		...
		if (!empty($filter['keywords'])) {
			...
			$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_action') . ' AS oa ON o.order_id = oa.order_id ' . $where . $where_store . $no_main_order . ' GROUP BY o.order_id';
		...
		$sql = 'SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id,o.shipping_fee,o.insure_fee,o.pay_fee,o.surplus,o.tax,o.integral_money,o.bonus,o.discount,o.coupons, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code, o.extension_id, o.is_zc_order, ' . 'o.pay_name, o.referer, o.froms, o.user_id, o.chargeoff_status, ' . '(' . order_amount_field('o.') . ') AS total_fee, (o.goods_amount - o.discount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee) AS total_fee_order' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS bai ON o.order_id=bai.order_id ' . $leftJoin . $where . $where_store . $no_main_order . $groupBy . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
...
function get_order_goods($order)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		if ($row['extension_code'] == 'package_buy') {
			...
			if ($activity) {
				if ((strpos($activity['activity_thumb'], 'http://') === false) && (strpos($activity['activity_thumb'], 'https://') === false)) {
...
function download_orderlist($result)
{
	...
	$data = i('订单号,' . $data_name . '下单会员,下单时间,收货人,联系电话,地址,总金额,配送费用,保价费用,支付费用,余额金额,积分金额,红包金额,发票税额,折扣金额,优惠券金额,应付金额,确认状态,付款状态,发货状态,订单来源' . "\n");
	$count = count($result);
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$ru_name = (!empty($data_cnt) ? i($result[$i]['shop_name']) . ',' : '');
		...
		$data .= $order_sn . ',' . $ru_name . $order_user . ',' . $order_time . ',' . $consignee . ',' . $tel . ',' . $address . ',' . i($result[$i]['total_fee']) . ',' . $shipping_fee . ',' . $insure_fee . ',' . $pay_fee . ',' . $surplus . ',' . $integral_money . ',' . $bonus . ',' . $tax . ',' . $discount . ',' . $coupons . ',' . $order_amount . ',' . $order_status . ',' . $pay_status . ',' . $shipping_status . ',' . $froms . "\n";
	}
...
if ($_REQUEST['act'] == 'order_query') {
...
else if ($_REQUEST['act'] == 'order_export') {
	...
	$orderInfo = get_table_date('order_info', 'order_id = \'' . $order_id . '\'', $date);
	...
	while ($row = $db->fetchRow($res)) {
		...
		$brand = get_goods_brand_info($row['ru_id'], $row['brand_id']);
	...
	$goods_sql = 'SELECT dg.*, g.goods_thumb  FROM ' . $ecs->table('delivery_goods') . ' AS dg LEFT JOIN ' . $ecs->table('goods') . ' as g ON dg.goods_id = g.goods_id ' . ' WHERE delivery_id = ' . $delivery_order['delivery_id'];
	$goods_list = $GLOBALS['db']->getAll($goods_sql);
...
else if ($_REQUEST['act'] == 'edit_bank_sort_order') {
	...
	if ($db->query($sql)) {
		...
		make_json_error('系统出错，请重试');
	}
}
else if ($_REQUEST['act'] == 'add_return_cause') {
	admin_priv('order_back_cause');
	$smarty->assign('ur_here', '添加退换货原因');
	...
	$order['insure_yn'] = empty($order['insure_fee']) ? 0 : 1;
	$goods_sql = 'SELECT b.*, g.goods_thumb FROM ' . $ecs->table('back_goods') . ' AS b LEFT JOIN ' . $ecs->table('goods') . ' AS g ON g.goods_id = b.goods_id ' . ' WHERE back_id = ' . $back_order['back_id'];
	$goods_list = $GLOBALS['db']->getAll($goods_sql);
	...
	$ret_d = (empty($_REQUEST['ret_d']) ? 0 : intval($_REQUEST['ret_d']));

	if ($type == 1) {
		if (0 < $ret_d) {
			$sql = 'SELECT should_return FROM ' . $GLOBALS['ecs']->table('order_return') . ' WHERE ret_id = \'' . $ret_d . '\' AND return_type IN(1, 3)';
			$should_return = $GLOBALS['db']->getOne($sql);
		}
		else {
		...
		$sql = 'SELECT SUM(return_shipping_fee) FROM ' . $GLOBALS['ecs']->table('order_return') . ' WHERE order_id = \'' . $order_id . '\' AND return_type IN(1, 3) AND ret_id <> \'' . $ret_d . '\'';
		$return_shipping_fee = $GLOBALS['db']->getOne($sql);
		$surplus_fee = $order_shipping_fee - $return_shipping_fee;

		if ($surplus_fee < $refound_amount) {
			$shipping_fee = $surplus_fee;
		}
		else {
	...
	if ('user' == $step) {
		...
		$shipping = shipping_area_info($shipping_id, $region_id_list, $order_info['ru_id']);
		$shipping_name = $shipping['shipping_name'];
		$consignee = array('country' => $order_info['country'], 'province' => $order_info['province'], 'city' => $order_info['city'], 'district' => $order_info['district']);
		$goods_list = order_goods($order_id);

		if ($GLOBALS['_CFG']['freight_model'] == 0) {
			$weight_amount = order_weight_price($order_id);
			$shipping_fee = shipping_fee($shipping['shipping_code'], unserialize($shipping['configure']), $weight_amount['weight'], $weight_amount['amount'], $weight_amount['number']);
		}
		else if ($GLOBALS['_CFG']['freight_model'] == 1) {
			$shipping_fee = get_goods_order_shipping_fee($goods_list, $consignee, $shipping_id);
		...
		$order = array('shipping_id' => $shipping_id, 'shipping_name' => addslashes($shipping_name), 'shipping_fee' => $shipping_fee);
		...
		$order['order_amount'] = (($order['goods_amount'] - $order['discount']) + $order['tax'] + $order['shipping_fee'] + $order['insure_fee'] + $order['pay_fee'] + $order['pack_fee'] + $order['card_fee']) - $order['coupons'] - $order['money_paid'];
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if ('user' == $step) {
			...
			$consignee = array('country' => $order['country'], 'province' => $order['province'], 'city' => $order['city'], 'district' => $order['district'], 'street' => $order['street']);
			$goods_list = order_goods($order_id);
			$total = order_weight_price($order_id);
			...
			foreach ($shipping_list as $key => $shipping) {
				...
				if ($GLOBALS['_CFG']['freight_model'] == 0) {
					...
					$shipping_list[$key]['shipping_fee'] = $shipping_fee;
					$shipping_list[$key]['format_shipping_fee'] = price_format($shipping_fee);
					$shipping_cfg = sc_unserialize_config($shipping['configure']);
					$shipping_list[$key]['free_money'] = price_format($shipping_cfg['free_money']);
					$shipping_list[$key]['freight_model'] = 0;
				}
				else if ($GLOBALS['_CFG']['freight_model'] == 1) {
					$shipping_fee = get_goods_order_shipping_fee($goods_list, $consignee, $shipping['shipping_id']);

					if ($shipping_fee['ru_list'][$order['ru_id']]) {
						$shipping_list[$key]['ru_list'] = array_values($shipping_fee['ru_list'][$order['ru_id']]);
						$shipping_list[$key]['ru_count'] = count($shipping_list[$key]['ru_list']);
					}
		...
		if (isset($_POST['confirm'])) {
			...
			if (!empty($invoice_no)) {
				...
				if ($delivery_id) {
					...
					if (!empty($goods_list)) {
						foreach ($goods_list as $value) {
							if (empty($value['extension_code']) || ($value['extension_code'] == 'virtual_card') || (substr($value['extension_code'], 0, 7) == 'seckill')) {
	...
	array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])
		...
		if ('confirm' == $operation) {
			...
			$is_update_sale = is_update_sale($order_id);
			if (($_CFG['sales_volume_time'] == SALES_PAY) && ($is_update_sale == 1)) {
				get_goods_sale($order_id);
			}
			...
			if ($user_mobile_phone != '') {
				...
				$store_smsParams = array('user_name' => $user_name, 'order_sn' => $order['order_sn'], 'code' => $stores_order['pick_code'], 'store_address' => $store_address, 'mobile_phone' => $user_mobile_phone);
		...
		else if ('unpay' == $operation) {
			admin_priv('order_ps_edit');
			$return_amount = (!empty($_REQUEST['refound_amount']) ? floatval($_REQUEST['refound_amount']) : 0);
			...
			if ($order['shipping_fee'] < $shippingFee) {
			...
			$arr = array('pay_status' => PS_UNPAYED, 'pay_time' => 0, 'money_paid' => $order_amount, 'surplus' => 0, 'shipping_fee' => $shipping_fee, 'order_amount' => $return_amount);
			update_order($order_id, $arr);
			$refund_type = @$_REQUEST['refund'];
			$refund_note = @$_REQUEST['refund_note'];
			order_refund($order, $refund_type, $refund_note, $refund_amount);
			...
			admin_priv('order_ss_edit');
			define('GMTIME_UTC', gmtime());
			$_REQUEST['send_number'] = isset($_REQUEST['send_number']) ? $_REQUEST['send_number'] : array();
			...
			$filter_fileds = array('order_sn', 'add_time', 'user_id', 'how_oos', 'shipping_id', 'shipping_fee', 'consignee', 'address', 'country', 'province', 'city', 'district', 'sign_building', 'email', 'zipcode', 'tel', 'mobile', 'best_time', 'postscript', 'insure_fee', 'agency_id', 'delivery_sn', 'action_user', 'update_time', 'suppliers_id', 'status', 'order_id', 'shipping_name', 'is_zc_order');
			$_delivery = array();
			...
			$return_info = return_order_info_byId($order_id);
			...
			if ($order['pay_status'] != PS_UNPAYED) {
				...
				if (!empty($_REQUEST['action_note'])) {
					...
					order_refound($order, $refund_type, $refund_note, $refund_amount);
					$return_status = array('refound_status' => 1, 'agree_apply' => 1, 'actual_return' => $refund_amount, 'return_shipping_fee' => $shippingFee, 'return_time' => gmtime());
	...
	else if ($_REQUEST['act'] == 'merge_order_list') {
		$store_list = get_common_store_list();
		$smarty->assign('store_list', $store_list);
		...
		if ($id_name != '') {
			$sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id LIKE \'%' . mysql_like_quote($id_name) . '%\'' . ' OR user_name LIKE \'%' . mysql_like_quote($id_name) . '%\'' . ' LIMIT 20';
			$res = $GLOBALS['db']->query($sql);
		...
		$goods_list = array();
		$goods_attr = array();
		$sql = 'SELECT o.*, g.goods_thumb, g.brand_id, g.user_id AS ru_id, g.goods_number AS storage, g.model_inventory, o.goods_attr, oi.order_sn ' . 'FROM ' . $ecs->table('order_goods') . ' AS o ' . 'LEFT JOIN ' . $ecs->table('goods') . ' AS g ON o.goods_id = g.goods_id ' . 'LEFT JOIN ' . $ecs->table('order_info') . ' AS oi ON oi.order_id = o.order_id ' . 'WHERE o.order_id = \'' . $order_id . '\' ';
		$res = $db->query($sql);
		...
		if (0 < count($_REQUEST['checkboxes'])) {
