	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if ($_REQUEST['act'] == 'insert') {
				...
				$sql = 'INSERT INTO ' . $ecs->table('user_account') . ' VALUES (\'\', \'' . $user_id . '\', \'' . $_SESSION['admin_name'] . '\', \'' . $amount . '\', \'' . gmtime() . '\', \'' . gmtime() . '\', \'' . $admin_note . '\', \'' . $user_note . '\', \'' . $process_type . '\', \'' . $payment . '\', \'' . $is_paid . '\')';
				$db->query($sql);
				$id = $db->insert_id();
			}
			else {
			...
			if ($account['is_paid'] == 0) {
				...
				$smsParams = array('user_name' => $user_info['user_name'], 'user_money' => $user_surplus['user_money'], 'op_time' => local_date('Y-m-d H:i:s', gmtime()), 'mobile_phone' => $user_info['mobile_phone'] ? $user_info['mobile_phone'] : '', 'add_time' => local_date('Y-m-d H:i:s', $account['add_time']), 'examine' => '通过', 'fmt_amount' => $amount);
				if (($is_paid == 1) && ($account['process_type'] == 1)) {
					...
					log_account_change($account['user_id'], 0, $amount, 0, 0, '【' . $_LANG['surplus_type_1'] . '-' . $_LANG['offline_transfer'] . '】' . $admin_note, ACT_DRAWING);
			...
			if ($checkboxes) {
				foreach ($checkboxes as $id) {
					...
					if ($account['is_paid'] == 0) {
						...
						$user_info = $db->getRow($sql);
						$smsParams = array('user_name' => $user_info['user_name'], 'user_money' => $user_surplus['user_money'], 'op_time' => local_date('Y-m-d H:i:s', gmtime()), 'mobile_phone' => $user_info['mobile_phone'] ? $user_info['mobile_phone'] : '', 'add_time' => local_date('Y-m-d H:i:s', $account['add_time']), 'examine' => $_LANG['through'], 'fmt_amount' => $amount);
