...
function tag_is_only($name, $tag_id, $goods_id = '')
{
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('tag') . ' WHERE tag_words = \'' . $name . '\'' . ' AND goods_id = \'' . $goods_id . '\' AND tag_id != \'' . $tag_id . '\'';
...
function get_tag_list()
{
	...
	$sql = 'SELECT t.tag_id, u.user_name, t.goods_id, g.goods_name, t.tag_words ' . 'FROM ' . $GLOBALS['ecs']->table('tag') . ' AS t ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON u.user_id=t.user_id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id=t.goods_id ' . 'ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ', ' . $filter['page_size'];
...
{
	$sql = 'SELECT t.tag_id, t.tag_words, t.goods_id, g.goods_name FROM ' . $GLOBALS['ecs']->table('tag') . ' AS t' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON t.goods_id=g.goods_id' . ' WHERE tag_id = \'' . $tag_id . '\'';
	$row = $GLOBALS['db']->getRow($sql);
	return $row;
}
...
if ($_REQUEST['act'] == 'list') {
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if ($is_add) {
			$tag = array('tag_id' => 0, 'tag_words' => '', 'goods_id' => 0, 'goods_name' => $_LANG['pls_select_goods']);
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if ($is_insert) {
				$sql = 'INSERT INTO ' . $ecs->table('tag') . '(tag_id, goods_id, tag_words)' . ' VALUES(\'' . $id . '\', \'' . $goods_id . '\', \'' . $tag_words . '\')';
				$db->query($sql);
			...
			$sort_flag = sort_flag($tag_list['filter']);
			$smarty->assign($sort_flag['tag'], $sort_flag['img']);
			make_json_result($smarty->fetch('tag_manage.dwt'), '', array('filter' => $tag_list['filter'], 'page_count' => $tag_list['page_count']));
		}
		...
		else if ($_REQUEST['act'] == 'edit_tag_name') {
			check_authz_json('tag_manage');
