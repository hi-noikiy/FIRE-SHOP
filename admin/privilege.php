...
function get_admin_userlist($ru_id)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('admin_user') . ' AS au ' . ' WHERE 1 AND parent_id = 0 AND ru_id = 0 ' . $where;
		...
		$sql = 'SELECT user_id, user_name, au.ru_id, email, add_time, last_login, agency_id ' . 'FROM ' . $GLOBALS['ecs']->table('admin_user') . ' AS au ' . 'WHERE 1 AND parent_id = 0 AND ru_id = 0 ' . $where . ' ORDER BY user_id DESC ' . 'LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
