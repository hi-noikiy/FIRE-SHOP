...
if ($_REQUEST['act'] == 'list') {
	...
	if ($exc->num('position_name', $position_name, 0, $where) == 0) {
		$sql = 'INSERT INTO ' . $ecs->table('touch_ad_position') . ' (position_name, ad_width, ad_height, position_desc, position_style, user_id, is_public, theme) ' . 'VALUES (\'' . $position_name . '\', \'' . $ad_width . '\', \'' . $ad_height . '\', \'' . $position_desc . '\', \'' . $_POST['position_style'] . '\', \'' . $adminru['ru_id'] . '\', \'' . $is_public . '\', \'' . $template . '\')';
		$db->query($sql);
	...
	if ($db->getOne($sql) == 0) {
		$sql = 'UPDATE ' . $ecs->table('touch_ad_position') . ' SET ' . 'position_name    = \'' . $position_name . '\', ' . 'ad_width         = \'' . $ad_width . '\', ' . 'ad_height        = \'' . $ad_height . '\', ' . 'position_desc    = \'' . $position_desc . '\', ' . 'is_public        = \'' . $is_public . '\', ' . 'position_style   = \'' . $_POST['position_style'] . '\' ' . 'WHERE position_id = \'' . $position_id . '\'';
