...
if ($_REQUEST['act'] == 'list') {
	...
	if ($start < $count) {
		...
		while ($rt = $db->fetch_array($query)) {
			...
			$smarty->assign(array('user_name' => $rt['user_name'], 'goods_name' => $rt['goods_name'], 'preg_replace' => $preg_replace, 'shop_name' => $_CFG['shop_title'], 'send_date' => local_date($_CFG['time_format'], gmtime())));
