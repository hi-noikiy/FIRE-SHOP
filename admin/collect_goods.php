...
class taobao
{
	...
	public function collect($id,$num=2,$goods_number=0,$cat_id=0,$pic_save=0,$price='N',$category_id=0,$brand_id=0)
	{
		...
		$this->skus = $data['data']['skuProps'];
		$this->apiskus = $data['data']['apiskus'];
	...
	public function setGoodsAttr($goods_id)
	{
		...
		// 插入SKU
		if( $this->skus['skuProps'] ) {

			$ppathId = array();
			foreach($this->skus['skuProps'] as $key=>$val) {
				$ppathId[$val['propId']] = array();
				$propName = $val['propName']; //  [propName] => 颜色分类
				$sql = "select attr_id from " .$GLOBALS['ecs']->table('attribute'). " where attr_name='".$propName."' and cat_id='".$this->cat_id."' and attr_input_type=0  and attr_type=1";
				$attr_id= $GLOBALS['db']->getOne($sql);

				# error 属性名称和 规格名称重复(不重要)
				if( !$attr_id ) {
					$GLOBALS['db']->query("INSERT INTO " .$GLOBALS['ecs']->table('attribute'). " (`cat_id` ,`attr_name`,`attr_input_type` ,`attr_type`  ) VALUES ('".$this->cat_id."', '".$propName."',0,1)");
					$attr_id = $GLOBALS['db']->insert_id();
				}
				# end

				foreach($val['values'] as $k=>$props_attr) {
					if( isset($prop['imgUrl']) ) {
						$skuProps[$val['propId']][$prop['valueId']]['imgUrl'] = $prop['imgUrl'];
					}
					$goods_attr_sql_arr = array('goods_id'=>$goods_id, 'attr_id'=>$attr_id,'attr_value'=>$props_attr['name'], 'attr_price'=>0);
					if( $this->type=='dscmall' and isset($props_attr['imgUrl']) ) {  // 属性图片
						
						$name = $this->download_img($props_attr['imgUrl'],ROOT_PATH.'data/goods_attr_img/');
						$img_url = $this->image->make_thumb(ROOT_PATH."data/goods_attr_img/".$name , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
						$goods_attr_sql_arr['attr_img_flie'] ="data/goods_attr_img/".$name;
						$goods_attr_sql_arr['attr_gallery_flie'] = reformat_image_name('gallery', $goods_id, $img_url, 'goods');
						get_oss_add_file(array($goods_attr_sql_arr['attr_img_flie'],$goods_attr_sql_arr['attr_gallery_flie']));
					}
					$GLOBALS['db']->autoExecute( $GLOBALS['ecs']->table('goods_attr'), $goods_attr_sql_arr);
					$goods_attr_id = $GLOBALS['db']->insert_id();
					$ppathId[$val['propId']][$props_attr['valueId']] = $goods_attr_id ;
				}

			}
			// 插入货品
			foreach($this->skus['ppathIdmap']  as $key=>$val) {
				$add_goods_attr_id = array();
				$props =  explode(';',$key); 
				foreach($props as $k=>$s) {
					$prop =  explode(':',$s);
					$add_goods_attr_id[] = $ppathId[$prop[0]][$prop[1]];
				}
				...
				$products_sql_arr = array('goods_id'=>$goods_id, 'product_sn'=>$goods_id."_".$val,
											'goods_attr'=>$goods_attr_str, 'product_number'=>$this->apiskus[$val]['quantity']);
				if( $this->type=='dscmall' ) {
					$price = str_replace('N',$this->apiskus[$val]['priceUnits'][0]['price'],$this->price);
					$product_price = eval("return ".$price."; ");
					$products_sql_arr['product_price'] =$product_price;
				}	
