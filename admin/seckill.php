...
function get_seckill_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT sec_id, begin_time, acti_title, is_putaway, acti_time ' . ' FROM ' . $GLOBALS['ecs']->table('seckill') . $where . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ', ' . $filter['page_size'];
...
{
	$sql = ' SELECT sec_id, begin_time, acti_title, acti_time, is_putaway FROM ' . $GLOBALS['ecs']->table('seckill') . ' WHERE sec_id = \'' . intval($_REQUEST['sec_id']) . '\' ';
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if ($_REQUEST['act'] == 'insert') {
				...
				$sql = 'UPDATE ' . $ecs->table('seckill') . ' SET ' . ' acti_title       = \'' . $acti_title . '\', ' . ' begin_time       = \'' . $begin_time . '\', ' . ' acti_time        = \'' . $acti_time . '\', ' . ' is_putaway       = \'' . $is_putaway . '\' ' . ' WHERE sec_id     = \'' . $sec_id . '\'';
				$db->query($sql);
				clear_cache_files();
				$link[0]['text'] = $_LANG['back_list'];
				$link[0]['href'] = 'seckill.php?act=list';
...
if ($_REQUEST['act'] == 'add_goods') {
	...
	if ($filter->act_range == FAR_ALL) {
		...
		$sql = 'SELECT c.cat_id AS id, c.cat_name AS name FROM ' . $ecs->table('category') . ' as c ' . ' WHERE c.cat_name LIKE \'%' . mysql_like_quote($filter->keyword) . '%\' LIMIT 50';
		$arr = $db->getAll($sql);
	}
	else if ($filter->act_range == FAR_BRAND) {
		if (0 < $ru_id) {
			$sql = 'SELECT bid AS id, brandName AS name FROM ' . $ecs->table('merchants_shop_brand') . ' WHERE brandName LIKE \'%' . mysql_like_quote($filter->keyword) . '%\' AND user_id = \'' . $ru_id . '\' AND audit_status = 1 LIMIT 50';
			$arr = $db->getAll($sql);
		}
		else {
