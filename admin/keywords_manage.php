...
function get_keywords_list()
{
	...
	while ($rows = $GLOBALS['db']->fetchRow($res)) {
		...
		if (!empty($arr)) {
			$sql_res = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 ' . $keywords;
			$rows['result_count'] = $GLOBALS['db']->getOne($sql_res);
		}
		else {
			$rows['result_count'] = 0;
		}
