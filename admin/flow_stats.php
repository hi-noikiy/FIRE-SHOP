...
if ($_REQUEST['act'] == 'view') {
	...
	if (!$is_multi) {
		...
		$sql = 'SELECT FLOOR((access_time - ' . $start_date . ') / (24 * 3600)) AS sn, access_time, COUNT(*) AS access_count' . ' FROM ' . $ecs->table('stats') . ' WHERE access_time >= \'' . $start_date . '\' AND access_time <= ' . ($end_date + 86400) . ' GROUP BY sn';
		$res = $db->query($sql);
		$key = 0;
		...
		foreach ($start_date_arr as $k => $val) {
			...
			$sql = 'SELECT FLOOR((access_time - ' . $start_date_arr[$k] . ') / (24 * 3600)) AS sn, access_time, COUNT(*) AS access_count' . ' FROM ' . $ecs->table('stats') . ' WHERE access_time >= \'' . $start_date_arr[$k] . '\' AND access_time <= ' . ($end_date_arr[$k] + 86400) . ' GROUP BY sn';
			$res = $db->query($sql);
			$lastDay = 0;
	...
	if (!$is_multi) {
		...
		$sql = 'SELECT COUNT(*) AS access_count, area FROM ' . $ecs->table('stats') . ' WHERE access_time >= \'' . $start_date . '\' AND access_time < ' . ($end_date + 86400) . ' GROUP BY area ORDER BY access_count DESC LIMIT 20';
		$res = $db->query($sql);
		$key = 0;
		...
		$sql = 'SELECT access_time, area FROM ' . $ecs->table('stats') . ' WHERE ' . $where;
		$res = $db->query($sql);
		$area_arr = array();
	...
	if (!$is_multi) {
		...
		$sql = 'SELECT COUNT(*) AS access_count, referer_domain FROM ' . $ecs->table('stats') . ' WHERE access_time >= \'' . $start_date . '\' AND access_time <= ' . ($end_date + 86400) . ' GROUP BY referer_domain ORDER BY access_count DESC LIMIT 20';
		$res = $db->query($sql);
		$key = 0;
		...
		$sql = 'SELECT access_time, referer_domain FROM ' . $ecs->table('stats') . ' WHERE ' . $where;
		$res = $db->query($sql);
		$domain_arr = array();
	...
	if (!$is_multi) {
		$filename = gmdate($_CFG['date_format'], $start_date + ($timezone * 3600)) . '_' . gmdate($_CFG['date_format'], $end_date + ($timezone * 3600));
	...
	$sql = 'SELECT FLOOR((access_time - ' . $start_date . ') / (24 * 3600)) AS sn, access_time, COUNT(*) AS access_count' . ' FROM ' . $GLOBALS['ecs']->table('stats') . ' WHERE access_time >= \'' . $start_date . '\' AND access_time <= ' . ($end_date + 86400) . ' GROUP BY sn';
