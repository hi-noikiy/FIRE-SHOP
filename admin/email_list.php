...
if ($_REQUEST['act'] == 'list') {
	...
	make_json_result($smarty->fetch('email_list.dwt'), '', array('filter' => $emaildb['filter'], 'page_count' => $emaildb['page_count']));
}
	...
	$sql = 'DELETE FROM ' . $ecs->table('email_list') . ' WHERE id ' . db_create_in(join(',', $_POST['checkboxes']));
	$db->query($sql);
...
else if ($_REQUEST['act'] == 'batch_unremove') {
	if (!isset($_POST['checkboxes']) || !is_array($_POST['checkboxes'])) {
		sys_msg($_LANG['no_select_email'], 1);
	}

	$sql = 'UPDATE ' . $ecs->table('email_list') . ' SET stat = 1 WHERE stat <> 1 AND id ' . db_create_in(join(',', $_POST['checkboxes']));
	$db->query($sql);
	$lnk[] = array('text' => $_LANG['back_list'], 'href' => 'email_list.php?act=list');
	sys_msg(sprintf($_LANG['batch_unremove_succeed'], $db->affected_rows()), 0, $lnk);
}
else if ($_REQUEST['act'] == 'batch_exit') {
	if (!isset($_POST['checkboxes']) || !is_array($_POST['checkboxes'])) {
		sys_msg($_LANG['no_select_email'], 1);
	}

	$sql = 'UPDATE ' . $ecs->table('email_list') . ' SET stat = 2 WHERE stat <> 2 AND id ' . db_create_in(join(',', $_POST['checkboxes']));
	$db->query($sql);
	$lnk[] = array('text' => $_LANG['back_list'], 'href' => 'email_list.php?act=list');
