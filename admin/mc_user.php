...
function mc_reg_user($str = '', $password = 'admin123')
{
	...
	for ($i = 0; $i < count($str_arr); $i++) {
		if (!empty($str_arr[$i])) {
			...
			$other = array('user_name' => str_iconv($str_arr[$i][0]), 'password' => $password, 'email' => $str_arr[$i][1], 'msn' => $str_arr[$i][2], 'qq' => $str_arr[$i][3], 'office_phone' => $str_arr[$i][4], 'home_phone' => $str_arr[$i][5], 'mobile_phone' => $str_arr[$i][6]);
