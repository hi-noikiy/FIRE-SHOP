...
if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
	...
	$theme_extension = 0;
	if (isset($GLOBALS['_CFG']['template']) && ($GLOBALS['_CFG']['template'] == EC_TEMPLATE)) {
	...
	$create_html = create_html($content_html, $adminru['ru_id'], $pc_html_name, $suffix, 1);
	$create = create_html($content, $adminru['ru_id'], $pc_page_name, $suffix, 1);
	$result['error'] = 0;
				...
				if (move_upload_file($file['tmp_name'], $file_name)) {
					...
					if ($bgtype) {
						...
						if (0 < $templates_left['id']) {
							...
							$sql = 'UPDATE' . $ecs->table('templates_left') . ' SET img_file = \'' . $content_file . '\' WHERE ru_id = \'' . $tem_RuId . '\' AND seller_templates = \'' . $suffix . '\' AND id=\'' . $templates_left['id'] . '\' AND type = \'' . $bgtype . '\' AND theme = \'' . $theme . '\'';
							$db->query($sql);
						}
						else {
							$sql = 'INSERT INTO' . $ecs->table('templates_left') . ' (`ru_id`,`seller_templates`,`img_file`,`type`,`theme`) VALUES (\'' . $tem_RuId . '\',\'' . $suffix . '\',\'' . $content_file . '\',\'' . $bgtype . '\',\'' . $theme . '\')';
							$db->query($sql);
						}
					}
