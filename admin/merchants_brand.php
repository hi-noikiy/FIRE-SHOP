...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('action_link', array('text' => $_LANG['06_goods_brand_list'], 'href' => 'merchants_brand.php?act=list'));
	...
	$is_only = $exc->is_only('brandName', $brand_name, 0, 'user_id = \'' . $adminru['ru_id'] . '\'');
	...
	$sql = 'INSERT INTO ' . $ecs->table('merchants_shop_brand') . '(user_id, brandName, bank_name_letter, site_url, brand_desc, brandLogo, is_show, sort_order, major_business) ' . 'VALUES (' . $adminru['ru_id'] . ', \'' . $brand_name . '\', \'' . $brand_letter . '\', \'' . $site_url . '\', \'' . $_POST['brand_desc'] . '\', \'' . $img_name . '\', \'' . $is_show . '\', \'' . $_POST['sort_order'] . '\', 0)';
	$db->query($sql);

	if (0 < $linkBrand) {
		$link_brand = array('bid' => $db->insert_id(), 'brand_id' => $linkBrand);
	...
	admin_log($brand_name, 'add', 'merchants_shop_brand');
	clear_cache_files();
	...
	admin_priv('merchants_brand');
	$sql = 'SELECT bid as brand_id, brandName as brand_name, bank_name_letter, site_url, brandLogo as brand_logo, brand_desc, is_show, sort_order, audit_status, major_business ' . 'FROM ' . $ecs->table('merchants_shop_brand') . ' WHERE bid = \'' . $_REQUEST['id'] . '\'';
	$brand = $db->GetRow($sql);
	...
	if ($brand_name != $_POST['old_brandname']) {
		$is_only = $exc->is_only('brandName', $brand_name, $_POST['id'], '', 'merchants_shop_brand', 'bid');
	...
	if (isset($_POST['audit_status']) && ($adminru['ru_id'] == 0)) {
		...
		if ($_POST['audit_status'] == 1) {
			if (empty($_POST['link_brand'])) {
				...
				$sql = 'SELECT brand_id FROM ' . $GLOBALS['ecs']->table('brand') . ' WHERE brand_id = \'' . $_POST['link_brand'] . '\'';
			}
			...
			if (0 < $linkBrand) {
				$sql = 'SELECT id FROM ' . $GLOBALS['ecs']->table('link_brand') . ' WHERE bid= \'' . $_POST['id'] . '\'';
				$lid = $GLOBALS['db']->getOne($sql);
				$link_brand = array('bid' => $_POST['id'], 'brand_id' => $linkBrand);

				if ($lid) {
					$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('link_brand'), $link_brand, 'UPDTAE', 'bid=' . $_POST['id']);
				}
				else {
					$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('link_brand'), $link_brand, 'INSERT');
				}
	...
	if ($exc->edit($param, $_POST['id'], 'merchants_shop_brand', 'bid')) {
		clear_cache_files();
	...
	$GLOBALS['db']->query($sql);
	$sql = 'UPDATE ' . $ecs->table('goods') . ' SET brand_id=0 WHERE brand_id=\'' . $id . '\' and user_id = \'' . $ru_id . '\'';
	$db->query($sql);
