...
function get_orderinfo($start_date, $end_date)
{
	...
	$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE 1 ' . order_query_sql('finished', 'o.') . ' AND o.add_time >= \'' . $start_date . '\' AND o.add_time < \'' . ($end_date + 86400) . '\'' . ' AND (SELECT og.ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' as og' . ' WHERE og.order_id = o.order_id limit 0, 1) = \'' . $adminru['ru_id'] . '\' ' . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
...
function get_order_general($type = 0)
{
	...
	$sql = 'SELECT count(*) as total_order_num, ' . $total_fee . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o' . ' WHERE 1 ' . order_query_sql('finished', 'o.') . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
...
if ($_REQUEST['act'] == 'list') {
	...
	if ($is_multi) {
		...
		foreach ($order_area as $val) {
			$order_area_xml .= '<set value=\'' . $val['area_num'] . '\' name=\'' . $val['region_name'] . '\' color=\'' . $color . '\' />';
		}
