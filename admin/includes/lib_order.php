...
{
	$sql = 'SELECT og.rec_id, og.goods_id, og.ru_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, og.warehouse_id, ' . 'og.goods_price, og.goods_attr, og.is_real, og.parent_id, og.is_gift, ' . 'og.goods_price * og.goods_number AS subtotal, og.extension_code,g.shop_price, g.is_shipping, g.goods_weight AS goodsWeight, g.goods_img, g.goods_thumb, ' . 'g.goods_cause FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON og.goods_id = g.goods_id ' . ' WHERE og.order_id = \'' . $order_id . '\'';
	$res = $GLOBALS['db']->query($sql);
...
function get_order_shipping_fee($cart_goods, $consignee = '', $cart_value = '', $shipping_list = '')
{
	$shipping_fee = 0;
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_id = 0, $area_id = 0, $stages_qishu = '-1', $store_id = 0, $take_time = '', $store_mobile = '')
{
	...
	if (0 < $num) {
		$sql = 'SELECT goods_number,stages_qishu,rec_id FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' AND goods_id = \'' . $goods_id . '\' ' . ' AND parent_id = 0 AND goods_attr = \'' . $goods_attr . '\' ' . ' AND extension_code <> \'package_buy\' ' . ' AND rec_type = \'CART_GENERAL_GOODS\' AND group_id=\'\' AND warehouse_id = \'' . $warehouse_id . '\' AND store_id = \'' . $store_id . '\'';
		$row = $GLOBALS['db']->getRow($sql);
		...
		if ($row) {
			...
			if (($GLOBALS['_CFG']['use_storage'] == 0) || ($num <= $goods_storage)) {
				$goods_price = get_final_price($goods_id, $num, true, $spec, $warehouse_id, $area_id, 0, 0, $add_tocart);
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' SET goods_number = \'' . $num . '\', stages_qishu = \'' . $stages_qishu . '\'' . ' , goods_price = \'' . $goods_price . '\'' . ' , area_id = \'' . $area_id . '\'' . ' , freight = \'' . $goods['freight'] . '\'' . ' , tid = \'' . $goods['tid'] . '\'' . ' WHERE ' . $sess_id . ' AND goods_id = \'' . $goods_id . '\' ' . ' AND parent_id = 0 AND goods_attr = \'' . $goods_attr . '\' ' . ' AND extension_code <> \'package_buy\' ' . ' AND warehouse_id = \'' . $warehouse_id . '\' ' . 'AND rec_type = \'CART_GENERAL_GOODS\' AND group_id = 0';
...
function user_bonus($user_id, $goods_amount = 0, $cart_value = 0, $seller_amount = array())
{
	$where = '';
...
function get_order_user_flow_bonus($today, $goods_amount, $user_id, $where, $ru_id = 0)
{
	if ($ru_id) {
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
	...
	$amount = (0 < $refund_amount ? $refund_amount : $order['money_paid'] + $order['surplus']);
	...
	if (($refund_type == 1) || ($refund_type == 2)) {
		$return_amount = $order['total_fee'] - $order['shipping_fee'];
		$return_shippingfee = $order['shipping_fee'];
		$sql = 'UPDATE ' . $GLOBALS['ecs']->table('seller_bill_order') . ' SET return_amount = return_amount + \'' . $return_amount . '\', ' . 'order_status = ' . $order['order_status'] . ', pay_status = ' . $order['pay_status'] . ', shipping_status = ' . $order['shipping_status'] . 'return_shippingfee = return_shippingfee + \'' . $return_shippingfee . '\' ' . 'WHERE order_id = \'' . $order['order_id'] . '\'';
		$GLOBALS['db']->query($sql);
	}
...
function get_cart_ru_goods_list($goods_list, $cart_value = '', $consignee = '', $store_id = 0)
{
	...
	foreach ($goods_list as $key => $row) {
		...
		if (defined('THEME_EXTENSION')) {
			...
			$arr[$key]['is_IM'] = $shop_information['is_IM'];
...
function integral_to_give($order)
{
	if ($order['extension_code'] == 'group_buy') {
		...
		$sql = 'SELECT SUM(og.goods_number * IF(IF(g.model_price < 1, g.give_integral, IF(g.model_price < 2, wg.give_integral, wag.give_integral)) > -1, IF(g.model_price < 1, g.give_integral, IF(g.model_price < 2, wg.give_integral, wag.give_integral)), og.goods_price)) AS custom_points,' . ' SUM(og.goods_number * IF(IF(g.model_price < 1, g.rank_integral, IF(g.model_price < 2, wg.rank_integral, wag.rank_integral)) > -1, IF(g.model_price < 1, g.rank_integral, IF(g.model_price < 2, wg.rank_integral, wag.rank_integral)), og.goods_price)) AS rank_points ' . 'FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON og.goods_id = g.goods_id ' . $leftJoin . 'WHERE og.order_id = \'' . $order['order_id'] . '\' ' . 'AND og.goods_id > 0 ' . 'AND og.parent_id = 0 ' . 'AND og.is_gift = 0 AND og.extension_code != \'package_buy\'';
		return $GLOBALS['db']->getRow($sql);
	}
}
...
function return_order_info($ret_id = 0, $order_sn = '', $order_id = 0)
{
	...
	if ($order) {
		...
		$order['formated_return_amount'] = price_format($order['should_return'] + $order['return_shipping_fee'], false);
		...
		$order_shop_info = get_order_shop_info($order['goods_id']);
		$ru_id = $order_shop_info['ru_id'];
		$order['shop_name'] = $order_shop_info['shop_name'];
		$order['shop_url'] = $order_shop_info['shop_url'] ? $order_shop_info['shop_url'] : 'index.php';
...
{
	$ret_id = intval($ret_id);
	$sql = 'SELECT rg.*, g.goods_thumb FROM ' . $GLOBALS['ecs']->table('return_goods') . ' as rg  LEFT JOIN ' . $GLOBALS['ecs']->table('order_return') . 'as r ON rg.rec_id = r.rec_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = rg.goods_id ' . ' WHERE r.ret_id = ' . $ret_id;
	$res = $GLOBALS['db']->query($sql);
...
function get_return_refound($order_id, $rec_id, $num)
{
	...
	if (1 < $count_goods) {
		$return_ratio = $aount_price / $goods_price;
		$return_price = ($orders['money_paid'] + $orders['surplus']) * $return_ratio;
	}
	else {
		$return_price = round(($orders['money_paid'] + $orders['surplus']) - $orders['shipping_fee'], 2);
	}
...
{
	if (defined('THEME_EXTENSION')) {
		$sql = 'SELECT g.goods_thumb, g.goods_name, o.ret_id , o.rec_id, o.goods_id , o.order_sn ,o.order_id , o.apply_time , o.should_return, o.return_status , o.refound_status, o.return_type, o.return_sn ' . ' FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS o LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON o.goods_id = g.goods_id ' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' order by ret_id DESC';
	}
	else {
		$sql = 'SELECT ret_id , rec_id, goods_id , order_sn ,order_id , apply_time , should_return, return_status , refound_status, return_type, return_sn ' . ' FROM ' . $GLOBALS['ecs']->table('order_return') . ' WHERE user_id = \'' . $_SESSION['user_id'] . '\' order by ret_id DESC';
	}
...
{
	if (0 <= $user_id) {
		$u_id = ' AND IF(userFav_type = 0, user_id = \'' . $user_id . '\', 1 = 1)';
	}
	...
	$user_rank = ',' . $user_rank . ',';
	$now = gmtime();
	$sql = 'SELECT act_range_ext, userFav_type, act_range ' . 'FROM ' . $GLOBALS['ecs']->table('favourable_activity') . ' WHERE CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'' . ' AND review_status = 3 AND start_time <= \'' . $now . '\' AND end_time >= \'' . $now . '\' ' . $u_id . $a_range . ' ORDER BY sort_order';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function get_seller_settlement_amount($order_id, $ru_id)
{
	...
	$total_fee = 'SUM((' . order_commission_field('o.') . ')) AS total_fee ';
	...
	if ($order_info) {
		...
		if (file_exists(MOBILE_DRP)) {
			$brokerage_amount = get_order_drp_money($total_fee, $ru_id, $order_id);
		...
		if ($commission_info['commission_model']) {
			...
			$total_fee = ($order_goods_commission * ($order_info['total_fee'] - $return_amount)) / $order_info['goods_amount'];
			$total_fee = $total_fee + $order_info['shipping_fee'];
		}
		else {
			$total_fee = (($order_info['total_fee'] - $return_amount) * $percent_value) + $order_info['shipping_fee'];
		}
...
function get_order_shop_info($goods_id = 0)
{
