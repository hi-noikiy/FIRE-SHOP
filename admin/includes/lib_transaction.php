...
function get_user_orders($user_id, $record_count, $page, $is_delete = 0, $where = '', $order = '', $handle_tyoe = 0, $pagesize = 10)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res)) {
		...
		if ($row['order_status'] == OS_UNCONFIRMED) {
			...
			if (($row['order_status'] == OS_SPLITED) || ($row['order_status'] == OS_CONFIRMED)) {
		...
		if (($row['order_status'] == OS_SPLITED) && ($row['shipping_status'] == SS_RECEIVED) && ($row['pay_status'] == PS_PAYED)) {
			$row['return_url'] = 'user.php?act=goods_order&order_id=' . $order_id;
		}
		else {
			if (($row['order_status'] == OS_CONFIRMED) && ($row['shipping_status'] == SS_RECEIVED) && ($row['pay_status'] == PS_PAYED)) {
				$row['return_url'] = 'user.php?act=goods_order&order_id=' . $order_id;
			}
		}
...
function get_order_detail($order_id, $user_id = 0)
{
	...
	$order_shop_info = order_shop_info($order_id);
...
function invoice_list($user_id = 0, $record_count, $page, $pagesize = 10)
{
	...
	if ($res) {
		foreach ($res as $k => $val) {
			...
			$arr[$k]['invoice_type'] = $arr[$k]['invoice_type'] ? '增值税发票' : '普通发票';
			if (($val['inv_content'] != '不开发票') && $val['inv_content']) {
				$arr[$k]['inv_status'] = '已开';
			}
			else if ($val['inv_content'] == '') {
				$arr[$k]['inv_status'] = '无法开票';
				$arr[$k]['invoice_type'] = '';
			}
			else {
				$arr[$k]['inv_status'] = $val['inv_content'];
				$arr[$k]['invoice_type'] = '';
			}
