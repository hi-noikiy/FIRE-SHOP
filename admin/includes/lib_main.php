...
function return_order_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid,' . '(SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE og.order_id = o.order_id LIMIT 1) AS ru_id, ' . 'o.pay_status, o.consignee, o.email, o.tel, o.extension_code, o.extension_id,' . 'r.ret_id ,r.rec_id, r.address , r.back , r.exchange ,r.attr_val , r.cause_id , r.apply_time , r.should_return , r.actual_return , r.remark , r.address , o.sign_time ,r.return_status , r.refound_status , ' . ' r.return_type, r.addressee, r.phone, r.return_sn, r.return_shipping_fee, ' . '(' . order_amount_field('o.') . ') AS total_fee, ' . 'IFNULL(u.user_name, \'' . $GLOBALS['_LANG']['anonymous'] . '\') AS buyer ' . 'FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS r ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON r.order_id = o.order_id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON u.user_id=o.user_id  ' . $where . $where_store . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
	...
	if ($result === false) {
		...
		if ($is_ajax == 1) {
			if ($filter['order_id_list']) {
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET `order_status` = \'' . OS_SPLITED . '\', `shipping_status` = \'' . SS_RECEIVED . '\',`pay_status` = \'' . PS_PAYED . '\' ' . ' WHERE shipping_status = 1 AND order_id ' . db_create_in($filter['order_id_list']);
				$GLOBALS['db']->query($sql);
			}
...
function get_goods_brand_info($ru_id = 0, $brand_id = 0, $goods_id = 0)
{
	...
	$sql = 'SELECT b.brand_id, b.brand_name FROM ' . $GLOBALS['ecs']->table('brand') . ' AS b' . $leftJoin . ' WHERE ' . $where . ' LIMIT 1';
...
function get_cat_level($parent_id = 0, $level = 0, $table = 'category', $goods_table = 'goods')
{
	...
	$sql = 'SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order ' . $select . ' ' . ' FROM ' . $GLOBALS['ecs']->table($table) . ' AS c WHERE c.parent_id = \'' . $parent_id . '\' ' . ' order by c.sort_order, c.cat_id';
