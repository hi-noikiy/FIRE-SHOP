...
function get_comment_list($ru_id)
{
	...
	$arr = array();
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') . ' WHERE ' . $where . ' ' . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . $filter['start'] . ', ' . $filter['page_size'];
...
if ($_REQUEST['act'] == 'reply') {
	...
	$sql = 'SELECT * FROM ' . $ecs->table('comment') . ' WHERE parent_id = \'' . $comment_id . '\' AND single_id = 0 AND dis_id = 0 and user_id = 0';
...
if ($_REQUEST['act'] == 'action') {
	...
	$comment_type = (isset($_REQUEST['comment_type']) && !empty($_REQUEST['comment_type']) ? intval($_REQUEST['comment_type']) : 0);
	...
	$sql = 'SELECT comment_id,content,parent_id,ru_id FROM ' . $ecs->table('comment') . ' WHERE parent_id = \'' . $comment_info['comment_id'] . '\' AND single_id = 0 AND dis_id = 0 and ru_id=\'' . $comment_info['ru_id'] . '\'';
	$reply_info = $db->getRow($sql);
	if (!empty($reply_info['content']) && ($adminru['ru_id'] == $comment_info['ru_id'])) {
		...
		$sql = 'INSERT INTO ' . $ecs->table('comment') . ' (comment_type, id_value, email, user_name , ' . 'content, add_time, ip_address, status, parent_id,ru_id) ' . 'VALUES(\'' . $comment_type . '\', \'' . $id_value . '\',\'' . $email . '\', ' . '\'' . $_SESSION['admin_name'] . '\',\'' . $content . '\',\'' . gmtime() . '\', ' . '\'' . $ip . '\', \'0\', \'' . $comment_id . '\',\'' . $adminru['ru_id'] . '\')';
	}
	else {
	...
	if (!empty($send_email_notice) || isset($remail)) {
		$sql = 'SELECT user_name, email, content ' . 'FROM ' . $ecs->table('comment') . ' WHERE comment_id =\'' . $comment_id . '\'';
		$comment_info = $db->getRow($sql);
