...
if ($_REQUEST['act'] == '') {
	...
	$sql = ' SELECT IFNULL(SUM(' . order_commission_field('oi.') . '),0) money_total FROM ' . $GLOBALS['ecs']->table('order_info') . 'oi ' . ' WHERE oi.add_time BETWEEN ' . $today_start . ' AND ' . $today_end . '  AND oi.supplier_id=0 AND oi.pay_status = 2 ' . order_query_sql('queren', 'oi.') . $where_date . $where_og;
	...
	if ($adminru['ru_id'] == 0) {
		$update_arr = array('sms_shop_mobile' => $mobile, 'service_email' => $seller_email, 'qq' => $kf_qq, 'ww' => $kf_ww, 'shop_title' => $shop_title, 'shop_keywords' => $shop_keyword, 'shop_country' => $shop_country, 'shop_province' => $shop_province, 'shop_city' => $shop_city, 'shop_address' => $shop_address, 'service_phone' => $kf_tel, 'shop_notice' => $notice);
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('users_real') . 'AS ur ' . ' JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON ur.user_id = u.user_id AND review_status = 0 AND user_type = 0';
	$arr['user_account'] = $db->getOne($sql);
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE order_status ' . db_create_in(array(OS_CONFIRMED)) . '  AND shipping_status = \'' . SS_UNSHIPPED . '\' AND pay_status ' . db_create_in(array(PS_PAYED));
	$arr['no_paid'] = $db->getOne($sql);
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_return') . ' WHERE is_check = \'0\' AND agree_apply = \'0\' AND return_status = \'0\' AND refound_status = \'0\' AND return_type NOT IN(\'0\')';
	$arr['no_change'] = $db->getOne($sql);
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('seller_account_log') . ' AS sal ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS mis ON sal.ru_id = mis.user_id ' . ' WHERE sal.log_type IN(1,4) AND sal.is_paid = 0 ';
	...
	if (!file_exists($dir)) {
		make_dir($dirclear_cache, 511, true);
	}
