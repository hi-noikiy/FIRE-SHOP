...
function steps_users_list()
{
	...
	if ($result === false) {
		...
		if ($filter['check'] == 1) {
			$ex_where .= ' AND merchants_audit = \'0\'';
		}
		else if ($filter['check'] == 2) {
			$ex_where .= ' AND merchants_audit = \'1\'';
		}
		else if ($filter['check'] == 3) {
			$ex_where .= ' AND merchants_audit = \'2\'';
		}
...
if (($_REQUEST['act'] == 'add_shop') || ($_REQUEST['act'] == 'edit_shop')) {
	...
	$sql = 'select steps_audit, merchants_audit, merchants_message, review_goods, self_run from ' . $ecs->table('merchants_shop_information') . ' where user_id = \'' . $user_id . '\'';
	$merchants = $db->getRow($sql);
	$smarty->assign('merchants', $merchants);
	$sql = 'SELECT status FROM ' . $ecs->table('seller_shopinfo') . ' WHERE ru_id = \'' . $user_id . '\'';
	...
	if (($_REQUEST['act'] == 'insert_shop') || ($_REQUEST['act'] == 'update_shop')) {
		...
		if ($_REQUEST['act'] == 'update_shop') {
			if ($merchants_audit != 1) {
				...
				$where = ' AND (SELECT COUNT(*) FROM ' . $ecs->table('presale_activity') . ' AS pa WHERE g.goods_id = pa.goods_id LIMIT 1) = 0 ';
				$db->query('UPDATE ' . $ecs->table('goods') . ' AS g SET g.is_on_sale = 1 WHERE g.user_id = \'' . $user_id . '\' ' . $where);
			}
		...
		$allow_number = $GLOBALS['db']->getOne($sql, true);
		...
		$seller_shopinfo = array('shopname_audit' => $shopname_audit, 'status' => $shop_close);
