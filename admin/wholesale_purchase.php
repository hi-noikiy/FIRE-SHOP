...
else if ($_REQUEST['act'] == 'query') 
{
	...
	$sort_flag = sort_flag($purchase_list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('purchase_list.dwt'), '', array('filter' => $purchase_list['filter'], 'page_count' => $purchase_list['page_count']));
}
else if ($_REQUEST['act'] == 'batch') 
{
	...
	else 
	{
		...
		if (isset($_POST['drop'])) 
		{
			$sql = 'DELETE w FROM ' . $ecs->table('wholesale_purchase') . ' AS W ' . ' WHERE purchase_id ' . db_create_in($ids);
			$db->query($sql);
...
function purchase_list() 
{
	...
	$arr = array();
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('wholesale_purchase') . $where . 'ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
