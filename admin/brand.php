...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('is_need', $_CFG['template'] == 'fire_shop_theme' ? 1 : 0);
	assign_query_info();
	...
	$sql = 'INSERT INTO ' . $ecs->table('brand') . '(brand_name, brand_letter, brand_first_char, site_url, brand_desc, brand_logo, is_show, sort_order) ' . 'VALUES (\'' . $_POST['brand_name'] . '\', \'' . $_POST['brand_letter'] . '\', \'' . strtoupper($_POST['brand_first_char']) . '\', \'' . $site_url . '\', \'' . $_POST['brand_desc'] . '\', \'' . $img_name . '\', \'' . $is_show . '\', \'' . $_POST['sort_order'] . '\')';
	$db->query($sql);
	...
	admin_priv('brand_manage');
	$sql = 'SELECT b.brand_id, b.brand_name, b.brand_letter, b.brand_first_char, b.site_url, b.brand_logo, b.index_img, b.brand_desc, b.brand_logo, b.is_show, b.sort_order,be.is_recommend ' . 'FROM ' . $ecs->table('brand') . ' as b left join ' . $ecs->table('brand_extend') . ' as be on b.brand_id=be.brand_id WHERE b.brand_id=\'' . $_REQUEST['id'] . '\'';
	$brand = $db->GetRow($sql);
	$brand['brand_logo'] = empty($brand['brand_logo']) ? '' : '../' . DATA_DIR . '/brandlogo/' . $brand['brand_logo'];
	$brand['index_img'] = empty($brand['index_img']) ? '' : '../' . DATA_DIR . '/indeximg/' . $brand['index_img'];
