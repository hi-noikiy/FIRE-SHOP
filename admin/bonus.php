...
if ($_REQUEST['act'] == 'update') {
	...
	$record = array('type_name' => $type_name, 'type_money' => $_POST[type_money], 'send_start_date' => $send_startdate, 'send_end_date' => $send_enddate, 'use_start_date' => $use_startdate, 'use_end_date' => $use_enddate, 'send_type' => $_POST[send_type], 'usebonus_type' => $usebonus_type, 'min_amount' => $min_amount, 'max_amount' => $max_amount, 'min_goods_amount' => floatval($_POST['min_goods_amount']));
...
if ($_REQUEST['act'] == 'query_bonus') {
	...
	$sort_flag = sort_flag($list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('bonus_view.dwt'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
