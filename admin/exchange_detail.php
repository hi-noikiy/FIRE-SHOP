...
{
	$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.give_integral, g.rank_integral, model_price, is_promote, promote_price, shop_price, model_price FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g WHERE g.user_id = \'' . $user_id . '\'';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
	...
	foreach ($res as $key => $row) {
		...
		$arr['give_integral'] += $row['give_integral'];
		$arr['rank_integral'] += $row['rank_integral'];
	}
