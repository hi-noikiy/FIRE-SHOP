...
if ($step == 'addChildCate') {
	...
	if (0 < $user_id) {
		...
		$cate_list = get_first_cate_list($cat_id, $type, $catarr, $_SESSION['user_id']);
...
if ($steps_audit == 1) {
	...
	$sql = 'select shoprz_brandName, shopNameSuffix, shop_class_keyWords, hopeLoginName, merchants_audit, merchants_message, steps_audit from ' . $ecs->table('merchants_shop_information') . ' where user_id = \'' . $_SESSION['user_id'] . '\'';
...
if (!empty($step) && ($step == 'stepTwo')) {
	...
	if (!empty($step) && ($step == 'stepThree')) {
		...
		if (!empty($step) && ($step == 'stepSubmit')) {
			$sid = 4;
			$sql = 'select shoprz_brandName, shopNameSuffix, shop_class_keyWords, hopeLoginName, merchants_audit, steps_audit from ' . $ecs->table('merchants_shop_information') . ' where user_id = \'' . $_SESSION['user_id'] . '\'';
			$shop_info = $db->getRow($sql);
			$shop_info['rz_shopName'] = str_replace('|', '', $shop_info['rz_shopName']);
			$smarty->assign('shop_info', $shop_info);
		}
