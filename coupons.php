...
function fromat_coupons($cou_data)
{
	...
	foreach ($cou_data as $k => $v) {
		...
		$cou_data[$k]['cou_type_name'] = $v['cou_type'] == 3 ? $GLOBALS['_LANG']['vouchers_all'] : ($v['cou_type'] == 4 ? $GLOBALS['_LANG']['vouchers_user'] : $GLOBALS['_LANG']['unknown']);
...
if ($_REQUEST['act'] == 'coupons_index') {
	...
	$sql = 'SELECT c.*,cu.user_id,cu.is_use FROM ' . $ecs->table('coupons') . ' c LEFT JOIN ' . $ecs->table('coupons_user') . ' cu ON c.cou_id=cu.cou_id WHERE c.review_status = 3 AND c.cou_type  NOT IN(1,2) AND c.cou_end_time>' . $time . ' GROUP BY c.cou_id ORDER BY c.cou_id DESC limit 6';
	...
	$order_field = (empty($_REQUEST['field']) ? 'c.cou_id' : 'c.' . addslashes($_REQUEST['field']));
