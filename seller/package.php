...
{
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('package_goods') . ' SET ' . ' package_id = \'' . $package_id . '\' ' . ' WHERE package_id = \'0\'' . ' AND admin_id = \'' . $_SESSION['seller_id'] . '\'';
...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('primary_cat', $_LANG['03_promotion']);
	$smarty->assign('menu_select', array('action' => '03_promotion', 'current' => '14_package_list'));
	$group_goods_list = array();
	$sql = 'DELETE FROM ' . $ecs->table('package_goods') . ' WHERE package_id = 0 AND admin_id = \'' . $_SESSION['seller_id'] . '\'';
	$db->query($sql);
	...
	$record = array('act_name' => $_POST['package_name'], 'act_desc' => $_POST['desc'], 'act_type' => GAT_PACKAGE, 'start_time' => $_POST['start_time'], 'user_id' => $adminru['ru_id'], 'activity_thumb' => $activity_thumb, 'end_time' => $_POST['end_time'], 'is_finished' => 0, 'ext_info' => serialize($info));
	...
	$package_goods_list = get_package_goods($act_id, $_SESSION['seller_id']);
	$smarty->assign('package', $package);
	...
	foreach ($arguments as $val) {
		...
		$sql = 'INSERT INTO ' . $ecs->table('package_goods') . ' (package_id, goods_id, product_id, goods_number, admin_id) ' . 'VALUES (\'' . $package_id . '\', \'' . $val_array[0] . '\', \'' . $val_array[1] . '\', \'' . $number . '\', \'' . $_SESSION['seller_id'] . '\')';
		$db->query($sql, 'SILENT');
	}

	$arr = get_package_goods($package_id, $_SESSION['seller_id']);
	$opt = array();
