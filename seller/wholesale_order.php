...
else if ($_REQUEST['act'] == 'operate') 
{
	...
	else if (isset($_POST['print'])) 
	{
		...
		foreach ($order_sn_list as $order_sn ) 
		{
			...
			$goods_list = array();
			$goods_attr = array();
			$sql = 'SELECT o.*, g.goods_thumb, g.goods_sn, g.brand_id, g.user_id AS ru_id, w.goods_number AS storage, w.act_id, g.model_inventory, o.goods_attr, oi.order_sn ' . 'FROM ' . $ecs->table('wholesale_order_goods') . ' AS o ' . 'LEFT JOIN ' . $ecs->table('goods') . ' AS g ON o.goods_id = g.goods_id ' . 'LEFT JOIN ' . $ecs->table('wholesale') . ' AS w ON w.goods_id = g.goods_id ' . 'LEFT JOIN ' . $ecs->table('wholesale_order_info') . ' AS oi ON oi.order_id = o.order_id ' . 'WHERE o.order_id = \'' . $order_id . '\' ';
			...
			$smarty->assign('goods_list', $goods_list);
			$smarty->template_dir = '../' . DATA_DIR;
			$html .= $smarty->fetch('wholesale_order_print.html') . '<div style="PAGE-BREAK-AFTER:always"></div>';
		}
		echo $html;
		exit();
	}
...
else if ($_REQUEST['act'] == 'get_goods_info') 
{
	...
	$goods_list = array();
	$goods_attr = array();
	$sql = 'SELECT o.*, g.goods_thumb, g.goods_sn, g.brand_id, g.user_id AS ru_id, w.goods_number AS storage, g.model_inventory, o.goods_attr, oi.order_sn ' . 'FROM ' . $ecs->table('wholesale_order_goods') . ' AS o ' . 'LEFT JOIN ' . $ecs->table('goods') . ' AS g ON o.goods_id = g.goods_id ' . 'LEFT JOIN ' . $ecs->table('wholesale') . ' AS w ON w.goods_id = g.goods_id ' . 'LEFT JOIN ' . $ecs->table('wholesale_order_info') . ' AS oi ON oi.order_id = o.order_id ' . 'WHERE o.order_id = \'' . $order_id . '\' ';
	$res = $db->query($sql);
	while ($row = $db->fetchRow($res)) 
	{
		if (empty($prod)) 
...
function wholesale_order_list() 
{
	...
	if ($result === false) 
	{
		...
		else if ($filter['composite_status'] == 1) 
		{
		...
		if (empty($filter['start_take_time']) || empty($filter['end_take_time'])) 
		{
			...
			{
				$where_store = ' AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' AS og ' . ' WHERE o.order_id = og.order_id AND og.ru_id = 0 LIMIT 1) > 0 ' . ' AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' AS oi2 WHERE oi2.main_order_id = o.order_id) = 0';
			}
		}
		if (!(empty($filter['start_take_time'])) || !(empty($filter['end_take_time']))) 
		{
			$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' AS o ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale_order_action') . ' AS oa ON o.order_id = oa.order_id ' . $where . $where_store . $no_main_order . ' GROUP BY o.order_id';
		...
		$sql = 'SELECT  o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.is_delete,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.user_id, o.referer, ' . '(' . order_amount_field('o.') . ') AS total_fee, (o.goods_amount - o.discount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee) AS total_fee_order ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' AS o ' . $leftJoin . $where . $where_store . $no_main_order . $groupBy . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
...
function download_orderlist($result) 
{
	...
	for ($i = 0; $i < $count; $i++) 
	{
		...
		$data .= $order_sn . ',' . $ru_name . $order_user . ',' . $order_time . ',' . $consignee . ',' . $tel . ',' . $address . ',' . $order_amount . ',' . $order_status . ',' . $pay_status . ',' . $shipping_status . "\n";
	}
	return $data;
}
function i($strInput) 
