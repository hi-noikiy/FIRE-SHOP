...
function warehouse_product_list($goods_id = 0)
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT region_id, region_name ' . ' FROM ' . $GLOBALS['ecs']->table('region_warehouse') . $ex_where . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
...
if ($_REQUEST['act'] == 'warehouse_list') {
	...
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('product_info.htm'), '', array('status' => 1, 'page_count' => $product['page_count']));
}
	...
	if ($product['product_number'] != $product_number) {
		...
		$logs_other = array('goods_id' => $product['goods_id'], 'order_id' => 0, 'use_storage' => $log_use_storage, 'admin_id' => $_SESSION['seller_id'], 'number' => $number, 'model_inventory' => $goods['model_inventory'], 'model_attr' => $goods['model_attr'], 'product_id' => $product_id, 'warehouse_id' => $product['warehouse_id'], 'area_id' => 0, 'add_time' => gmtime());
	...
	if (0 < product_warehouse_number_count($product['goods_id'], '', $product['warehouse_id'])) {
		$insert = false;
	}
	...
	foreach ($product['product_sn'] as $key => $value) {
		...
		if ($product['product_number'][$key]) {
			$logs_other = array('goods_id' => $product['goods_id'], 'order_id' => 0, 'use_storage' => 9, 'admin_id' => $_SESSION['seller_id'], 'number' => $number, 'model_inventory' => $goods['model_inventory'], 'model_attr' => $goods['model_attr'], 'product_id' => $GLOBALS['db']->insert_id(), 'warehouse_id' => $product['warehouse_id'], 'area_id' => 0, 'add_time' => gmtime());
			$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_inventory_logs'), $logs_other, 'INSERT');
		}
