		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			if ($_REQUEST['act'] == 'insert') {
				...
				if ($db->query($sql) == true) {
					...
					$sql = 'INSERT INTO' . $ecs->table('store_user') . '(`ru_id`,`store_id`,`stores_user`,`stores_pwd`,`ec_salt`,`store_action`,`add_time`,`tel`,`email`) VALUES (\'' . $adminru['ru_id'] . '\',\'' . $store_id . '\',\'' . $stores_user . '\',\'' . $stores_pwd . '\',\'' . $ec_salt . '\',\'all\',\'' . $time . '\',\'' . $stores_tel . '\',\'' . $email . '\')';
					$db->query($sql);
