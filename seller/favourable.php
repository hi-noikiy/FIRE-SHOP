...
function favourable_list($ru_id)
{
	...
	if ($result === false) {
		...
		if (0 < $filter['fav_dateout']) {
			...
			$where .= 'AND (end_time - 24 * 60 * 60 * 3) < \'' . $firstSecToday . '\'';
		}
...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('menu_select', array('action' => '03_promotion', 'current' => '12_favourable'));
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
		...
		if (($favourable['act_range'] != FAR_ALL) && !empty($favourable['act_range_ext'])) {
			if ($favourable['act_range'] == FAR_CATEGORY) {
				...
				if (0 < $ru_id) {
					$sql = 'SELECT bid AS id, brandName AS name FROM ' . $ecs->table('merchants_shop_brand') . ' WHERE bid ' . db_create_in($favourable['act_range_ext']) . ' AND user_id = \'' . $ru_id . '\' AND audit_status = 1';
				}
				else {
		...
		if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
			...
			$act_range_ext = intval($_POST['act_range_ext']);
			...
			if ($filter->act_range == FAR_ALL) {
				...
				if (0 < $ru_id) {
					...
					$sql = 'SELECT c.cat_id AS id, c.cat_name AS name FROM ' . $ecs->table('category') . ' as c ' . ' WHERE c.cat_name LIKE \'%' . mysql_like_quote($filter->keyword) . '%\' ' . 'AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_category') . ' as mc WHERE mc.cat_id = c.cat_id) < 1 ' . 'LIMIT 50';
					$arr = $db->getAll($sql);
				}
			}
