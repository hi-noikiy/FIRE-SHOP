...
if ($_REQUEST['act'] == 'add') {
	...
	if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
		...
		$sql = 'SELECT product_id FROM ' . $GLOBALS['ecs']->table('products') . ' WHERE goods_id = \'' . $result['list']['goods_id'] . '\'' . ' AND goods_attr = \'' . $result['list']['goods_attr'] . '\'';

		if ($GLOBALS['db']->getOne($sql, true)) {
			$db->autoExecute($ecs->table('products'), $other, 'UPDATE', 'goods_id = \'' . $result['list']['goods_id'] . '\' AND goods_attr = \'' . $result['list']['goods_attr'] . '\'');
