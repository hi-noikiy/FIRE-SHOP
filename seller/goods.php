...
if (($_REQUEST['act'] == 'list') || ($_REQUEST['act'] == 'trash') || ($_REQUEST['act'] == 'no_comment')) {
	...
	$action_link2 = ($_REQUEST['act'] == 'list' ? array('href' => 'goods.php?act=add_desc', 'text' => '商品统一详情', 'class' => 'icon-edit') : '');
	$smarty->assign('action_link2', $action_link2);
	$action_link3 = ($_REQUEST['act'] == 'list' ? array('href' => 'goods_transport.php?act=list', 'text' => '商品运费模板', 'class' => 'icon-edit') : '');
	$smarty->assign('action_link3', $action_link3);
	...
	assign_query_info();
	$htm_file = ($_REQUEST['act'] == 'list' ? 'goods_list.dwt' : ($_REQUEST['act'] == 'trash' ? 'goods_trash.dwt' : ($_REQUEST['act'] == 'no_comment' ? 'goods_no_comment.dwt' : 'group_list.dwt')));
	...
	if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit') || ($_REQUEST['act'] == 'copy')) {
		...
		if ($is_add) {
			...
			if ($_REQUEST['act'] == 'copy') {
				...
				$sql = 'DELETE FROM ' . $ecs->table('link_goods') . ' WHERE (goods_id = 0 OR link_goods_id = 0)' . ' AND admin_id = \'' . $_SESSION['seller_id'] . '\'';
				$db->query($sql);
				$sql = 'SELECT \'0\' AS goods_id, link_goods_id, is_double, \'' . $_SESSION['seller_id'] . '\' AS admin_id' . ' FROM ' . $ecs->table('link_goods') . ' WHERE goods_id = \'' . intval($_REQUEST['goods_id']) . '\' ';
				$res = $db->query($sql);
				...
				$sql = 'SELECT goods_id, \'0\' AS link_goods_id, is_double, \'' . $_SESSION['seller_id'] . '\' AS admin_id' . ' FROM ' . $ecs->table('link_goods') . ' WHERE link_goods_id = \'' . intval($_REQUEST['goods_id']) . '\' ';
				$res = $db->query($sql);
				...
				$sql = 'DELETE FROM ' . $ecs->table('group_goods') . ' WHERE parent_id = 0 AND admin_id = \'' . $_SESSION['seller_id'] . '\'';
				$db->query($sql);
				$sql = 'SELECT 0 AS parent_id, goods_id, goods_price, \'' . $_SESSION['seller_id'] . '\' AS admin_id ' . 'FROM ' . $ecs->table('group_goods') . ' WHERE parent_id = \'' . intval($_REQUEST['goods_id']) . '\' ';
				$res = $db->query($sql);
				...
				$sql = 'DELETE FROM ' . $ecs->table('goods_article') . ' WHERE goods_id = 0 AND admin_id = \'' . $_SESSION['seller_id'] . '\'';
				$db->query($sql);
				$sql = 'SELECT 0 AS goods_id, article_id, \'' . $_SESSION['seller_id'] . '\' AS admin_id ' . 'FROM ' . $ecs->table('goods_article') . ' WHERE goods_id = \'' . intval($_REQUEST['goods_id']) . '\' ';
				$res = $db->query($sql);
		...
		if (($_REQUEST['act'] == 'set_attribute_table') || ($_REQUEST['act'] == 'goods_attribute_query')) {
			...
			$option = '<li><a href="javascript:;" data-value="0" class="ftx-01">' . $_LANG['select_please'] . '</a></li>';

			foreach ($brand_list as $key => $value) {
				$option .= '<li><a href="javascript:;" data-value="' . $key . '" class="ftx-01">' . $value . '</a></li>';
			}
			...
			$tab_menu = array();
			$tab_menu[] = array('curr' => 1, 'text' => '添加关联描述', 'href' => 'javascript:;', 'ext' => 'data-tab="linkgoods"');
			$tab_menu[] = array('curr' => 0, 'text' => '描述列表', 'href' => 'javascript:;', 'ext' => 'data-tab="linklist"');
			$smarty->assign('tab_menu', $tab_menu);
			set_default_filter(0, 0, $adminru['ru_id']);
			assign_query_info();
			...
			if (($_REQUEST['act'] == 'insert_link_desc') || ($_REQUEST['act'] == 'update_link_desc')) {
				...
				if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
					...
					$shipping_fee = (empty($_POST['shipping_fee']) ? '0.00' : floatval($_POST['shipping_fee']));
					...
					if ($is_insert) {
						...
						$thumb_img_id = $_SESSION['thumb_img_id' . $_SESSION['seller_id']];
					...
					foreach ($product['product_sn'] as $key => $value) {
						...
						$logs_other = array('goods_id' => $goods_id, 'order_id' => 0, 'admin_id' => $_SESSION['seller_id'], 'model_inventory' => $goods['model_inventory'], 'model_attr' => $goods['model_attr'], 'add_time' => gmtime());
					...
					while ($row = $db->fetchRow($res)) {
						...
						if (!empty($arr)) {
							if ($GLOBALS['_CFG']['open_oss'] == 1) {
								$post_data = array('bucket' => $bucket_info['bucket'], 'keyid' => $bucket_info['keyid'], 'keysecret' => $bucket_info['keysecret'], 'is_cname' => $bucket_info['is_cname'], 'endpoint' => $bucket_info['outside_site'], 'object' => $arr);
					...
					$action_link = array('href' => 'goods.php?act=list', 'text' => '商品列表', 'class' => 'icon-reply');
					$smarty->assign('action_link', $action_link);
