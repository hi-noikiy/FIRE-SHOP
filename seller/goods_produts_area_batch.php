...
function get_list_download($goods_sn = '', $warehouse_info = array(), $attr_info, $attr_num, $model = 0)
{
	...
	$arr = array();
	if ((0 < count($warehouse_info)) && ($model == 2)) {
		if ($attr_info) {
...
if ($_REQUEST['act'] == 'add') {
	...
	if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
		...
		if ($GLOBALS['db']->getOne($sql)) {
			$db->autoExecute($ecs->table('products_area'), $other, 'UPDATE', 'goods_id = \'' . $result['list']['goods_id'] . '\'' . ' AND goods_attr = \'' . $result['list']['goods_attr'] . '\'' . ' AND area_id = \'' . $result['list']['area_id'] . '\'');
	...
	Header('Content-Disposition: attachment; filename=goods_produts_area_batch' . $goods_id . '_' . $area_id . '.csv');
	...
	if (isset($_LANG['upload_product'])) {
		if (($_GET['charset'] == 'zh_cn') || ($_GET['charset'] == 'zh_tw')) {
			...
			$attr_info = get_list_download($goods_info['goods_sn'], array($region_name), $_attribute, count($_attribute), $model);
