...
if ($_REQUEST['act'] == 'add') {
	...
	if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
		...
		$sql = 'SELECT product_id from ' . $GLOBALS['ecs']->table('products_warehouse') . ' WHERE goods_id = \'' . $result['list']['goods_id'] . '\'' . ' AND goods_attr = \'' . $result['list']['goods_attr'] . '\'' . ' AND warehouse_id = \'' . $result['list']['warehouse_id'] . '\'';
		$res = $GLOBALS['db']->getOne($sql);

		if ($GLOBALS['db']->getOne($sql)) {
			$db->autoExecute($ecs->table('products_warehouse'), $other, 'UPDATE', 'goods_id = \'' . $result['list']['goods_id'] . '\'' . ' AND goods_attr = \'' . $result['list']['goods_attr'] . '\'' . ' AND warehouse_id = \'' . $result['list']['warehouse_id'] . '\'');
