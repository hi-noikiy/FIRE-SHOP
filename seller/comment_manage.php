...
if ($_REQUEST['act'] == 'action') {
	...
	if (!empty($reply_info['content']) && ($adminru['ru_id'] == $comment_info['ru_id'])) {
		...
		$sql = 'INSERT INTO ' . $ecs->table('comment') . ' (comment_type, id_value, email, user_name , ' . 'content, add_time, ip_address, status, parent_id,ru_id) ' . 'VALUES(\'' . $_POST['comment_type'] . '\', \'' . $_POST['id_value'] . '\',\'' . $_POST['email'] . '\', ' . '\'' . $_SESSION['seller_name'] . '\',\'' . $_POST['content'] . '\',\'' . gmtime() . '\', ' . '\'' . $ip . '\', \'0\', \'' . $_POST['comment_id'] . '\',\'' . $adminru['ru_id'] . '\')';
	}
	else {
