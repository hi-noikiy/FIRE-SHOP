...
function order_list()
{
	...
	if ($result === false) {
		...
		if (($filter['source'] == 'start') || (0 < $adminru['ru_id']) || $filter['keywords']) {
		...
		$where_store = '';
		if (empty($filter['keywords']) && (empty($filter['start_take_time']) || empty($filter['end_take_time']))) {
			if (($store_search == 0) && ($adminru['ru_id'] == 0)) {
				$where .= ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
				$where_ru = ' (SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE og.order_id = o.order_id LIMIT 1) = 0';
			}
		}
		else if (!empty($filter['keywords'])) {
			$groupBy = ' GROUP BY o.order_id ';
		}
		else {
			$groupBy = ' GROUP BY o.order_id ';
			$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('order_action') . ' AS oa ON o.order_id = oa.order_id ';
		}

		$filter = page_and_size($filter);
		$sql = 'SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete,' . 'o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code, o.extension_id, o.user_id, o.referer, o.froms, o.chargeoff_status, ' . '(' . order_amount_field('o.') . ') AS total_fee, (o.goods_amount - o.discount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee) AS total_fee_order ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS bai ON o.order_id=bai.order_id ' . $leftJoin . $where . $where_store . $no_main_order . $groupBy . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
...
function download_orderlist($result)
{
	...
	$data = i('订单号,' . $data_name . '下单会员,下单时间,收货人,联系电话,地址,总金额,应付金额,确认状态,付款状态,发货状态,订单来源' . "\n");
	$count = count($result);
	...
	for ($i = 0; $i < $count; $i++) {
		...
		$address = i('[' . $result[$i]['region'] . '] ' . $result[$i]['address']);
		...
		$data .= $order_sn . ',' . $ru_name . $order_user . ',' . $order_time . ',' . $consignee . ',' . $tel . ',' . $address . ',' . i($result[$i]['total_fee']) . ',' . $order_amount . ',' . $order_status . ',' . $pay_status . ',' . $shipping_status . ',' . $froms . "\n";
	}
...
if ($_REQUEST['act'] == 'order_query') {
	...
	if ($order_finish) {
		...
		if ($cfg == '1') {
			...
			$smarty->assign('confirm_url', $ecs->seller_url() . 'user.php?act=order_detail&order_id=' . $order['order_id']);
			$smarty->assign('send_msg_url', $ecs->seller_url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
