...
if ($_REQUEST['act'] == 'list') {
	...
	if (admin_priv('take_manage')) {
		$tab_menu[] = array('curr' => 0, 'text' => $_LANG['gift_gard_list'], 'href' => 'gift_gard.php?act=take_list');
	}
...
if ($_REQUEST['act'] == 'handle_log') {
	$smarty->assign('menu_select', array('action' => '03_promotion', 'current' => 'take_list'));
...
if ($_REQUEST['act'] == 'add') {
	admin_priv('gift_gard_manage');
	$smarty->assign('primary_cat', $_LANG['03_promotion']);
	$smarty->assign('menu_select', array('action' => '03_promotion', 'current' => 'gift_gard_list'));
...
if ($_REQUEST['act'] == 'bonus_list') {
	...
	$smarty->assign('record_count', $list['record_count']);
	$smarty->assign('page_count', $list['page_count']);
	$smarty->assign('action_link2', array('href' => 'gift_gard.php?act=gift_add&bonus_type=' . $list['item'][0]['gift_id'], 'text' => $_LANG['gift_gard_add'], 'class' => 'icon-plus'));
	$smarty->assign('action_link3', array('href' => 'gift_gard.php?act=export_gift_gard&bonus_type=' . $list['item'][0]['gift_id'], 'text' => $_LANG['gift_gard_export'], 'class' => 'icon-download-alt'));
	$sort_flag = sort_flag($list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	assign_query_info();
...
if ($_REQUEST['act'] == 'take_list') {
	...
	if (admin_priv('take_manage')) {
		$tab_menu[] = array('curr' => 1, 'text' => $_LANG['gift_gard_list'], 'href' => 'gift_gard.php?act=take_list');
	}
