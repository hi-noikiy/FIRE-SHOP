...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('action_link', array('text' => $_LANG['discuss_add'], 'href' => 'discuss_circle.php?act=add', 'class' => 'icon-plus'));
	assign_query_info();
...
if ($_REQUEST['act'] == 'reply') {
	...
	$smarty->assign('action_link', array('text' => $_LANG['discuss_circle'], 'href' => 'discuss_circle.php?act=list', 'class' => 'icon-reply'));
	assign_query_info();
	$smarty->display('discuss_info.dwt');
}
...
if ($_REQUEST['act'] == 'action') {
	...
	if (!empty($reply_info['content'])) {
		...
		$sql = 'INSERT INTO ' . $ecs->table('comment') . ' (comment_type, id_value, email, user_name , ' . 'content, add_time, ip_address, status, parent_id) ' . 'VALUES(\'' . $_POST['comment_type'] . '\', \'' . $_POST['id_value'] . '\',\'' . $_POST['email'] . '\', ' . '\'' . $_SESSION['seller_name'] . '\',\'' . $_POST['content'] . '\',\'' . gmtime() . '\', \'' . $ip . '\', \'0\', \'' . $_POST['comment_id'] . '\')';
	}
