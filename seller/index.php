...
function get_sales($day_num)
{
	...
	if ($day_num == 1) {
		$date_start = local_mktime(0, 0, 0, date('m', time()), date('d', time()), date('Y', time()));
		$date_end = local_mktime(23, 59, 59, date('m', time()), date('d', time()), date('Y', time()));
	}
	else if ($day_num == 2) {
		$date_end = local_mktime(0, 0, 0, date('m'), date('d'), date('Y')) - 1;
		$date_start = ($date_end - (3600 * 24)) + 1;
	}

	$sql = 'SELECT IFNULL(SUM(' . order_commission_field('o.') . '),0) AS \'ga\', COUNT(o.order_id) AS \'oi\' ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('baitiao_log') . ' AS bai ON o.order_id=bai.order_id WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND o.referer NOT IN(\'touch\', \'mobile\')' . ' AND (SELECT og.ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE og.order_id = o.order_id LIMIT 1) = \'' . $adminru['ru_id'] . '\'' . $where;
	$row = $GLOBALS['db']->getRow($sql);
	if ($row && $row['oi']) {
		$sales = price_format($row['ga'] / $row['oi']);
		$count = price_format($row['ga']);
		$arr = array('sales' => $sales, 'count' => $count, 'order' => $row['oi']);
		return $arr;
	}
	else {
		return 0;
...
function get_sub_order($day_num)
{
	...
	$sql = 'SELECT COUNT(o.order_id) AS \'oi\' ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND o.referer NOT IN(\'touch\', \'mobile\')' . ' AND (SELECT og.ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE og.order_id = o.order_id LIMIT 1) = \'' . $adminru['ru_id'] . '\'' . $where;
	$row = $GLOBALS['db']->getRow($sql);
	if ($row && $row['oi']) {
		$sub_order = $row['oi'];
...
function get_move_sub_order($day_num)
{
	...
	$where = ' AND o.pay_status = 2';
	$where .= ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi2 WHERE oi2.main_order_id = o.order_id) = 0 ';
	$sql = 'SELECT COUNT(*) AS \'oi\' ' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND o.referer IN(\'touch\', \'mobile\')' . ' AND (SELECT og.ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE og.order_id = o.order_id LIMIT 1) = \'' . $adminru['ru_id'] . '\'' . $where;
	$row = $GLOBALS['db']->getRow($sql);
	if ($row && $row['oi']) {
		$sub_order = $row['oi'];
		$arr = array('sub_order' => $sub_order);
...
function viewip($ru_id)
{
	$date_start = local_mktime(0, 0, 0, date('m', time()), date('d', time()), date('Y', time()));
	$date_end = local_mktime(23, 59, 59, date('m', time()), date('d', time()), date('Y', time()));
	$sql = 'SELECT COUNT(i.ipid) AS ip ' . ' FROM ' . $GLOBALS['ecs']->table('source_ip') . ' AS i ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . 'AS s ON i.storeid = s.ru_id ' . ' WHERE i.iptime BETWEEN ' . $date_start . ' AND ' . $date_end . ' AND i.storeid = \'' . $ru_id . '\'';
	$row = $GLOBALS['db']->getRow($sql);
	if ($row && $row['ip']) {
		$todaycount = $row['ip'];
...
if ($_REQUEST['act'] == '') {
	...
	foreach ($seller_goods as $K => $v) {
		...
		if ($v['goods_number'] <= $v['warn_number']) {
			++$seller_goods_info['is_warn'];
		}
	}
	...
	$order['finished'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . ' as oi ' . ' WHERE 1 ' . order_query_sql('finished') . $where_og);
	$status['finished'] = CS_FINISHED;
	$order['await_ship'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . ' as oi ' . ' WHERE 1 ' . order_query_sql('await_ship') . $where_og);
	$status['await_ship'] = CS_AWAIT_SHIP;
	$order['await_pay'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . ' as oi ' . ' WHERE 1 ' . order_query_sql('await_pay') . $where_og);
	$status['await_pay'] = CS_AWAIT_PAY;
	$order['unconfirmed'] = $db->getOne('SELECT count(*) FROM ' . $ecs->table('order_info') . ' as oi ' . ' WHERE 1 ' . order_query_sql('unconfirmed') . $where_og);
	...
	$t_count = str_replace('<em>¥</em>', '', $today_sales['count']);
	$y_count = str_replace('<em>¥</em>', '', $today_move_sales['count']);
	$all_count = price_format($t_count + $y_count);
	...
	foreach ($seller_list as $key => $row) {
		...
		if ($row['cycle'] == 7) {
			...
			$last_year_end = local_date('Y-m-d', local_strtotime($last_year_start . ' +1 month -1 day')) . ' 23:59:59';
		}
		else if ($row['cycle'] == 2) {
			$nowMonth = $nowMonth - 1;
			...
			if ($halfDay < $nowDay) {
				...
				$middleMonth = $lastMonth[0] . '-' . $lastMonth[1] . '-' . $halfMonth;
				$lastMonth_firstDay = $nowYear . '-' . $nowMonth . '-01 00:00:00';
		...
		$last_year_start = local_strtotime($last_year_start);
		$last_year_end = local_strtotime($last_year_end);
		...
		if (!$bill_id && (0 < $last_year_start) && (0 < $last_year_end) && ($is_charge == 1)) {
			$bill_sn = get_order_sn();
			$other = array('seller_id' => $row['seller_id'], 'bill_sn' => $bill_sn, 'proportion' => $row['percent_value'], 'start_time' => $last_year_start, 'end_time' => $last_year_end, 'bill_cycle' => $row['cycle'], 'operator' => $_SESSION['seller_name']);
	...
	$shipping_area = read_static_cache('shipping_area_' . $adminru['ru_id'], 'data/sc_file/shiping_area/');

	if ($shipping_area === false) {
		$shipping_area = shippingArea($adminru['ru_id']);
		write_static_cache('shipping_area_' . $adminru['ru_id'], $shipping_area, 'data/sc_file/shiping_area/');
	}
