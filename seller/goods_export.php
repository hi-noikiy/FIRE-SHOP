...
if ($_REQUEST['act'] == 'goods_export') {
	...
	$page_size = 50;
	$sql = 'SELECT count(*) FROM ' . $ecs->table('goods') . ' AS g LEFT JOIN ' . $ecs->table('merchants_shop_brand') . ' AS b ' . 'ON g.brand_id = b.bid' . $where;
	$count = $db->getOne($sql);

	if ($result['page'] <= ceil($count / $result['page_size'])) {
		$start_time = gmtime();
		$sql = 'SELECT g.*, b.brandName as brandname ' . ' FROM ' . $ecs->table('goods') . ' AS g LEFT JOIN ' . $ecs->table('merchants_shop_brand') . ' AS b ' . 'ON g.brand_id = b.bid' . $where;
