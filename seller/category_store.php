...
if ($_REQUEST['act'] == 'list') {
	...
	if (isset($_REQUEST['back_level']) && (0 < $_REQUEST['back_level'])) {
		...
		$parent_id = $db->getOne('SELECT parent_id FROM ' . $ecs->table('merchants_category') . ' WHERE cat_id = \'' . $parent_id . '\'', true);
	}
	else {
	...
	if (0 < $adminru['ru_id']) {
		$smarty->assign('action_link', array('href' => 'category_store.php?act=add', 'text' => $_LANG['04_category_add'], 'class' => 'icon-plus'));
	}

	$cat_list = get_category_store_list($adminru['ru_id'], 0, $level, $parent_id);
