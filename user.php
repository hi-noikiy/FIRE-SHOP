...
if ($action == 'default') {
	...
	$record_count = $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('order_info') . ' as oi_1' . ' WHERE oi_1.user_id = \'' . $user_id . '\' and oi_1.is_delete = \'' . $type . '\' ' . ' AND oi_1.is_zc_order = 0 ' . ' and (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi_2 where oi_2.main_order_id = oi_1.order_id) = 0 ');
...
if ($action == 'register') {
	...
	if ($c) {
		...
		$count = get_users_auth($info['info_user_id'], $unionid);
		$connect_user = get_connect_user($info['user_id'], $unionid);
		$is_pc = 0;
	...
	if ($bind_type == 1) {
		if (!empty($sess_user)) {
			...
			$db->query($sql);
			$connect_serialize = array('openid' => str_replace($type . '_', '', $info_user_id), 'nickname' => $name, 'user_id' => $user_info['user_id'], 'unionid' => $unionid);
			$connect_other = array('connect_code' => 'sns_' . $type, 'user_id' => $user_info['user_id'], 'open_id' => str_replace($type . '_', '', $info_user_id), 'profile' => serialize($connect_serialize), 'create_at' => gmtime());
			$db->autoExecute($ecs->table('connect_user'), $connect_other, 'INSERT');
		}
	}
	else if ($bind_type == 2) {
		...
		$db->autoExecute($ecs->table('users_auth'), $other, 'INSERT');
		$connect_serialize = array('openid' => str_replace($type . '_', '', $info_user_id), 'nickname' => $name, 'user_id' => $other['user_id'], 'unionid' => $unionid);
		$connect_other = array('connect_code' => 'sns_' . $type, 'user_id' => $other['user_id'], 'open_id' => str_replace($type . '_', '', $info_user_id), 'profile' => serialize($connect_serialize), 'create_at' => gmtime());
		$db->autoExecute($ecs->table('connect_user'), $connect_other, 'INSERT');
	}
	...
	if ($user_id) {
		...
		if (file_exists(MOBILE_WECHAT)) {
			$sql = 'UPDATE ' . $ecs->table('wechat_user') . ' SET ect_uid = \'' . $user_id . '\' WHERE unionid = \'' . $info['unionid'] . '\'';
			$db->query($sql);
		}
	...
	include_once 'includes/cls_json.php';
	$_POST = get_request_filter($_POST, 1);
	$is_jsonp = (isset($_POST['is_jsonp']) ? intval($_POST['is_jsonp']) : 0);
	...
	if (empty($user_arr)) {
		show_message($_LANG['Mobile_username'], $_LANG['back_home_lnk'], './', 'info');
	}
	...
	if (($action == 'order_list') || ($action == 'order_recycle')) {
		...
		if (($action == 'order_detail') || ($action == 'auction_order_detail')) {
			...
			$noTime = gmtime();
			$sql = 'SELECT order_id, user_id, order_sn , order_status, shipping_status, pay_status, auto_delivery_time, add_time, ' . 'order_amount, goods_amount, tax, invoice_type, shipping_fee, insure_fee, pay_fee, pack_fee, card_fee, shipping_time, ' . 'bonus, integral_money, coupons, discount, money_paid, surplus, confirm_take_time, tax_id ' . 'FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE order_id = \'' . $order_id . '\' LIMIT 1';
			...
			if (0 < $sign_time) {
				$sql = 'SELECT log_time FROM ' . $GLOBALS['ecs']->table('order_action') . ' WHERE order_id =\'' . $order_id . '\' and action_note != \'\' order by action_id DESC';
				$log_time = $GLOBALS['db']->getOne($sql);
				$day = ($time - $log_time) / 3600 / 24;
				if (($orderInfo['order_status'] != OS_CANCELED) && ($orderInfo['pay_status'] == PS_PAYED)) {
					if ($day < $sign_time) {
						$order['return_url'] = 'user.php?act=goods_order&order_id=' . $order_id;
					}
					else {
			...
			if (!empty($order['invoice_no'])) {
				$invoice_no_list = explode('<br>', $order['invoice_no']);
			...
			$page = (isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1);
			$record_count = $db->getAll('SELECT oi.order_id FROM ' . $ecs->table('order_info') . ' as oi' . ' left join ' . $ecs->table('order_goods') . ' as og on oi.order_id = og.order_id' . $left_join . ' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = \'' . $show_type . '\' ' . $act . ' and (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi_2 where oi_2.main_order_id = oi.order_id) = 0 ' . $where . ' group by oi.order_id');
			$record_count = count($record_count);
			$orders = get_user_orders($user_id, $record_count, $page, $is_delete = 0, $where, $order, $show_type, $pagesize = 10);
				...
				if (0 < $surplus['rec_id']) {
					...
					if (!empty($sc_guid) && !empty($sc_rand) && isset($_COOKIE['user_account_cookie'])) {
						...
						if ($is_ok == 1) {
							...
							log_account_change($user_id, $amount, $frozen_money, 0, 0, '【' . $_LANG['Application_withdrawal'] . '】' . $surplus['user_note'], ACT_ADJUSTING);
			...
			if (($GLOBALS['_CFG']['sms_order_payed'] == '1') && ($sms_shop_mobile != '')) {
				...
				$smsParams = array('shop_name' => $shop_name, 'order_sn' => $order['order_sn'], 'consignee' => $order['consignee'], 'order_region' => $order_region, 'address' => $order['address'], 'order_mobile' => $order['mobile'], 'mobile_phone' => $sms_shop_mobile);
			...
			foreach ($coupons_list as $k => $v) {
				...
				$v['cou_type_name'] = $v['cou_type'] == 1 ? $_LANG['vouchers_login'] : ($v['cou_type'] == 2 ? $_LANG['vouchers_shoping'] : ($v['cou_type'] == 3 ? $_LANG['vouchers_all'] : ($v['cou_type'] == 4 ? $_LANG['vouchers_user'] : $_LANG['unknown'])));
			...
			$goods_cause = get_goods_cause($goods_info['goods_cause'], $order['chargeoff_status']);
			...
			if ($type == 'change_password') {
				...
				if ($step == 'first') {
					...
					$front_name = $image->upload_image('', 'idcard', '', 1, $_FILES['front_of_id_card']['name'], $_FILES['front_of_id_card']['type'], $_FILES['front_of_id_card']['tmp_name'], $_FILES['front_of_id_card']['error'], $_FILES['front_of_id_card']['size']);
					$reverse_name = $image->upload_image('', 'idcard', '', 1, $_FILES['reverse_of_id_card']['name'], $_FILES['reverse_of_id_card']['type'], $_FILES['reverse_of_id_card']['tmp_name'], $_FILES['reverse_of_id_card']['error'], $_FILES['reverse_of_id_card']['size']);
			...
			$sql = 'select zp.*,oi.order_id,oi.pay_status,oi.shipping_status,oi.goods_amount,join_num as zhichi_num from ' . $ecs->table('zc_goods') . ' as zg left join ' . $ecs->table('zc_project') . " as zp on zg.pid=zp.id\r\n            left join " . $ecs->table('order_info') . ' as oi on zg.id=oi.zc_goods_id where oi.user_id = \'' . $user_id . '\' and oi.is_zc_order=1 order by oi.order_id desc';
