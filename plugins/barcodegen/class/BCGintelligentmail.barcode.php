...
class BCGintelligentmail extends BCGBarcode1D
{
	...
	public function setTrackingCode($barcodeIdentifier, $serviceTypeIdentifier, $mailerIdentifier, $serialNumber)
	{
		...
		if (($serviceTypeIdentifier < 0) || (999 < $serviceTypeIdentifier)) {
		...
		if (899999 < $mailerIdentifier) {
			$mailerIdentifierLength = 9;
		}

		if (($mailerIdentifierLength === 9) && (6 < strlen($serialNumber))) {
		...
		if ($mailerIdentifierLength === 9) {
			if (($mailerIdentifierLength < 0) || (999999999 < $mailerIdentifier)) {
	...
	static private function executeStep1($text, $barcodeIdentifier, $serviceTypeIdentifier, $mailerIdentifier, $serialNumber)
	{
	...
	static private function executeStep2($number)
	{
		$byteArray = str_pad(self::bcdecuc($number), 13, chr(0), STR_PAD_LEFT);
		$generatorPolynomial = 3893;
		$frameCheckSequence = 2047;
		$data = 0;
		$byteIndex = 0;
		$bit = 0;
		$data = (ord($byteArray[$byteIndex]) << 5) & 65535;

		for ($bit = 2; $bit < 8; $bit++) {
			if (($frameCheckSequence ^ $data) & 1024) {
		...
		for ($byteIndex = 1; $byteIndex < 13; $byteIndex++) {
			$data = (ord($byteArray[$byteIndex]) << 3) & 65535;
	...
	static private function executeStep3($number)
	{
		$codewords = array();
	...
	static private function executeStep4($codewords, $crc)
	{
		$codewords[9] *= 2;
	...
	static private function executeStep5($codewords, $crc)
	{
		$characters = array();
	...
	static private function executeStep6($characters)
	{
		$bars = '';
		...
		for ($i = 0; $i < 65; $i++) {
			...
			else if ($extenderDescender) {
				$bars .= 'D';
			}
			else if ($extenderAscender) {
				$bars .= 'A';
			}
			else {
				$bars .= 'T';
	...
	static private function conversionRoutingCode($zipcode)
	{
		$number = $zipcode;
	...
	static private function conversionTrackingCode($number, $barcodeIdentifier, $serviceTypeIdentifier, $mailerIdentifier, $serialNumber)
	{
	...
	static private function bcdecuc($dec)
	{
		$last = bcmod($dec, 256);
