...
{
	$sql = ' select count(topic_id) from ' . $GLOBALS['ecs']->table('zc_topic') . ' where pid=\'' . $zcid . '\' AND topic_status = 1 ';
	return $GLOBALS['db']->getOne($sql);
}
...
if ($_REQUEST['act'] == 'detail') {
	...
	if (!file_exists('images/weixin_zc')) {
		mkdir('images/weixin_zc');
	}

	$filename = 'images/weixin_zc/zc_' . $cid . '.png';
...
if ($_REQUEST['act'] == 'done') {
	...
	foreach ($arr_payment as $val) {
		if ($val['pay_code'] == 'alipay') {
			$zhifu['pay_id'] = $val['pay_id'];
			$zhifu['pay_name'] = $val['pay_name'];
			$zhifu['pay_code'] = $val['pay_code'];
		}
	}

	if (empty($zhifu)) {
		$zhifu['pay_id'] = $arr_payment[0]['pay_id'];
		$zhifu['pay_name'] = $arr_payment[0]['pay_name'];
		$zhifu['pay_code'] = $arr_payment[0]['pay_code'];
	}
...
if ($_REQUEST['act'] == 'search_paixu_tid') {
	...
	echo $sql = 'SELECT `id`,`cat_id`,`title`,`start_time`,`end_time`,`amount`,`join_money`,`join_num`,`title_img`,`describe`,(end_time-unix_timestamp(now())) as shenyu_time FROM' . $ecs->table('zc_project') . ' WHERE cat_id=\'' . $tid . '\' ' . $where_wenzi . ' ' . $where_tj . ' ';
	$zc_arr = $db->getAll($sql);
