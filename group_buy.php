...
function group_buy_list($children = '', $size, $page, $keywords, $sort, $order, $type = '')
{
	...
	while ($group_buy = $GLOBALS['db']->fetchRow($res)) {
		...
		$mc_one = ments_count_rank_num($group_buy['goods_id'], 1);
		$mc_two = ments_count_rank_num($group_buy['goods_id'], 2);
		$mc_three = ments_count_rank_num($group_buy['goods_id'], 3);
		$mc_four = ments_count_rank_num($group_buy['goods_id'], 4);
		$mc_five = ments_count_rank_num($group_buy['goods_id'], 5);
...
if ($_REQUEST['act'] == 'list') {
	...
	$cart = array('user_id' => $_SESSION['user_id'], 'session_id' => $sess, 'goods_id' => $group_buy['goods_id'], 'product_id' => $product_info['product_id'], 'goods_sn' => addslashes($goods['goods_sn']), 'goods_name' => addslashes($goods['goods_name']), 'market_price' => $goods['market_price'], 'goods_price' => $goods_price, 'goods_number' => $number, 'goods_attr' => addslashes($goods_attr), 'goods_attr_id' => $specs, 'ru_id' => $goods['user_id'], 'warehouse_id' => $region_id, 'area_id' => $area_id, 'is_real' => $goods['is_real'], 'extension_code' => addslashes($goods['extension_code']), 'parent_id' => 0, 'rec_type' => CART_GROUP_BUY_GOODS, 'is_gift' => 0);
