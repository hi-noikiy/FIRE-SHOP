...
function get_pre_goods($cat_id, $min = 0, $max = 0, $start_time = 0, $end_time = 0, $sort, $status = 0, $order)
{
	...
	foreach ($res as $key => $row) {
		$res[$key]['goods_name'] = !empty($row['act_name']) ? $row['act_name'] : $row['goods_name'];
...
if ($_REQUEST['act'] == 'index') {
	...
	if (!empty($_REQUEST['act']) && ($_REQUEST['act'] == 'guess_goods')) {
		...
		$area = array('region_id' => $region_id, 'province_id' => $province_id, 'city_id' => $city_id, 'district_id' => $district_id, 'street_id' => $street_id, 'street_list' => $street_list, 'goods_id' => $goods_id, 'user_id' => $user_id, 'area_id' => $area_info['region_id'], 'merchant_id' => $goods['user_id']);
		...
		$cart = array('user_id' => $_SESSION['user_id'], 'session_id' => $sess, 'goods_id' => $presale['goods_id'], 'product_id' => $product_info['product_id'], 'goods_sn' => addslashes($goods['goods_sn']), 'goods_name' => addslashes($goods['goods_name']), 'market_price' => $goods['market_price'], 'goods_price' => $goods['goods_price'], 'goods_number' => $number, 'goods_attr' => addslashes($goods_attr), 'goods_attr_id' => $specs, 'ru_id' => $goods['user_id'], 'warehouse_id' => $region_id, 'area_id' => $area_id, 'is_real' => $goods['is_real'], 'extension_code' => 'presale', 'parent_id' => 0, 'rec_type' => CART_PRESALE_GOODS, 'is_gift' => 0);
