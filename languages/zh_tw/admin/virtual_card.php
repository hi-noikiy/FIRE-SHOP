...
$_LANG['use_help'] = '使用說明：' . '<ol>' . '<li>上傳文件應為CSV文件<br />' . 'CSV文件第一列為卡片序號；第二列為卡片密碼；第三列為使用截至日期。<br />' . '(用EXCEL創建csv文件方法：在EXCEL中按卡號、卡片密碼、截至日期的順序填寫數據，完成後直接保存為csv文件即可)' . '<li>密碼，和截至日期可以為空，截至日期格式為2006-11-6或2006/11/6' . '<li>卡號、卡片密碼、截至日期中不要使用中文</li>' . '</ol>';
$_LANG['virtual_card_change'] = '更改加密串';
$_LANG['user_guide'] = '使用說明：' . '<ol>' . '<li>加密串是在加密虛擬卡類商品的卡號和密碼時使用的</li>' . '<li>加密串保存在文件 data/config.php 中，對應的常量是 AUTH_KEY</li>' . '<li>如果要更改加密串，在下面的文本框中輸入原加密串和新加密串，點\'確定\'按鈕後即可</li>' . '</ol>';
