...
function insert_history()
{
	...
	if (!empty($_COOKIE['ECS']['history'])) {
		...
		$sql = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE ' . $where . ' AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 order by INSTR(\'' . $_COOKIE['ECS']['history'] . '\',goods_id) limit 0,5';
...
function insert_history_test()
{
	...
	if (!empty($_COOKIE['ECS']['history'])) {
		...
		$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, ' . $shop_price . 'IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\'), g.shop_price * \'' . $_SESSION['discount'] . '\')  AS shop_price, ' . 'IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, ' . 'g.is_promote, g.promote_start_date, g.promote_end_date FROM ' . $GLOBALS['ecs']->table('goods') . ' as g ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE ' . $where . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 limit 0,10';
		$query = $GLOBALS['db']->query($sql);
		$res = array();
...
function insert_cart_info($type = 0, $num = 0)
{
	...
	if ((0 < $type) || ($type == 4)) {
		$sql = 'SELECT c.*,g.goods_thumb,g.goods_id,c.goods_number,c.goods_price' . ' FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id=c.goods_id ' . ' WHERE ' . $c_sess . ' AND rec_type = \'' . CART_GENERAL_GOODS . '\' and c.stages_qishu=\'-1\' AND c.store_id = 0 ' . $limit;
...
function insert_get_adv_child($arr)
{
	...
	if (defined('THEME_EXTENSION')) {
		...
		if ($bonushome_ad == 'bonushome') {
			if ($_COOKIE['bonushome'] == 1) {
				$val = '';
			}
			else {
				setcookie('bonushome', 1, gmtime() + (3600 * 10), $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
...
function get_ad_posti_child($cat_n_child = '', $warehouse_id = 0, $area_id = 0)
{
	...
	foreach ($res as $key => $row) {
		...
		$arr[$key]['ad_name'] = $row['ad_name'];
		$arr[$key]['ad_code'] = $GLOBALS['_CFG']['site_domain'] . DATA_DIR . '/afficheimg/' . $row['ad_code'];
		$arr[$key]['ad_bg_code'] = $GLOBALS['_CFG']['site_domain'] . DATA_DIR . '/afficheimg/' . $row['ad_bg_code'];
		if (($GLOBALS['_CFG']['open_oss'] == 1) && !empty($row['ad_code'])) {
			$bucket_info = get_bucket_info();
			$arr[$key]['ad_code'] = $bucket_info['endpoint'] . DATA_DIR . '/afficheimg/' . $row['ad_code'];
			$arr[$key]['ad_bg_code'] = $bucket_info['endpoint'] . DATA_DIR . '/afficheimg/' . $row['ad_bg_code'];
		}
...
function insert_history_goods($parameter)
{
	...
	if (!empty($_COOKIE['ECS']['history'])) {
		...
		$sql = 'SELECT g.goods_id, g.user_id, g.goods_name, g.goods_thumb, g.goods_img, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS org_price, ' . $shop_price . $promote_price . 'g.market_price, g.sales_volume, ' . 'g.promote_start_date, g.promote_end_date' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' as g ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . $leftJoin . ' WHERE ' . $where . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 order by INSTR(\'' . $_COOKIE['ECS']['history'] . '\',g.goods_id) limit 0,10';
...
function insert_goods_delivery_area_js($arr)
{
	...
	$GLOBALS['smarty']->caching = false;
	$GLOBALS['smarty']->force_compile = true;
	$area = array('goods_id' => $arr['area']['goods_id'], 'region_id' => $arr['area']['region_id'], 'province_id' => $arr['area']['province_id'], 'city_id' => $arr['area']['city_id'], 'district_id' => $arr['area']['district_id'], 'street_id' => $arr['area']['street_id'], 'street_list' => $arr['area']['street_list'], 'merchant_id' => $arr['area']['merchant_id']);
	$area['region_id'] = isset($_COOKIE['area_region']) ? $_COOKIE['area_region'] : $area['region_id'];
