...
function get_business_category_tree()
{
	$sql = 'SELECT cat_id, cat_name, keywords, cat_desc, cat_icon, pinyin_keyword, cat_alias_name, style_icon FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ' WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order ASC limit 16';
	...
	foreach ($res as $key => $row) {
		...
		$arr[$row['cat_id']]['url'] = build_uri('wholesale_cat', array('act' => 'list', 'cid' => $row['cat_id']), $row['cat_name']);
	}
...
function cat_brand_count($cat = 0)
{
	...
	$row = $GLOBALS['db']->getAll($sql);
	$sql = 'SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > \'\', \'1\', \'0\') AS tag, g.user_id ' . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('link_brand') . 'AS lb ON lb.brand_id = b.brand_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_brand') . 'AS msb ON msb.bid = lb.bid AND msb.audit_status = 1 AND msb.is_show = 1 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = msb.bid AND g.user_id > 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . 'WHERE ' . $children . ' AND b.is_show = 1 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC LIMIT 0,1';
	$msb_row = $GLOBALS['db']->getAll($sql);
...
function get_merchants_brands($cat = 0, $app = 'brand')
{
	...
	$sql = 'SELECT b.bid, b.brandName as brand_name, b.brandLogo as brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brandLogo > \'\', \'1\', \'0\') AS tag, ' . '(SELECT lb.brand_id FROM ' . $GLOBALS['ecs']->table('link_brand') . ' AS lb WHERE lb.bid = b.bid) AS brand_id ' . 'FROM ' . $GLOBALS['ecs']->table('merchants_shop_brand') . 'AS b, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'WHERE g.brand_id = b.bid ' . $children . ' AND is_show = 1 ' . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND b.audit_status = 1 ' . 'GROUP BY b.bid HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC';
	$row = $GLOBALS['db']->getAll($sql);
...
function get_promotion_info($goods_id = '', $ru_id = 0)
{
	...
	$favourable = array();
	$sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time, act_type, userFav_type FROM ' . $GLOBALS['ecs']->table('favourable_activity') . ' WHERE review_status = 3 AND start_time <= \'' . $gmtime . '\' AND end_time >= \'' . $gmtime . '\' AND ' . $fav_where;
	...
	if (empty($goods_id)) {
		...
		$sql = 'SELECT g.cat_id, g.brand_id FROM ' . $GLOBALS['ecs']->table('goods') . ' as g' . $leftJoin . ' WHERE g.goods_id = \'' . $goods_id . '\' LIMIT 1';
...
{
	$is_go = true;
	$sql = 'SELECT u.user_id, u.email, u.user_name, u.mobile_phone, u.user_money, u.frozen_money, u.pay_points, u.rank_points' . ' FROM ' . $GLOBALS['ecs']->table('users') . ' AS u ' . ' WHERE u.user_id = \'' . $user_id . '\'';
	...
	if ($is_go && ($user_money || $frozen_money || $rank_points || $pay_points)) {
		$account_log = array('user_id' => $user_id, 'user_money' => $user_money, 'frozen_money' => $frozen_money, 'rank_points' => $rank_points, 'pay_points' => $pay_points, 'change_time' => gmtime(), 'change_desc' => $change_desc, 'change_type' => $change_type);
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array(), $warehouse_id = 0, $area_id = 0, $type = 0, $presale = 0, $add_tocart = 1, $show_goods = 0)
{
	...
	$sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('presale_activity') . ' AS pa, ' . $GLOBALS['ecs']->table('goods') . ' AS g WHERE pa.goods_id = \'' . $goods_id . '\' AND pa.review_status = 3 AND pa.end_time > ' . gmtime() . ' AND pa.goods_id = g.goods_id AND g.is_on_sale = 0';
...
function get_for_purchasing_goods($start_date = 0, $end_date = 0, $goods_id = 0, $user_id = 0, $extension_code = '', $attr_id = '')
{
	...
	$sql = 'SELECT og.goods_number FROM ' . $GLOBALS['ecs']->table('order_goods') . ' as og, ' . $GLOBALS['ecs']->table('order_info') . ' as oi ' . 'WHERE oi.order_id = og.order_id ' . ' AND og.goods_id = \'' . $goods_id . '\' AND oi.add_time > \'' . $start_date . '\' AND oi.add_time < \'' . $end_date . '\'' . $where;
	$res = $GLOBALS['db']->getAll($sql);
	$goods_number = 0;
...
function get_store_shop_list($libType = 0, $keywords = '', $count = 0, $size = 16, $page = 1, $sort = 'shop_id', $order = 'DESC', $warehouse_id = 0, $area_id = 0, $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '')
{
	...
	$sql = 'SELECT msi.shop_id, msi.user_id, msi.shoprz_brandName, msi.shopNameSuffix, msi.self_run ' . $select . ' FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as msi ' . $where_table . ' where ' . $whereShop . ' AND msi.merchants_audit = 1 ORDER BY ' . $sort . ' ' . $order . ' ' . $limit;
	$res = $GLOBALS['db']->query($sql);
	$arr = array();
...
function get_store_shop_count($keywords = '', $sort = 'shop_id', $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '', $libType = 0)
{
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' AS msi' . $where_table . ' where ' . $whereShop . ' ' . ' AND msi.merchants_audit = 1 ';
...
function get_shop_goods_count_list($user_id, $warehouse_id, $area_id, $type = 0, $isType = '', $show_type = 0)
{
	...
	if ($type == 1) {
		...
		$leftJoin .= 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ';
		$select = 'g.goods_id, g.goods_thumb, g.goods_name, g.user_id, g.promote_start_date, g.promote_end_date, g.market_price, g.sales_volume, ';
		$select .= 'IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\'), g.shop_price * \'' . $_SESSION['discount'] . '\')  AS shop_price, ';
		$select .= 'IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price';
	}
	else {
...
function get_user_bouns_new_list($user_id = 0, $page = 1, $type = 0, $pageFunc = '', $amount = 0, $size = 10)
{
	...
	$record_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' as u,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b' . ' WHERE u.bonus_type_id = b.type_id AND b.review_status = 3 ' . $uOrder . ' AND u.user_id = \'' . $user_id . '\' ' . $useDate);
	$bouns_paper = '';
	$limit = '';
	...
	$sql = 'SELECT  u.bonus_id, u.bonus_sn, u.order_id, u.bind_time, b.type_name, b.type_money,b.min_amount, b.min_goods_amount, b.use_start_date, b.use_end_date, ' . 'b.usebonus_type, b.user_id AS ru_id FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' AS u ,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b' . ' WHERE u.bonus_type_id = b.type_id AND b.review_status = 3 ' . $uOrder . ' AND u.user_id = \'' . $user_id . '\' ' . $useDate . ' order by u.bonus_id DESC ' . $limit;
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
{
	$sql = 'SELECT cat_id, cat_name, style_icon, cat_icon, category_links, cat_alias_name FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC limit 16';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function get_category_brands_ad($cat_id)
{
	...
	$sql = 'SELECT b.brand_id, b.brand_name,  b.brand_logo, COUNT(*) AS goods_num, IF(b.brand_logo > \'\', \'1\', \'0\') AS tag, g.user_id ' . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('link_brand') . 'AS lb ON lb.brand_id = b.brand_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_brand') . 'AS msb ON msb.bid = lb.bid AND msb.audit_status = 1 AND msb.is_show = 1 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = msb.bid AND g.user_id > 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods_cat') . ' AS gc ON g.goods_id = gc.goods_id ' . 'WHERE (' . $g_children . ' OR ' . $gc_children . ') AND b.is_show = 1 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC LIMIT 0,12';
...
function available_shipping_list($region, $ru_id = 0, $is_limit = 0)
{
	...
	$sql = 'SELECT r.region_id, ' . '(SELECT re.region_name FROM ' . $GLOBALS['ecs']->table('region') . ' AS re WHERE re.region_id = r.region_id LIMIT 1 ) AS region_name, ' . '(SELECT re.parent_id FROM ' . $GLOBALS['ecs']->table('region') . ' AS re WHERE re.region_id = r.region_id LIMIT 1 ) AS parent_id, ' . ' s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' . $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' . $GLOBALS['ecs']->table('area_region') . ' AS r ' . 'WHERE r.region_id ' . db_create_in($region) . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND a.ru_id = \'' . $ru_id . '\' AND s.enabled = 1' . $order_by . $limit;
	return $GLOBALS['db']->getAll($sql);
}
...
{
	$spec_price = 0;
	$price = array('market_price' => !empty($goods_price_info['market_price']) ? $goods_price_info['market_price'] + $spec_price : $spec_price, 'org_price' => !empty($goods_price_info['org_price']) ? $goods_price_info['org_price'] + $spec_price : $spec_price, 'shop_price' => !empty($goods_price_info['shop_price']) ? $goods_price_info['shop_price'] + $spec_price : $spec_price, 'promote_price' => !empty($goods_price_info['promote_price']) ? $goods_price_info['promote_price'] + $spec_price : 0);
	return $price;
}
...
function get_goods_extend_info($goods_id = 0)
{
	...
	$sql = ' SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('goods_extend') . ' WHERE goods_id = \'' . $goods_id . '\' ';
...
{
	static $cat_list = '';
	$sql = 'SELECT parent_id,cat_id,cat_name FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE cat_id=\'' . $cat_id . '\' ORDER BY sort_order';
...
function get_filter_goods_list($filter = array('goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0), $size = 10, $page = 1, $sort = 'sort_order', $order = 'ASC', $warehouse_id = 0, $area_id = 0)
{
	$where = ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ';
	if (isset($filter['goods_ids']) && !empty($filter['goods_ids'])) {
		$where .= ' AND g.goods_id IN (' . $filter['goods_ids'] . ') ';
	}
	...
	if (isset($filter['cat_ids']) && !empty($filter['cat_ids'])) {
		...
		$where .= ' AND g.cat_id IN (' . implode(',', $cat_ids) . ') ';
	}

	if (isset($filter['brand_ids']) && !empty($filter['brand_ids'])) {
		$where_brand = ' AND b.brand_id IN (' . $filter['brand_ids'] . ') ';
	...
	$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('warehouse_area_goods') . ' AS wag ON g.goods_id = wag.goods_id AND wag.region_id = \'' . $area_id . '\' ';
	$leftJoin .= ' LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ';
	$sql = ' SELECT COUNT(*) ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . ' WHERE 1 ' . $where . ' ';
	$record_count = $GLOBALS['db']->getOne($sql);
	$page_count = (0 < $record_count ? ceil($record_count / $size) : 1);
	$sql = ' SELECT g.goods_id, g.goods_name, g.goods_thumb, g.promote_start_date, g.promote_end_date, ' . $shop_price . ',' . $promote_price . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . ' WHERE 1 ' . $where . ' GROUP BY g.goods_id ORDER BY ' . $sort . ' ' . $order . ' ';
	$start = ($page - 1) * $size;
...
function create_snapshot($order_id = 0)
{
	...
	if ($order_ids = $GLOBALS['db']->getAll($sql)) {
