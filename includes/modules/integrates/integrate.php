...
class integrate
{
	...
	public function check_user($username, $password = '')
	{
		...
		if (preg_match('/[^\\d-., ]/', $username)) {
			...
			if (preg_match($a, $username)) {
				$where = $this->field_email . '=\'' . $post_username . '\'';
			}
			else {
			...
			if ($GLOBALS['db']->getOne($sql)) {
				...
				$where = $this->field_phone . '=\'' . $post_username . '\'';
			}
		}

		$sql = 'SELECT ' . $this->field_id . ' FROM ' . $this->table($this->user_table) . ' WHERE ' . $where . ' AND ' . $this->field_pass . ' =\'' . $this->compile_password(array('password' => $password)) . '\'';
	...
	public function set_cookie($username = '', $remember = NULL, $is_oath = 1)
	{
		if (empty($username)) {
			$time = time() - 3600;
			...
			if ($is_oath == 1) {
				...
				$sql = 'SELECT user_id, password FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE ' . $field_name . ' LIMIT 1';
				$row = $GLOBALS['db']->getRow($sql);
			}
			else {
				$sql = 'SELECT u.user_id, u.password, u.email, identifier FROM ' . $GLOBALS['ecs']->table('users') . ' AS u,' . $GLOBALS['ecs']->table('users_auth') . ' AS ua' . ' WHERE u.user_id = ua.user_id AND ua.user_name = \'' . $username . '\' LIMIT 1';
				$row = $GLOBALS['db']->getRow($sql);
			}
	...
	public function set_session($username = '', $is_oath = 1)
	{
		if (empty($username)) {
			...
			if ($is_oath == 1) {
				...
				$sql = 'SELECT u.user_id, u.password, u.email FROM ' . $GLOBALS['ecs']->table('users') . ' AS u WHERE ' . $field_name . ' LIMIT 1';
				$row = $GLOBALS['db']->getRow($sql);
			}
			else {
				$sql = 'SELECT u.user_id, u.password, u.email, identifier FROM ' . $GLOBALS['ecs']->table('users') . ' AS u,' . $GLOBALS['ecs']->table('users_auth') . ' AS ua' . ' WHERE u.user_id = ua.user_id AND ua.user_name = \'' . $username . '\' LIMIT 1';
			...
			if ($row) {
				$_SESSION['identifier'] = isset($row['identifier']) ? $row['identifier'] : '';
