...
class shopex47
{
	...
	{
		return array($this->sprefix . 'mall_offer_pcat', $this->sprefix . 'mall_brand', $this->sprefix . 'mall_goods', $this->sprefix . 'mall_offer_linkgoods', $this->sprefix . 'mall_member_level', $this->sprefix . 'mall_member', $this->sprefix . 'mall_offer_p', $this->sprefix . 'mall_offer_deliverarea', $this->sprefix . 'mall_offer_t', $this->sprefix . 'mall_offer_ncat', $this->sprefix . 'mall_offer_ncon', $this->sprefix . 'mall_offer_link', $this->sprefix . 'mall_orders', $this->sprefix . 'mall_items', $this->sprefix . 'mall_offer');
	}

	public function required_dirs()
	{
		return array('/syssite/home/shop/1/pictures/brandimg/', '/syssite/home/shop/1/pictures/newsimg/', '/syssite/home/shop/1/pictures/productsimg/big/', '/syssite/home/shop/1/pictures/productsimg/small/', '/syssite/home/shop/1/pictures/linkimg/', '/cert/');
	}
	...
	public function process_brand()
	{
		...
		while ($row = $this->sdb->fetchRow($res)) {
			$brand = array('brand_name' => $row['brand_name'], 'brand_desc' => '', 'site_url' => ecs_iconv($this->scharset, $this->tcharset, addslashes($row['brand_site_url'])), 'brand_logo' => ecs_iconv($this->scharset, $this->tcharset, addslashes($row['brand_logo'])));
	...
	public function process_goods()
	{
		...
		while ($row = $this->sdb->fetchRow($res)) {
			...
			else if ($row['ifobject'] == '1') {
				$goods['is_real'] = '1';
			}
			else if ($row['ifobject'] == '2') {
				continue;
			}
			else if ($row['ifobject'] == '3') {
				continue;
			}
			else {
				continue;
			...
			$sql = 'SELECT DISTINCT pv.prop_id, pv.prop_value ' . 'FROM ' . $this->sprefix . 'mall_goods_prop_grp_value AS gp, ' . $this->sprefix . 'mall_prop_value AS pv ' . 'WHERE gp.prop_value_id = pv.prop_value_id ' . 'AND gp.gid = \'' . $row['gid'] . '\'';
