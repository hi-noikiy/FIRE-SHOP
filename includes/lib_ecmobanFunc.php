...
function get_admin_ru_id()
{
	...
	$sql = 'SELECT ru_id FROM ' . $GLOBALS['ecs']->table('admin_user') . ' WHERE user_id = \'' . $admin_id . '\' LIMIT 1';
	return $GLOBALS['db']->getRow($sql);
}
	...
	foreach ($res as $key => $row) {
		...
		if ($row['steps_style'] == 1) {
			...
			if ((0 < $ec_shop_bid) || (0 < $brand_m['brand_id'])) {
				...
				if (!empty($bank_name_letter)) {
					...
					if (0 < $bRes) {
						...
						$back_url = 'merchants_steps.php?step=stepThree&pid_key=1';
			...
			if (!empty($ec_rz_shopName)) {
				$parent = array('user_id' => $_SESSION['user_id'], 'shoprz_brandName' => $ec_shoprz_brandName, 'shop_class_keyWords' => $ec_shop_class_keyWords, 'shopNameSuffix' => $ec_shopNameSuffix, 'rz_shopName' => $ec_rz_shopName, 'hopeLoginName' => $ec_hopeLoginName);
...
function get_produts_list2($goods_list, $attr_num = 0)
{
	...
	for ($i = 0; $i < count($goods_list); $i++) {
		...
		for ($j = 0; $j < $attr_num; $j++) {
			if ($j == ($attr_num - 1)) {
				$attr_name[$j] = $goods_list[$i]['goods_attr' . $j];
				$sql = 'SELECT goods_attr_id FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' WHERE attr_value=\'' . $goods_list[$i]['goods_attr' . $j] . '\' AND goods_id = \'' . $arr[$i]['goods_id'] . '\'';
				$attr[$j] = $GLOBALS['db']->getOne($sql);
			}
			else {
...
function get_warehouse_id_attr_number($goods_id, $attr_id = '', $admin_id, $warehouse_id, $area_id, $model_attr = '', $store_id = 0)
{
	if (empty($model_attr)) {
	...
	$sql = 'SELECT product_number, product_sn ' . $bar_code . ' FROM ' . $GLOBALS['ecs']->table($table) . ' WHERE goods_id = \'' . $goods_id . '\' AND goods_attr = \'' . $attr_id . '\'' . $where;
	return $GLOBALS['db']->getRow($sql);
}
...
function get_goods_cart_num($goods_id = 0, $warehouse_id = 0)
{
	...
	$sql = 'SELECT goods_number FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE goods_id = \'' . $goods_id . '\' AND warehouse_id = \'' . $warehouse_id . '\' AND ' . $sess_id;
	return $GLOBALS['db']->getOne($sql);
}
		...
		do {
			...
			if (($GLOBALS['_CFG']['sms_order_placed'] == '1') && ($sms_shop_mobile != '')) {
				if (!empty($auto_sms)) {
					...
					$smsParams = array('shop_name' => $shop_name, 'order_sn' => $row['order_sn'], 'consignee' => $row['consignee'], 'order_region' => $order_region, 'address' => $row['address'], 'order_mobile' => $row['mobile'], 'mobile_phone' => $sms_shop_mobile);
...
function get_all_area_list($parent_id = 0, $region_type = 1)
{
	...
	$sql = 'SELECT r.region_id, r.region_name FROM ' . $GLOBALS['ecs']->table('region_warehouse') . ' as rw, ' . $GLOBALS['ecs']->table('region') . ' as r ' . ' WHERE ' . $where . ' GROUP BY r.region_id order by r.region_id ASC';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function goodsShippingFee($goods_id = 0, $warehouse_id = 0, $region = array())
{
	...
	$sql = ' SELECT g.*, g.user_id AS ru_id, ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS goods_price ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE g.goods_id = \'' . $goods_id . '\' LIMIT 1';
	$goodsInfo = $GLOBALS['db']->getRow($sql);
	$sellerShippingInfo = get_seller_shipping_type($goodsInfo['user_id']);
	$where = '';
	if (isset($region[5]) && !empty($region[5])) {
		$street_list = explode(',', $region[5]);
		$region = array($region[0], $region[1], $region[2], $region[3], $region[4]);
		$where .= 'r.region_id ' . db_create_in($region) . ' OR r.region_id ' . db_create_in($street_list);
	}
	else {
	...
	$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' . $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' . $GLOBALS['ecs']->table('area_region') . ' AS r ' . 'WHERE ' . $where . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 AND a.ru_id = \'' . $goodsInfo['user_id'] . '\' GROUP BY s.shipping_id ORDER BY s.shipping_order';
		...
		if ($is_shipping) {
			if ($goodsInfo['is_shipping'] == 1) {
				...
				if ($GLOBALS['_CFG']['freight_model'] == 0) {
					...
					$region = array('province' => $region[1], 'city' => $region[2], 'district' => $region[3]);
...
function get_goods_transport_tpl($goodsInfo = array(), $region = array(), $shippingInfo = array(), $goods_number = 1)
{
	...
	$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' . $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' . $GLOBALS['ecs']->table('area_region') . ' AS r ' . 'WHERE r.region_id ' . db_create_in($region) . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id ' . $where . ' AND s.enabled = 1 AND a.ru_id = \'' . $goodsInfo['user_id'] . '\'';
	$val = $GLOBALS['db']->getRow($sql);
	$is_shipping = 0;
...
function get_order_transport($goods_list, $consignee, $shipping_id, $shipping_code)
{
	$sprice = 0;
	$type_left = array();
	$freight = 0;
	$shippingInfo = array('shipping_id' => $shipping_id);
	if ($goods_list && ($shipping_code != 'cac')) {
			...
			if ($row['freight'] && ($row['is_shipping'] == 0)) {
				if ($row['freight'] == 1) {
					...
					if ($trow['freight_type']) {
						$region = array_values($consignee);
						$row['user_id'] = $row['ru_id'];
						$transport_tpl = get_goods_transport_tpl($row, $region, $shippingInfo, $row['goods_number']);
						$sprice += $transport_tpl['shippingFee'];
					}
					else {
						$transport = array('top_area_id', 'area_id', 'tid', 'ru_id', 'sprice');
		...
		$arr = array('sprice' => $sprice + $unified_total, 'freight' => $freight);
	}
	else {
...
function get_goods_shipping_fee($goodsInfo, $val, $sellerShippingInfo, $warehouse_id)
{
	if ($GLOBALS['_CFG']['freight_model'] == 0) {
		...
		$goods = array('weight' => $goodsInfo['goods_weight'], 'goods_price' => $goodsInfo['shop_price'], 'number' => 1, 'ru_id' => $goodsInfo['user_id'], 'warehouse_id' => $warehouse_id, 'warehouse_name' => '');
...
{
	$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' . $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' . $GLOBALS['ecs']->table('area_region') . ' AS r ' . 'WHERE r.region_id ' . db_create_in($sellerOrderInfo['region']) . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.shipping_id = \'' . $sellerOrderInfo['shipping_id'] . '\' AND s.enabled = 1 AND a.ru_id = \'' . $sellerOrderInfo['ru_id'] . '\' ORDER BY s.shipping_order';
	$val = $GLOBALS['db']->getRow($sql);
	...
	$consignee = array('city' => $sellerOrderInfo['region'][2]);
...
function attr_group()
{
	...
	if (func_num_args() == 1) {
		return call_user_func_array('attr_group', $t[0]);
	}
...
function get_coupons_type_info($cou_type = '1,2,3,4', $ru_id = 0)
{
	$where = '';
...
function get_user_coupons_list($user_id = '', $is_use = false, $total = '', $cart_goods = false, $user = true)
{
	$time = gmtime();
	if ($is_use && isset($total) && $cart_goods) {
		...
		foreach ($res as $key => $row) {
			$row['goods_id'] = substr($row['goods_id'], 0, -1);
			$row['cat_id'] = substr($row['cat_id'], 0, -1);
			$sql = 'SELECT c.*, cu.uc_id, s.shop_name FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' AS cu ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('coupons') . ' AS c ON cu.cou_id = c.cou_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' AS s ON c.ru_id = s.ru_id ' . ' WHERE c.review_status = 3 AND c.cou_end_time > ' . $time . ' AND ' . $time . ' > c.cou_start_time' . ' AND ' . $row['order_total'] . ' >= c.cou_man AND c.ru_id = \'' . $row['seller_id'] . '\'' . ' AND cu.order_id = 0 AND cu.user_id = \'' . $user_id . '\' AND s.status = 1';
			$couarr[$key] = $GLOBALS['db']->getAll($sql);
			$goods_ids = (!empty($row['goods_id']) ? array_unique(explode(',', $row['goods_id'])) : array());
			$goods_cats = (!empty($row['cat_id']) ? array_unique(explode(',', $row['cat_id'])) : array());
		...
		if (!empty($user_id) && $user) {
			$where = ' WHERE cu.user_id IN(' . $user_id . ') AND c.review_status = 3 AND s.status = 1';
		}
		else if (!empty($user_id)) {
			$where = ' WHERE cu.user_id IN(' . $user_id . ') AND c.review_status = 3 AND s.status = 1 GROUP BY c.cou_id';
		}

		$sql = ' SELECT c.*,cu.*,o.order_sn,o.add_time, s.shop_name FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' cu LEFT JOIN ' . $GLOBALS['ecs']->table('coupons') . ' c ON c.cou_id=cu.cou_id LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' o ON cu.order_id = o.order_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' AS s ON c.ru_id = s.ru_id ' . ' ' . $where . ' ';
		$res = $GLOBALS['db']->getAll($sql);
		return $res;
	}
}
...
function return_act_range_ext($act_range_ext, $userFav_type, $act_range)
{
	if ($act_range_ext) {
		if (($userFav_type == 1) && ($act_range == FAR_BRAND)) {
			...
			$brand_sql = 'SELECT lb.bid FROM ' . $GLOBALS['ecs']->table('link_brand') . ' as lb, ' . $GLOBALS['ecs']->table('brand') . ' as b WHERE lb.brand_id = b.brand_id AND lb.brand_id ' . db_create_in($id_list);
...
function shippingArea($ru_id = 0)
{
	$sql = ' SELECT region_id FROM ' . $GLOBALS['ecs']->table('shipping_area') . ' AS s LEFT JOIN ' . $GLOBALS['ecs']->table('area_region') . ' AS a ON a.shipping_area_id = s.shipping_area_id WHERE s.ru_id = \'' . $ru_id . '\' ';
	$region_ids = $GLOBALS['db']->getAll($sql);
	$region_ids_str = '';

	foreach ($region_ids as $v) {
		$region_ids_str .= $v['region_id'] . ',';
		$region_id = region_parent($v['region_id']);
		$region_ids_str .= $region_id;
		$region_children = region_children($v['region_id']);
		$region_ids_str .= $region_children;
	}

	$region_ids = explode(',', rtrim($region_ids_str, ','));
