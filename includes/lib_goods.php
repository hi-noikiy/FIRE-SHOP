...
function get_top10($cats = 0, $presale = '', $ru_id = 0, $warehouse_id = 0, $area_id = 0)
{
	...
	for ($count = count($arr); $i < $count; $i++) {
		...
		if (0 < $arr[$i]['market_price']) {
			$discount_arr = get_discount($arr[$i]['goods_id']);
		}
...
function get_promote_goods($cats = '', $warehouse_id = 0, $area_id = 0)
{
	...
	foreach ($result as $idx => $row) {
		...
		if (0 < $row['market_price']) {
			$discount_arr = get_discount($row['goods_id'], $warehouse_id, $area_id);
		}
...
function get_goods_gallery($goods_id, $gallery_number = 0)
{
	...
	$sql = 'SELECT img_id, img_url, thumb_url, img_desc, external_url' . ' FROM ' . $GLOBALS['ecs']->table('goods_gallery') . ' WHERE goods_id = \'' . $goods_id . '\'  ORDER BY img_desc ASC LIMIT ' . $gallery_number;
...
{
	$sql = 'SELECT cat_name, cat_alias_name FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE cat_id = \'' . $cat_id . '\' AND is_show = 1';
...
{
	$group_buy_id = intval($group_buy_id);
	$sql = 'SELECT goods_id ' . 'FROM ' . $GLOBALS['ecs']->table('goods_activity') . 'WHERE review_status = 3 AND act_id = \'' . $group_buy_id . '\' ' . 'AND act_type = \'' . GAT_GROUP_BUY . '\'';
...
{
	$sql = 'SELECT rank_points FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id = \'' . $_SESSION['user_id'] . '\' LIMIT 1';
	$user_res = $GLOBALS['db']->getRow($sql);
	$sql = 'SELECT min(max_points) AS rank_points FROM ' . $GLOBALS['ecs']->table('user_rank') . ' WHERE special_rank = 0 LIMIT 1';
	$res = $GLOBALS['db']->getRow($sql);

	if ($user_res['rank_points'] <= $res['rank_points']) {
		$regular = 1;
	}
	else {
		$regular = 0;
...
function sec_goods_stats($sec_id)
{
	...
	$sql = 'SELECT COUNT(*) AS total_order, SUM(g.goods_number) AS total_goods ' . 'FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o, ' . $GLOBALS['ecs']->table('order_goods') . ' AS g ' . ' WHERE o.order_id = g.order_id ' . ' AND g.extension_code = \'seckill' . $sec_id . '\' ' . ' AND g.goods_id = \'' . $sec_goods_id . '\' ' . ' AND (order_status = \'' . OS_CONFIRMED . '\' OR order_status = \'' . OS_UNCONFIRMED . '\')';
	$stat = $GLOBALS['db']->getRow($sql);
...
function seckill_goods_results($date = '', $left_join = '', $where = '', $day = 0)
{
	...
	$seckill = $GLOBALS['db']->getRow($sql);
	$where .= ' AND s.sec_id ' . db_create_in($seckill['sec_id']);
	$sql = ' SELECT g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.goods_name, sg.id, sg.sec_price, sg.sec_num, sg.sec_limit, stb.begin_time, stb.end_time, s.sec_id, s.acti_title, s.acti_time FROM ' . $GLOBALS['ecs']->table('seckill_goods') . ' AS sg LEFT JOIN ' . $GLOBALS['ecs']->table('seckill_time_bucket') . ' AS stb ON sg.tb_id = stb.id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seckill') . ' AS s ON s.sec_id = sg.sec_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON sg.goods_id = g.goods_id ' . $left_join . ' WHERE s.is_putaway = 1 AND s.begin_time <= \'' . $date_begin . '\' AND s.acti_time >= \'' . $date . '\' ' . $where . '  ORDER BY stb.begin_time ASC ';
	$res = $GLOBALS['db']->getAll($sql);
	...
	if ($res) {
		foreach ($res as $k => $v) {
			...
			$res[$k]['percent'] = intval(($res[$k]['sales_volume']['valid_goods'] / ($v['sec_num'] + $res[$k]['sales_volume']['valid_goods'])) * 100);
