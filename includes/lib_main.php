...
function get_user_info($id = 0)
{
	...
	if ($user['user_name'] != $_SESSION['user_name']) {
		...
		if (isset($_SESSION['identifier']) && !empty($_SESSION['identifier'])) {
			$sql = 'SELECT u.user_id, ua.user_name FROM ' . $GLOBALS['ecs']->table('users') . ' AS u,' . $GLOBALS['ecs']->table('users_auth') . ' AS ua' . ' WHERE u.user_id = ua.user_id AND ua.user_id = \'' . $id . '\' AND ua.identifier = \'' . $_SESSION['identifier'] . '\' LIMIT 1';
			$auth_row = $GLOBALS['db']->getRow($sql);
			$user['username'] = $auth_row['user_name'];
		}
	}
	else {
		$user['username'] = $user['user_name'];
...
function recalculate_price()
{
	...
	foreach ($res as $row) {
		...
		if (($xiangouInfo['is_xiangou'] == 1) && ($start_date < $nowTime) && ($nowTime < $end_date)) {
			$sql = 'SELECT goods_number FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE rec_id = \'' . $row['rec_id'] . '\' LIMIT 1';
			$cartGoodsNumInfo = $GLOBALS['db']->getRow($sql);
		...
		if ($error == 1) {
			...
			if (0 < $rec_id) {
				...
				if ($row['extension_code'] != 'package_buy') {
					$goods_sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' SET goods_price = \'' . $goods_price . '\'' . $sess . $set . ' WHERE goods_id = \'' . $row['goods_id'] . '\' AND rec_id = \'' . $row['rec_id'] . '\' AND warehouse_id = \'' . $row['warehouse_id'] . '\'';
				}
				else if (!empty($_SESSION['user_id'])) {
					$sess_id = 'AND session_id = \'' . real_cart_mac_ip() . '\' ';
					$goods_sql = 'UPDATE ' . $GLOBALS['ecs']->table('cart') . ' SET ' . $pack_sess . $set . 'WHERE rec_id = \'' . $row['rec_id'] . '\'';
				}
			}
	...
	if (!empty($_SESSION['user_id'])) {
		$sess_id = ' session_id = \'' . real_cart_mac_ip() . '\' ' . ' AND';
	}

	$GLOBALS['db']->query('DELETE FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . ' is_gift > 0');
}
...
{
	$sql = 'SELECT link_logo, link_name, link_url FROM ' . $GLOBALS['ecs']->table($table) . ' ORDER BY show_order';
	$res = $GLOBALS['db']->getAll($sql);
	$links['img'] = $links['txt'] = array();

	foreach ($res as $row) {
		if (($GLOBALS['_CFG']['open_oss'] == 1) && $row['link_logo']) {
			$bucket_info = get_bucket_info();
			$row['link_logo'] = $bucket_info['endpoint'] . $row['link_logo'];
		}
...
function license_info()
{
	if (0 < $GLOBALS['_CFG']['licensed']) {
		...
		$license = '<a href="http://www.ecmoban.com/license.php?product=ecshop_b2c&url=' . urlencode($host) . "\" target=\"_blank\"\r\n>&nbsp;&nbsp;Licensed</a>";
		return $license;
	}
	else {
		return '';
