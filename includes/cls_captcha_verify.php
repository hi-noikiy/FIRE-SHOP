...
class Verify
{
	...
	public function check($code = '', $seKey = '', $id = '', $type = '')
	{
		$now_time = gmtime();

		if ($seKey != '') {
			$key = $this->authcode($seKey) . $id;
		}
		else {
		...
		if ($this->expire < ($now_time - $secode['verify_time'])) {
			$_SESSION[$key] = NULL;
			return false;
		}
	...
	public function entry($id = '')
	{
		...
		$ttfPath = ROOT_PATH . '/data/verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
		...
		if ($this->useZh) {
			...
			for ($i = 0; $i < $this->length; $i++) {
				...
				$codeNX += mt_rand($this->fontSize * 1.2, $this->fontSize * 1.6000000000000001);
				imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.6000000000000001, $this->_color, $this->fontttf, $code[$i]);
			}
		}
	...
	private function _writeCurve()
	{
		...
		$b = mt_rand((0 - $this->imageH) / 4, $this->imageH / 4);
		$f = mt_rand((0 - $this->imageH) / 4, $this->imageH / 4);
		$T = mt_rand($this->imageH, $this->imageW * 2);
		$w = (2 * M_PI) / $T;
		$px1 = 0;
		$px2 = mt_rand($this->imageW / 2, $this->imageW * 0.80000000000000004);
	...
	{
		$path = ROOT_PATH . '/data/verify/bgs/';
		$dir = dir($path);
		$bgs = array();
