...
class ECS
{
	...
	public function http()
	{
		...
		if (isset($_SERVER['HTTPS'])) {
			...
			return isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && (strtolower($_SERVER['HTTP_X_FORWARDED_PROTO']) != 'off') ? 'https://' : 'http://';
		}
		else {
			...
			if (count($ex_rec) <= 1) {
				if ($type == 1) {
					...
					if ((strpos($str, 'updatexml') !== false) || (strpos($str, 'extractvalue') !== false) || (strpos($str, 'floor') !== false)) {
						...
						if ((strpos($str, 'or') !== false) || (strpos($str, 'and') !== false)) {
							$str = '';
						}
					}
				}
				...
				foreach ($ex_rec as $key => $row) {
					if ($type == 1) {
						...
						if ((strpos($row, 'updatexml') !== false) || (strpos($row, 'extractvalue') !== false) || (strpos($row, 'floor') !== false)) {
							...
							if ((strpos($row, 'or') !== false) || (strpos($row, 'and') !== false)) {
								$row = '';
							}
						}
