...
function get_collection_goods($user_id, $record_count, $page, $pageFunc, $size = 10)
{
	...
	foreach ($res as $key => $row) {
		...
		$price_info = get_goods_one_attr_price($row['goods_id'], $region_id, $area_id, $price_other);
...
function get_collection_brands($user_id, $record_count, $page, $pageFunc, $size = 5)
{
	...
	$limit = $collection->limit;
	$pager = $collection->fpage(array(0, 4, 5, 6, 9));
	$sql = '(SELECT cb.*, b.brand_name, b.brand_logo FROM ' . $GLOBALS['ecs']->table('collect_brand') . ' AS cb LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON cb.brand_id = b.brand_id WHERE cb.ru_id = 0 AND b.brand_id > 0 AND cb.user_id = \'' . $user_id . "')\r\n            UNION\r\n            (SELECT cb_new.*, msb.brandName AS brand_name, msb.brandLogo AS brand_logo FROM " . $GLOBALS['ecs']->table('collect_brand') . ' AS cb_new LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_shop_brand') . ' AS msb ON cb_new.brand_id = msb.bid ' . 'WHERE cb_new.ru_id > 0 AND msb.bid > 0) ORDER BY rec_id DESC ' . $limit;
	$res = $GLOBALS['db']->getAll($sql);
	$brand_list = array();
	...
	foreach ($res as $key => $row) {
		...
		$mbid = ((0 < $row['brand_id']) && $row['ru_id'] ? $row['brand_id'] : 0);

		if ($mbid) {
			$ru_id = $GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['ecs']->table('merchants_shop_brand') . ' WHERE bid=\'' . $mbid . '\' ');
			$brand_list[$row['rec_id']]['collect_count'] = get_collect_brand_user_count($mbid, $ru_id);
			$brand_list[$row['rec_id']]['is_collect'] = get_collect_user_brand($mbid, $ru_id);
		}
		else {
...
function get_link_brand_str($brand_id = 0)
{
	$sql = 'SELECT bid FROM ' . $GLOBALS['ecs']->table('link_brand') . ' WHERE brand_id = \'' . $brand_id . '\'';
	$res = $GLOBALS['db']->getAll($sql);
	$str = '';
...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
	...
	if ($order_id) {
		$sql .= ' WHERE parent_id = 0 AND msg_status = 0 AND order_id = \'' . $order_id . '\' AND user_id = \'' . $user_id . '\' ORDER BY msg_time DESC';
	}
	else {
		$sql .= ' WHERE parent_id = 0 AND msg_status = 0 AND user_id = \'' . $user_id . '\' AND user_name = \'' . $_SESSION['user_name'] . '\' AND order_id=0 ORDER BY msg_time DESC';
	}
...
{
	$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('user_account') . ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid)' . ' VALUES (\'' . $surplus['user_id'] . '\', \'\', \'' . $amount . '\', \'' . gmtime() . '\', 0, \'\', \'' . $surplus['user_note'] . '\', \'' . $surplus['process_type'] . '\', \'' . $surplus['payment'] . '\', 0)';
	$GLOBALS['db']->query($sql);
	return $GLOBALS['db']->insert_id();
}
