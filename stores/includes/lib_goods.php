...
{
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('link_goods') . ' SET ' . ' goods_id = \'' . $goods_id . '\' ' . ' WHERE goods_id = \'0\'' . ' AND admin_id = \'' . $_SESSION['store_user_id'] . '\'';
	$GLOBALS['db']->query($sql);
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('link_goods') . ' SET ' . ' link_goods_id = \'' . $goods_id . '\' ' . ' WHERE link_goods_id = \'0\'' . ' AND admin_id = \'' . $_SESSION['store_user_id'] . '\'';
	$GLOBALS['db']->query($sql);
}

function handle_group_goods($goods_id)
{
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('group_goods') . ' SET ' . ' parent_id = \'' . $goods_id . '\' ' . ' WHERE parent_id = \'0\'' . ' AND admin_id = \'' . $_SESSION['store_user_id'] . '\'';
	$GLOBALS['db']->query($sql);
}

function handle_goods_article($goods_id)
{
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('goods_article') . ' SET ' . ' goods_id = \'' . $goods_id . '\' ' . ' WHERE goods_id = \'0\'' . ' AND admin_id = \'' . $_SESSION['store_user_id'] . '\'';
	$GLOBALS['db']->query($sql);
}

function handle_goods_area($goods_id)
{
	$sql = 'UPDATE ' . $GLOBALS['ecs']->table('link_area_goods') . ' SET ' . ' goods_id = \'' . $goods_id . '\' ' . ' WHERE goods_id = \'0\'' . ' AND ru_id = (SELECT ru_id FROM ' . $GLOBALS['ecs']->table('admin_user') . '  WHERE user_id = \'' . $_SESSION['store_user_id'] . '\') ';
	$GLOBALS['db']->query($sql);
}
...
function delete_goods($goods_id)
{
	...
	$GLOBALS['db']->query($sql);
	$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('store_products') . ' WHERE goods_id ' . db_create_in($goods_id);
	$GLOBALS['db']->query($sql);
...
function build_attr_html($cat_id, $goods_id = 0, $model_attr = 0)
{
	...
	foreach ($attr as $key => $val) {
		...
		if ($new_model_attr == 0) {
			$html .= (($val['attr_type'] == 1) || ($val['attr_type'] == 2) ? '<span name="span_attrPrice[]">' . $GLOBALS['_LANG']['spec_price'] . ' <input type="text" class="text_3 mr10" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' . '</span>' : ' <input type="hidden" name="attr_price_list[]" value="0" />');
			...
			$html .= (($val['attr_type'] == 1) || ($val['attr_type'] == 2) ? '<span name="span_attrPrice[]" style="display:none;">' . $GLOBALS['_LANG']['spec_price'] . ' <input type="text" class="text_3 mr10" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' . '</span>' : ' <input type="hidden" name="attr_price_list[]" value="0" />');
		...
		if (!empty($val['attr_value']) && (0 < $val['attr_type'])) {
			if (!empty($attr_name)) {
...
function goods_list($is_delete, $real_goods = 1, $conditions = '')
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT g.goods_id, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, ' . ' g.is_promote, g.model_price, g.model_inventory, g.model_attr, g.review_status, g.review_content, g.store_best, g.store_new , g.store_hot , g.is_real, g.is_shipping, g.stages,  ' . ' g.is_alone_sale, g.is_xiangou, g.promote_end_date, g.xiangou_end_date ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . ' left join ' . $GLOBALS['ecs']->table('goods_cat') . ' as gc on g.goods_id = gc.goods_id' . ' WHERE g.is_delete=\'' . $is_delete . '\' ' . $where . ' group by g.goods_id' . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
