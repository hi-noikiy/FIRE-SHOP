...
function store_order_list($ru_id = 0, $store_id = 0)
{
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('store_order') . ' AS so ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON o.order_id = so.order_id ' . $where;
	$filter['record_count'] = $GLOBALS['db']->getOne($sql);
	$filter = page_and_size($filter);
	$arr = array();
	$sql = 'SELECT so.*, o.order_id, o.order_sn, o.add_time, o.consignee, o.order_status, o.shipping_status, o.pay_status, so.store_id, ' . '(' . order_amount_field('o.') . ') AS total_fee ' . 'FROM ' . $GLOBALS['ecs']->table('store_order') . ' AS so ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON o.order_id = so.order_id ' . $where . 'ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
...
{
	$sql = ' SELECT og.order_id, og.goods_id, og.goods_name, og.goods_price, og.goods_number, g.goods_thumb FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = og.goods_id ' . ' WHERE og.order_id = \'' . $order_id . '\' ';
	...
	if ($pick_code) {
		...
		if (($order_info['pay_status'] != 2) && !in_array($payment['pay_code'], array('bank', 'cod'))) {
			...
			if ($store_order_info && ($store_order_info['pick_code'] == $pick_code)) {
				$sql = ' UPDATE ' . $GLOBALS['ecs']->table('order_info') . ' SET order_status = \'' . OS_SPLITED . '\', pay_status = \'' . PS_PAYED . '\', shipping_status = \'' . SS_RECEIVED . '\' ' . ' WHERE order_id = \'' . $order_id . '\' ';
